...
| `excluded` | **25** | 反応済み・抑止側は 1 件ずつレコードを読む。50 でも時間切れ（504）を実測、**25 で全窓完走**（267 件 / 11 窓）|

### 「1 件も見つからない」を失敗にしない（2026-09-19 本番実測）

`POST /v3/marketing/contacts/search/emails` は、**渡した全員が SendGrid に居ないと 404** を返す。
これを失敗として扱うと「引けなかった」と「居ない」が混ざる。混ざると、
**間違った list に居る人を外さずに正しい list へ足す**ことになり、両方に載って **2 通届く**。

- 404 は**空の結果**として扱う（居ないだけ）
- それ以外で**状態を引けなかった人は計画から外す**（fail closed。分からないなら触らない）
- 応答に `触らなかった_状態不明` を件数で返す

---

## 15. 「予約してよい」の判定（2026-09-19 改訂）

### 反応が続くこと自体は異常ではない

旧 AK が送った分の `delivered` / `open` は**あとから届き続ける**。届けば ENGAGED が増え、
その人は webhook が list から外す。**これは設計どおりの動作**で、異常ではない。

**だから「動きが止まるまで待つ」運用はしない。** 反応は止まらないので、
止まるのを待つと**配信開始が無期限に延びる**（2026-09-19 に実際そうなりかけた）。

### 待たなくてよい理由（構造）

- Single Send は**送信時**に list の在籍を解決する。予約時点の名簿で固定されない
- 反応した人は webhook が**その場で** list から外す（§11-f / `sendgridSelectionExit.js`）
- list は「**次に送る号**」そのものなので、既に送った号の list には居ない
  （＝**既送信号の再送は構造的に起きない**）

つまり予約後に反応が届いても、**次の号から自動で外れる**。

### 予約直前のゲート（**これだけ満たせば予約してよい**）

予約の直前に、**その時点の最新状態で全件照合**する。

| # | 確認 | 合格条件 |
|---|---|---|
| 1 | reconcile 下見（`active` 全窓 ＋ `excluded` 全窓）| **変更予定 0 件** |
| 2 | 総数の整合 | `A（AK active） − provider rejected = L合計` |
| 3 | 通し番号ごとの整合 | `D[n] = L[n] + その号の provider rejected` |
| 4 | Single Send | 27 通が draft・未予約・送信 0 |
| 5 | 旧 AK | `engine=sendgrid`（prospect 宛の enqueue が無い）|

1 が 0 でなければ **reconcile を 1 回流してから**もう一度 1〜3 を見る。
**0 になったら予約する。** そこから先の反応は webhook が引き取る。

### 監視（**判断材料であって、延期の理由にしない**）

`A` / `D` / `E` / `L` を read-only で取り、**不整合（上の 2・3 の差）が増えていないか**だけ見る。
ENGAGED が増えること・list が減ることは**正常な動き**として扱う。

- 見るのは**不整合の増減**であって、数字が動かないことではない
- 不整合が増え続ける場合だけ、原因（webhook の失敗など）を調べる
- **新しい cron や監査基盤は作らない。** 既存の read-only な admin API で足りる
