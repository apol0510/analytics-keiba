...
### 第 1 層 `/free-prediction/`（本件のために役割・公開範囲を変えない）

有料版プレビュー／サンプルという**位置づけを維持**する。第 2 層を兼ねさせる目的で
解放範囲を広げる変更は行わない。

> **射程の限定**: これは**第 2 層を理由とする変更の禁止**である。
> `/free-prediction/` の通常の UI 改善・不具合修正まで永久凍結する仕様ではない。
> ただし公開範囲（下表の左右の切り分け）を動かす変更は、本節とは**別の判断**として扱う。
...
未登録は ◎ のみ表示。無料登録（`/free-signup/`）で ○▲△ と全出走馬が開く。
**買い目は無料会員でも解放しない。**
...
- **第 2 層を兼ねさせる目的で** `/free-prediction/` の公開範囲を広げること（第 2 層は**別の新規ページ**で作る。`docs/decisions.md` §2026-08-19）。※ `/free-prediction/` の通常の UI 改善・不具合修正はここに含まない
