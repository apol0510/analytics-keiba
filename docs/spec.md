...
4. まだ取得していない
5. 取得の記録を本番へ保存できる（gate 有効）

⚠️ **`salePaused` は取得資格の必須条件にしない。**
⚠️ **未開始の会員は取得も予約も申込（クーポン適用）もできない**（fail closed）。
⚠️ 販売停止中でも、開始済み・期限内なら**取得できる**（ただし**購入はできない**）。
⚠️ **既に取得済みのクーポンは保持する**（未開始・期限切れでも保有の事実は消えない）。

#### 通常フロー

1. admin で対象会員の「▶ この会員の再募集を開始する」
2. その会員だけ販売停止が解除され、`reopenStartsAt` が確定する
3. その会員は **開始から 14 日間**、未取得ならクーポンを取得できる
   （マイページ / クーポンページ / 受付休止ページのいずれからでも）
4. 取得後は **58,000円**で申し込める
