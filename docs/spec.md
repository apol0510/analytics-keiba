...
の**各段と離脱**を運用確認できるようにする。目的はアクセス数を見ることではなく、
**どの段で離脱し、どこから有料転換につながっているか**を見ることである。

| 段 | 見るもの |
|---|---|
| 無料予想 / results-showcase / pricing 到達 | `page_view`（URL で判別できるのでイベントを増やさない）|
| 申込開始 | `application_start`（モーダルが開いた）|
| 申込成功 | `application_submitted`（**サーバーが受理を返した後だけ**）|

**確定した制約**

1. ボタンのクリックを申込成功にしない。
2. `application_submitted` は**入金確認（課金確定）ではない**。入金確認は Airtable の
   手作業でブラウザに届かない。だから GA4 の `purchase` は使わない。
3. GA4 へ**個人情報を送らない**。送るのは閉じた語彙 `plan` / `plan_type` だけ。
4. 計測コードを**ページへ書き足さない**（申込導線は 13 ページにコピペで散在しており、
   ページに書くと必ず直し漏れる）。既存関数を 1 か所で包む。
5. 予想ロジック・予想データ・会員権限・決済ロジックは変更しない。

詳細仕様・GA4 管理画面側の作業・確認手順・rollback は
...
- 高齢で PC・スマートフォン操作が苦手
- ログインや入金連絡フォームの操作ができない
- メールや問い合わせ等で本人・入金内容を運営者が確認できている
- その他、通常の入金連絡操作を顧客自身に求めることが現実的でない

といった場合に**限り**、運営者が顧客本人に代わって「入金連絡」を登録できるようにする。
これは**通常の申込経路ではなく、運営者による応急・例外対応**とする。

**Airtable 直接編集を通常の解決方法にしない。**

## 意味論（誤解を作らない）

これは「顧客としてログインしてなりすます」機能では**ない**。
**運営者が顧客本人の代わりに申請情報を登録する正式な運営操作**として実装する。

通常フォームの「ログインセッションのメールアドレス固定」を**緩めない**。
一般ユーザーへ開放しない。代理登録は別経路・別認可で提供する。

## 既存フローの再利用（第二の昇格処理を作らない）

代理登録後に別系統の会員昇格ロジックを**新設しない**。
`RequestedPlan` / `RequestedPlanType` / `RequestedAmount` / `Status='pending'` /
`PaymentConfirmed` / `buildConfirmationFields` / 有効期限計算 / `PaidAt` /
利用開始メール処理は**すべて既存の正規経路**を通す。

二重昇格・二重メール・二重処理を防ぐため、
**昇格の入口は `PaymentConfirmed` ただ 1 つ**という現行の設計を変えない。

## 実入金額

サイト掲載価格と実際の入金額が一致しないケースでも、運営者が**実際の着金額**を
登録できるようにする（例: 掲載 ¥44,820 / 着金 ¥44,800）。

**入金額を掲載価格へ捏造しない。**
`RequestedAmount` は通常フォームでも「その申込の金額」であり、キャンペーン・
クーポンが無ければ顧客申告の振込額そのものなので、代理登録でも実入金額を入れる。

昇格後も実入金額を残すには Airtable の列追加が要る。**本番 schema 変更は行わない**。
列が用意されたときだけ書けるよう env gate（`PROXY_PAYMENT_NOTICE_FIELDS_READY=1`）で
分岐し、列が無い間は構造化ログに残す。

## 監査

後から「顧客本人が送信したのか / 運営者が代理登録したのか」を区別できるようにする。
既存 schema に適切な列が無いため、

- 列が作成済み（env gate ON）… `ApplicationSource='admin_proxy'` / `ApplicationProxyBy` /
  `ApplicationProxyAt` / `ReceivedAmount` / `ApplicationProxyReason`
- 列が未作成（既定）… Function の構造化ログ `event: 'admin_proxy_payment_notice'`

列の作成は**本番 schema 変更＝高リスク操作**なので、承認境界で止める。

## 認可（fail closed）

admin 以外・未認証・URL 直打ち・直接 POST のいずれでも通らないこと。
...
- **会員レコードの新規作成**（未登録アドレスは拒否）
- **顧客宛メールの送信**（`PaymentConfirmed` より前に利用開始メールを出さない）

## 完成条件

運営者が管理画面で「顧客メール / プラン / 契約種別 / 実入金額」を入れて確認・登録でき、
そのあと Airtable で `PaymentConfirmed` にチェックを入れるだけで、
**既存経路がそのまま昇格とメール送信を行う**こと。

正本: [`astro-site/docs/BANK_TRANSFER_FLOW.md`](../astro-site/docs/BANK_TRANSFER_FLOW.md)
の「運営者による代理入金連絡」節。
...
1〜数個のボタンで決まる。内部用語（phase / override / eligibility）を主役にしない。

| 役割 | 単一源 |
|---|---|
| 状態の要約・次の操作 | `astro-site/src/lib/premiumPlus/premiumPlusAdminStatusSummary.js` |
| 販売可否・段階公開 | `premiumPlusRelease.js`（**再実装しない**）|
| 停止フラグの読み | `premiumPlusMember.js` → `resolveUpsellForCustomer()` |
| クーポン期間の開始 | `premiumPlusReopenLaunch.js` / `premiumPlusReopenStartStore.js` |

検証: `npm run test:premium-plus-admin`（`check:safety` に組込）

## 0. 最優先の完成条件 — 細かい文章を読まなくても運営できる（2026-09-09 追加）

**眼精疲労を抑える。** 運営者が細かい文字を追わないと判断できない状態は**未完成**とする。

| 要件 | 具体 |
|---|---|
| 状態は**大きく 1 行** | 会員ごとに現在状態を 1 行で断定（例「購入可能（9/4 から）」）|
| **色**で判別 | 販売中 / 停止中 / 段階表示中 / 購入可能 を色分け |
| **短文** | 状態バッジは 10 文字前後。長文の説明を主表示にしない |
| **大きなボタン** | 次に可能な操作だけを大きく出す（小さなリンクにしない）|
| 上部に集約 | 残日数・何日目・購入可否は**上部だけ**で完結して読める |
| 詳細は**折りたたみ** | 従来の全項目は削除せず、下部の折りたたみへ移動 |
| 内部用語は主表示にしない | phase / override / eligibility を前面に出さない（折りたたみ内で参照可）|
| 数秒で完了 | 状態確認と操作が**長文を読まずに**終わる |
| 押した結果が即反映 | 操作後に状態表示が即座に更新される（誤操作防止）|

## 0-b. 一覧画面（2026-09-09 追加固定）

詳細パネルと**同じ思想**を一覧へ広げる。日常運用で見るのは
「**誰が今どの状態か**」「**次に何を押せるか**」だけ。

単一源: `astro-site/src/lib/premiumPlus/premiumPlusAdminListView.js`
...
| `paused` | **販売停止中** | その会員だけ止めている（最優先で判定）|
| `staged` | **段階表示中** | 資格はあるが、まだ買えない |
| `out` | **対象外** | 販売対象外 / 資格保留（**理由を一覧に短く添える**）|

- **「購入可能」は `purchaseEnabled`（サーバー解決値）だけで決める。**
  override / PHASE を見て分岐すると片方が漏れる（odamoto を取り逃した原因）
- **停止は最優先。** 停止中を「購入可能」「段階表示中」に混ぜない
- `即時販売` / `override` / `PHASE` は**主表示・主フィルタに出さない**
  （`即時販売` は override **操作の名前**であって状態ではない）

### 一覧の要件

| 要件 | 実装 |
|---|---|
| 上部に運営サマリー | 「購入可能 N 名 / 販売停止中 N 名 / 段階表示中 N 名 / 対象外 N 名」。押すと絞り込む |
| 状態は**大きな色バッジ** | 顧客セルの隣。緑 / 赤 / 黄 / 灰 |
| 「対象外」の理由 | バッジの下に**短く**添える（`販売対象外` / `資格保留`）。詳細を開かないと区別できない状態を作らない |
| 次の操作は**1 個** | 販売中→「販売を停止」/ 停止中→「販売を再開」。**結果が名前で分かる** |
| 押すとどうなるか | ボタン内に短く併記（長文は置かない）|
| 各行に「詳細・操作」 | 1 クリックで詳細パネル（§1 の構成）を開く |
| **メール完全一致 1 件** | その会員の詳細・操作パネルを**自動で開く**（一覧から再度探させない）|
| 危険な操作だけ確認 | 「販売を停止」だけ確認ダイアログ。再開では増やさない |
| 操作後 | 一覧へ戻さず、その場で状態・ボタン・日時を更新 |
| 警告 | **異常・矛盾のある会員だけ**。正常な会員に注意文を並べない |
| 要対応 | 「販売可なのに未案内」は数字で終わらせず、**対象会員を開けるボタン**を出す |
| 効果測定 | 表示→クリック→到達→決済開始は**既定で閉じた折りたたみ**（「効果測定を開く」）|
| 横スクロール | 日常運用で要求しない。一覧の列は 顧客 / 状態 / プラン / 販売許可日 / 最終更新 / 操作 の 6 列 |

一覧から外した列（Route / 区分 / 販売CTA / 表示判定 / 実閲覧 / 案内 / PHASE）は
**削除ではなく**、詳細パネルと「効果測定」で見る。

### 一覧の追加要件（2026-09-10 確定）

| 要件 | 具体 |
|---|---|
| 上部は**大きな 4 カード** | 購入可能 / 販売停止中 / 段階表示中 / 対象外。件数を大きく出し、押すとその状態だけに絞り込む |
| 意味不明な数字を出さない | 件数は**ラベルと別要素**で出す（文言に混ぜて「購入可能 4名 0」になっていた）。別指標なら必ず意味の分かるラベルを付け、不要なら表示しない |
| 会員行は**どこを押しても**詳細・操作を開く | メールアドレス・状態バッジ・行全体。キーボード（Enter / Space）でも開く。右端の「詳細・操作」ボタンは**残す** |
| 行内のボタンは行クリックとして扱わない | `button / a / input / select` を押したときは行のクリックにしない（誤操作防止）|
| 検索は**日常運用の主入口** | 折りたたまず常時展開。完全一致 1 件で詳細を自動で開く。部分一致は該当者だけ一覧表示 |
| 細かい条件は**「詳細条件を開く」** | Route / 区分 / 販売CTA / 実閲覧 / クーポン / 停止 / 並び は折りたたみ（既定は閉じる）。**削除はしない** |
| 通常運用の主表示 | **検索** と **状態** だけ |

### 文字・視認性（2026-09-10 確定）

**眼精疲労を増やさない。下記より小さくしないこと。**

| 対象 | 最低ライン |
|---|---|
| 管理画面の本文 | **16px** / 行間 1.75 |
| 一覧の表 | **1rem**（メールは 1.05rem・太字）|
| フィルタ・検索入力 | **1rem** |
| 4 カードの件数 | **2.1rem** 太字 |
| 一覧の状態バッジ | 0.95rem 太字 |
| 薄い文字 | `muted` を `#a9b6c9` まで明るくする |

長文の説明を主操作領域に置かない。詳細情報は**削除せず**折りたたみへ。

### 重要な管理操作は「ロジックテスト＋実 DOM E2E」の両方で守る（2026-09-10 確定）

**片方だけにしない。** 実際に起きた 2 件はどちらもロジックテストが全部通っていた。

| 層 | 守るもの | 単一源 |
|---|---|---|
| ロジックテスト | 判定・文言・組み合わせの矛盾 | `test:premium-plus-admin` |
| **実 DOM E2E** | **要素が DOM にあるか / 押せるか / 表示と実状態が一致するか** | `npm run e2e:admin-plus` |

E2E は **CI で必須実行**（`.github/workflows/safety-check.yml`）。
ubuntu ランナー同梱の Chrome を使うので**依存パッケージは追加しない**。
実行できない環境では**勝手に依存を足さず**、理由を出して失敗させる。

#### E2E が最低限確認すること（減らさないこと）

1. メール完全一致検索 → 対象会員の詳細が開く（部分一致は該当者だけ一覧）
2. 行クリック / メール / 状態バッジ / 「詳細・操作」の **4 経路**から詳細へ進める
3. 購入可能 / 販売停止中 / 段階表示中 / 対象外 が実状態と一致
4. 販売停止 → 表示と操作が「販売停止中 / 販売を再開」へ切り替わる
5. 販売再開 → 「購入可能 / 販売を停止」へ復帰し、購入可否も戻る
6. 今すぐ販売可 → **未来の購入可能日を出さない**
7. クーポン 14 日の開始で**販売可否が変わらない**（混同しない）
8. 上部 4 カードの件数が一覧と一致し、押すと正しく絞り込む
9. 要対応 N 名 → 対象会員だけ表示し、そこから 1 クリックで詳細へ
10. 操作後に**古い状態・古いボタンが残らない**（カードの件数も更新）
11. 対象外に**無意味な販売操作を出さない**
12. 見えるのに押せない要素が無い（押せないなら理由がある）
13. 全行に「詳細・操作」が **DOM にある**（生成しただけで append 漏れを検出）
14. 未認証では admin 画面 / 実績画像 API に到達できない

⚠️ **14 は dev サーバーが要る**（Edge の Basic 認証と Functions は `dist` の静的配信では動かない）。

- **ブラウザプロファイルを repo の中に作らないこと。** OS の一時ディレクトリ
...
- **疎通確認は `public/` の静的ファイル**（`robots.txt`）へ投げる。`/` は SSR が走り、
  CI では返ってこないことがある。
- **待ちには必ず時間制限を付ける。** `fetch` の 1 回ごとにも付ける
  （付けないとループが 1 周目から進まず、上限に到達しない）。WebSocket の open も CDP の各呼び出しも、
...
- admin の認証情報は渡さない（未設定なら fail closed で 401 になるのが正しい挙動で、それを検証する）。
- 未認証チェックの **2 観点（admin 画面 / 実績画像 API）は環境に関わらず必ず実行する**。
  黙ってスキップしない（PR 時 56 項目 / CI 54 項目という、観点が減るズレが実際に起きたため）。
- ただし**確認手段は環境で替える**。この repo の `astro dev` は edge function
  (`netlify/edge-functions/admin-auth.ts`) を必ずローカル実行しようとし、それには **Deno** が要る。
  Deno は初回に netlify のキャッシュへ落ちるためローカルでは動くが、
  **GitHub Actions の runner には Deno が無く**、edge-functions-dev の待ち時間（3 秒固定）に
  cold start が間に合わず **dev サーバーが 1 リクエストも返さない**（2026-09-10 に CI で 3 回再現）。
  「実行不能なら依存を勝手に追加しない」方針に従い、**Deno のセットアップは CI へ足さない**。
  - Deno が**ある**（ローカル）… `astro dev` を起動して **HTTP で 401 / 404** を確認する
  - Deno が**無い**（CI）… **本物の判定モジュール／本物のハンドラを直接呼んで**同じ観点を確認する
    （`/admin/*` への edge 認証の配線・env 未設定でも通さない fail closed・`handleMediaGet` が
    セッション無しで 404 を返すこと）
  - 起動しても無駄と分かっている環境では **dev サーバーを起動しない**（Deno の有無を先に見る）
- 実行サマリに「未認証チェックを HTTP で実施 / ハンドラ直呼びで実施」を必ず出す。

停止中に商品内容・実績画像を URL 直打ちで見せないことは、
サーバー側のガード（`pausedHidesContent.guard.test.mjs`）と
本番の read-only 確認で担保する（E2E では未認証到達不可までを見る）。

### なぜ E2E が要るか（実例）

**ユニットテストだけでは足りない。** 実際に起きた 2 件はどちらも
ユニットテストが全部通っている状態で本番へ出た。

| 起きたこと | ユニットで捕まらなかった理由 |
|---|---|
| 「今すぐ販売可」なのに「9/19 から購入可」と出る | 判定は正しく `purchaseEnabled: true`。**見出しの文言**が間違っていた |
| 全行から「詳細・操作」ボタンが消えていた | ボタンを**生成する行はある**が `appendChild` が抜けていた。ソース検査では通る |

→ `astro-site/scripts/e2e-admin-plus.mjs`（`npm run e2e:admin-plus`）で
**実ページを Chromium で開いて DOM を検証**する。

- 管理 API はブラウザ側の `fetch` 差し替えで合成データを返す。**本番へは触れない**
- 検証項目: 4 カードの有無と並び / バッジ / 行の操作名（販売を停止・販売を再開）/
  **「詳細・操作」ボタンが行にある** / 詳細の見出しに**未来日を出さない** /
  すでに買えるなら「今すぐ販売可」を出さない / 詳細の折りたたみが閉じている
- `/admin/*` は Edge の Basic 認証の背後。ローカルで開けないときは認証情報を与えるか、
  **検証中だけ** edge function を退避する（**必ず戻す**。戻したことを差分で確認する）

### 完成基準

**メールアドレスを入力 → 対象会員が出る → 現在状態が分かる → 必要なボタンを押す**
までを、細かい説明を読まず数秒で完了できること。
運営者が細かい文字を追わないと判断できない状態は**未完成**。

検証: `npm run test:premium-plus-admin`

## 1. 画面の構成（上から順に固定）

1. **現在の状態**… 大きな 1 行 + 色 + 短いバッジ
   （購入可否 / 販売停止 / 段階表示の現在地と残日数 / クーポン取得 / クーポン期間の開始）
2. **次に可能な操作**… その状態で意味のある操作**だけ**を大きなボタンで出す
3. **詳細情報**… 従来の全項目を**削除せず**、下部の**折りたたみ**（既定は閉じる）に入れる
4. 従来の操作セクション（通常操作 / 強い操作 / クーポン）も折りたたみ内

## 2. 段階表示は人間向けに書く

`PHASE 3` だけを見せて判断させない。会員ごとに現在地と残りを日本語で出す。

| 状態 | 表示 |
|---|---|
| 資格確定当日〜2日目 | 段階表示 N 日目 ・ あと N 日で予告開始 |
| 予告のみ | 予告のみ表示中 ・ あと N 日で商品ページ表示 |
| 商品ページ閲覧可 | 商品ページ閲覧可・購入はまだ不可 ・ あと N 日で購入可能 |
| 購入解禁 | 購入可能（YYYY-MM-DD から）|
| 即時販売 | 購入可能（待機日数を飛ばして販売中）|
| 停止中 | 販売停止中（購入も閲覧もできません）|

残日数は**表示のためだけ**に資格確定日から計算する。段階そのものの判定は
`premiumPlusRelease.js` の値をそのまま使い、画面や要約で作り直さない。

## 3. 販売状態のボタン（トグル 1 つ / **2026-09-10 に文言を修正**）

**ボタンには「押すと起こること」を書く。状態名をボタンにしない。**
いまの状態は上の要約（バッジ・大きな 1 行）が伝える。

| いまの状態 | 出すボタン | 押すと |
|---|---|---|
| 販売中 | **「販売を停止」** | その会員だけ販売停止 |
| 販売停止中 | **「販売を再開」** | その会員だけ販売再開 |

### ⚠️ 停止中は必ず再開できること（2026-09-10 の不具合修正）

旧実装は「未開始 + 停止中」で再開スイッチを隠していた
（当時は「クーポン期間の開始」が同時に再開する設計だったため）。
2026-09-09 に販売の切替をトグル 1 つへ集約した結果、
**止めた会員を戻す手段が画面から消えていた。**

- **停止中は、クーポン期間がどの状態（未開始 / 開始済み / 途中 / 確認不能）でも
  「販売を再開」を出す。**
- クーポン期間を確認できない（Redis 不通）ことを理由に**再開を塞がない**。
  販売の停止/再開は Airtable だけの操作で、クーポン期間の保存先とは無関係
- 停止と再開を同時に出さない（常にどちらか 1 つ）
- 停止 ⇄ 再開が往復できることを**テストで固定**する（UI とサーバー実効状態の両方）

- 2 つを並べない。**いまの状態を表すボタン 1 つ**で切り替える
- 書込 gate（`PREMIUM_PLUS_SALE_PAUSE_READY`）が無ければ押せない（**fail closed**）
- **他会員には影響しない**（Airtable の当該レコード 1 行だけを更新）

## 4. 販売停止の意味（**2026-09-09 で拡張**）

停止は「購入 CTA を止める」だけではない。**その会員に Premium Plus の
結果・商品内容を再開まで見せない**状態とする。

| 面 | 停止中の扱い |
|---|---|
| 商品ページ `/premium-plus-v2/` | 商品内容を出さない（受付休止の案内のみ）|
| 実績画像 `premium-plus-media`（GET）| **404**（会員本人でも取得させない）|
| 予告枠 `/api/premium-plus-stage.json` | 案内とクーポン導線のみ。**商品内容・実績は出さない** |
| クーポンページ `/premium-plus-coupon/` | **見せる**（停止中こそクーポンを配る相手）|
| お知らせ（ベル） | Premium Plus のお知らせを出さない |

- **UI だけでなくサーバー側の実効状態も同じ判定**にする。
  URL 直打ち・API 直叩きで迂回できないこと（各面が停止を読んで 404 にする）
- 判定は既存の単一源のみ（`resolveUpsellForCustomer()` の `salePaused`）。
  面ごとに独自の停止判定を書かない

### ⚠️ 旧決定の上書き

2026-08-22 に「停止中も予告枠は**通常と同じ見た目**で出す」と決めていた
（クーポン導線へ到達させるため）。2026-09-09 の指示により、
**結果・商品内容は停止中は出さない**へ変更する。
**クーポン導線は維持**する（停止中こそ配る相手なので、ここを閉じると当時の
「クーポンへ到達できない」問題が再発する）。

## 5. 意味のないボタンは出さない／押させない

| 操作 | 出さない・押させない条件 | 理由 |
|---|---|---|
| 今すぐ販売可 | **すでに購入できる** | 飛ばす待機日数が無い（結果が変わらない）|
| 今すぐ販売可 | 販売停止中 | 停止の解除が先 |
| 販売を停止 | **対象外（資格なし）で停止していない** | 商品ページ自体が出ないので結果が変わらない |
| 販売を再開 | — | **停止中は資格に関係なく必ず出す**（止めたまま戻せない状態を作らない）|
| クーポン期間の開始 | すでに開始済み | 期間は延長しない（first-write-wins）|
| 販売停止/再開 | 書込 gate 未設定 | fail closed |

判定は `describeActionConflicts()` / `describeNextActions()` の単一源。
画面側で条件を作らない。
...
| 旧 | 新 | なぜ |
|---|---|---|
| この会員の再募集を開始する | **クーポンの利用期間（14日）を開始する** | 「再募集」が販売再開と誤読される。実際に始まるのはクーポン期間 |

クーポンの話と販売状態の話を同じ欄・同じ語で混ぜない。

## 7. 完成条件（これが満たされていないものは未完成）

1. 対象会員を開くと、まず「今どういう状態か」が**大きな 1 行と色**で分かる
2. 次に押せる操作が 1〜数個の**大きなボタン**で明確
3. 押した結果がその場で状態表示へ反映される
3-b. **長文を読まずに**状態確認と操作が数秒で終わる（詳細は折りたたみの中）
4. 販売中 ⇄ 販売停止を admin から安全に切替できる
5. 停止中は Premium Plus の結果・商品内容が本人に見えない（**サーバー側も**）
6. 段階表示の現在地・残日数・購入可否が明確
7. **状態とボタンが矛盾する組み合わせが 1 件も無い**ことをテストで固定
8. 他会員へ影響しない / URL 直打ちで迂回できない / fail closed / rollback 手順あり
...
| 停止フラグ | admin のトグルを押し戻す（Airtable 1 行）|
| クーポン期間の開始 | `HDEL ak:pp:reopen:v1:members <recordId>`（既存手順）|

# 顧客向け画面に内部運用を出さない（2026-09-09 確定）

お客様の画面に出るのは**お客様に必要な情報だけ**。社内の事情・社内の言葉・社内ツールへの
導線を出さない。強制は guard テスト（CI）で行う。

単一源: `astro-site/src/lib/copy/customerFacingInternalWording.guard.test.mjs`
検証: `npm run test:copy`（`check:safety` に組込済み）

## きっかけ（2026-09-09 MK 報告）

> 内部情報が堂々とユーザーに公開されてしまってます

`/premium-plus-v2/` の注記が、社内の事情をそのまま書いていた。

| 出ていた文 | 何が問題か |
|---|---|
| 当時は不的中の日の控えを保存していなかったため | **運用の不備をこちらから明かしている** |
| （画像の「集計外」表記が該当分です）| 社内の集計用語。お客様には意味が伝わらない |
| 記録を開始した以降の実データのみで自動集計しています | 社内の処理手順の説明 |

横断監査で同種の露出が他にもあった（下表）。

## 禁止するもの（guard が検知する）

画面に出る文字だけを見る（コメント・style・script・frontmatter は対象外。`{...}` の中は
変数の値が出るので識別子名として数えない）。
...
| 内部処理の語 | 取込 |
| 内部の識別子 | `AI_SANRENPUKU_AXIS_V1` のような SCREAMING_SNAKE の直書き |
| インフラ・サービス名 | Airtable / SendGrid / Redis / localStorage |
| 運用不備の告白 | 保存していなかった / 暫定 / 仮実装 / 未対応 |

## 誤検知させない（消してはいけない言葉）

競馬の言葉と社内用語は別物。**禁止語に入れない**:
ゲート（発馬機）・フルゲート・ゲート練習 / 資金運用・運用 / 投票控え /
「不具合を報告したい」（お問い合わせの選択肢）

guard 自身にこの回帰テストを持たせている（禁止語が競馬用語を誤検知しないことを固定）。

## 直した箇所（2026-09-09）

| ページ | 直した内容 |
|---|---|
| `premium-plus-v2.astro` | 注記から運用不備の告白と社内用語を削除 |
| `archive-sanrenpuku-all/index.astro` | `AI_SANRENPUKU_AXIS_V1` の直書きを削除 |
| `archive/jra/[year]/[month]/index.astro` | 「内部ルールに基づき調整」→ 「当社の予想ロジックに基づいて構成」 |
| `premium-prediction/jra.astro` | **有料会員のエラー画面にあった管理画面ボタンを削除** |
| `archive/nankan/` 3 ページ | 「旧集計基準 / 現在基準集計」→ 「参考実績」 |
| `free/jra.astro` / `free/nankan.astro` | 「取込が終わり次第」→ 「準備が整い次第」 |

## 例外（allowlist・理由必須）

guard の `ALLOW` に理由付きで置く。**黙って足さないこと**（直すべきものを黙らせる場所ではない）。

| ページ | 例外 | 理由 |
|---|---|---|
| `about.astro` | Airtable / Netlify | 技術スタックの紹介として意図的に出している（MK 判断待ち）|
| `service-description.astro` | Netlify | 同上 |
| `pro-demo.astro` | 管理画面 | 商品デモとして管理画面の例を見せている（MK 判断待ち）|

管理用ページ（`/admin/` 配下・`admin-*.astro`）は顧客向けでないため監査対象外。

## 4 領域横断の確認

表示文言の修正なので CLAUDE.md の 4 領域ルールに従い、
`free-prediction/{jra,nankan}` / `premium-prediction/{jra,nankan}` を横断確認した。
管理画面リンクは **premium-prediction/jra.astro のみ**に存在し、他 3 領域には無かった。

## 仕組みの説明も出さない（2026-09-10 追加・認証まわりで確定）

> 今後、ユーザー向け画面・メールには、token消費、ブラウザ差、長押し、先読み、セッション等の
> 内部事情を原則出さず、「ユーザーが今すること／結果」だけを表示する方針として正本にも固定
> — MK

社内の事情（前段）だけでなく、**動作の仕組み**もお客様には書かない。
「なぜそうなるか」を説明するほど、読み手は**自分が何をすればいいか**が分からなくなる。

| 出さない（内部事情）| 出す（今すること／結果）|
|---|---|
| リンクを開いただけでは使われません（確認画面で押したときに使われます）| ログインするには、下のボタンを押してください。 |
| このブラウザへのログインが完了しました。次回から同じブラウザのブックマークから… | ログインしました。まもなくマイページへ移動します。 |
| メールアプリ内のブラウザで開くと、別のブラウザでは再度ログインが必要になる場合があります | （出さない）|
| トークンを確認しています / 無効なトークンです | そのままお待ちください。 / このリンクは使用できません。 |

**理由の説明はソースのコメントと docs に置く**（消さない。将来の担当者には必要）。

禁止語（ユーザーに出る文字列のみ・コメントは対象外）:
`トークン` `token` `セッション` `Cookie` `クッキー` `localStorage` `HttpOnly`
`長押し` `先読み` `プレビュー` `キャッシュ` `WKWebView`
`メールアプリ内のブラウザ` `別のブラウザでは再度ログイン` `同じブラウザ`

| 何で守るか | 対象 |
|---|---|
| `src/lib/auth/userFacingNoInternals.guard.test.mjs` | `/auth/verify` の表示文言 / `/login` の理由表示 / ログインメール本文 |
| `scripts/e2e-magic-link-verify.mjs`（実ブラウザ）| **実際に描画された文字**（確認 / 成功 / 失敗の 3 画面）|

新しい画面・メールを足すときも、**画面に出る文だけ**を同じ観点で見る。
`/auth/verify` の「押すまでリンクは使われません」のような**善意の補足**が典型的な違反例で、
書いた本人には親切に見えるが、読み手には内部の話でしかない。
...
| 5 | **購入が発生したとき**に段が正しく進む（前段が止まり、次段の育成の対象になる）ことを実運用で確認できる |
| 6 | 上記が**購入停止・配信停止・重複防止・帰属の契約を守ったまま**成立している |

⚠️ **実顧客が実際に購入すること自体は完成条件ではない**（2026-09-14 MK 確定）。
売上や成約件数は事業の成果であって、DRM の実装完成の判定基準にしない。
5 と帰属で見るのは **購入が起きたときに処理が正しいか**であり、
購入が起きるまで「未完成」として保留するものではない。

### 完成条件に**含めないもの**（非ブロッカー）

| 項目 | 扱い |
|---|---|
| **A/B（`variant`）の実運用** | **将来課題・非ブロッカー**（2026-09-14 MK 確定）。`DeliveryKey` は campaign × version × step × 受信者で variant を含まないため、送り分け・帰属・重複防止の設計判断が別途要る。**DRM 完成の残件に数えない** |
| 実顧客の購入件数・売上 | 事業の成果であって実装完成の判定基準にしない（上記） |
...
#### ファーストビューと無料登録の入口（2026-09-27 MK 確定 / プレビューは維持）

売上減少の調査で、2026-08-20 の変更によりこのページから**無料登録へ進む入口が消えていた**ことが分かった。
プレビューの位置づけは変えずに、ページ上部を次の順に並べる（スマホ優先）。

1. **結論・注目馬**: 各会場のメインレースの ◎（と ○▲△）。
2. **短い理由**: 公開事実（前走着順・近走の 3 着以内回数・騎手）だけから作る。
3. **無料登録 CTA**（未ログインにだけ出す）: 何が得られるか（`/free/` の詳しい表示・ポイント）を書き、
   `/free-signup/` へ進める。「登録すると買い目・全頭が見られる」と読める文言は書かない。
4. その下に既存のプレビューバナー（有料 CTA）・会場タブ・全レースの詳しい分析。

| 守ること | 単一源 / 検証 |
|---|---|
| 注目馬・理由は公開 DTO（`buildFreePublicRows()`）由来だけ。`pt` / AI総合指数 / 役割 / 特徴量 / 買い目を材料にも表示にも使わない | `src/lib/freePreviewFirstView.js` |
| 無料登録 CTA は**ページの中身を出し分けない**（ゲートではない）。未ログインかどうかで CTA の表示だけ切り替える | `src/components/FreePreviewFirstView.astro` |
| 有料 CTA（`locked-paid`）は各レース 1 枚のまま。無料登録 CTA は別クラスで上部に 1 つだけ | `freePreviewFirstView.guard.test.mjs` |
| GA4 はイベントを増やさない（`/free-signup/` 到達は `page_view`）| `docs/GA4_CONVERSION_FUNNEL.md` |
...
#### 休み明け / 叩き◯戦目 の定義（2026-09-01 改定）

単一源は `src/lib/freeViewpoints/memberExtras.js` の 2 定数。**ここ以外に閾値を持たない。**

| 定数 | 値 | 意味 |
|---|---|---|
| `LAYOFF_DAYS` | **60**（= 2ヶ月） | この日数以上空いたら「長い休み」 |
| `CAMPAIGN_MAX_GAP_DAYS` | **42**（= 中6週） | 「続けて使っている」と言える間隔の上限 |

判定（`calcLayoffRun`）:

1. **今日 ← 前走 が `LAYOFF_DAYS` 以上** → 🏖 **休み明け**（＝復帰初戦 / `nth: 1`）
2. それより前に長い休みがあり、**その休み以降の各間隔がすべて `CAMPAIGN_MAX_GAP_DAYS` 以内**
   → 🔁 **叩き◯戦目**（`nth` = 休み明けから数えた走目）
3. 長い休み以降に `CAMPAIGN_MAX_GAP_DAYS` を超える間隔が 1 本でもある → **`null`（チップを出さない）**
4. 持っている過去走（最大 5 走）の範囲に長い休みが無い / 日付欠け / 並び破綻 → **`null`**

**なぜ 3 が要るか（2026-09-01 の実例）**: 2026-09-02 大井 11R ③ティントレットは
1/14 →(91日)→ 4/15 →(70日)→ 6/24 →(70日)→ 9/2 というローテーション。
旧実装（`LAYOFF_DAYS = 84`・間隔を見ない）では**「叩き3戦目」**と表示されたが、
各走 70 日ずつ空いており「続けて使って仕上げている」実態ではない。
新実装では **今日の 70 日が 60 日以上なので「休み明け」**になる。
仮に閾値だけ下げて 3 を入れないと、「70 日おきに走る馬が叩き2戦目」という同型の誤表示が残る。

**`null` は「休みではない」ではなく「判定しない」。** 断定できないときは何も出さない（fail-closed）。
`calcInterval` の `休養明け（中◯週）` ラベルも同じ `LAYOFF_DAYS` を使うので、
2 つのチップの意味は常に一致する。

回帰テストでティントレットの実データ（`nankan/horseStats` 2026-09-02 OOI R11 の実日付）を固定している。
...
### クーポンを「取得できるか」と「いま購入できるか」は**別軸**（2026-08-22 整合修正）

> **Superseded**: 「クーポン取得 CTA は販売一時停止中の会員にだけ出す」という記述は**無効**。
> 再募集の開始が販売停止の解除を含む 1 操作になったため、停止を条件にすると
> **開始した瞬間に取得できなくなる**（実際に本番で発生した不整合）。
> **「販売再開前に先に取得させる」を通常運用にはしない。**

| 軸 | 何が決めるか | 単一源 |
|---|---|---|
| **クーポンを取得できるか** | Plus の対象会員 ＋ **いま購入できない（販売停止中）** ＋ 未取得 | `premiumPlusCouponAccess.js` |
| **クーポンを使えるか** | 取得済み ＋ **その会員の再募集が開始済みで期限内** | `premiumPlusCouponAccess.js` |
...
クーポンは**買えなかった人への埋め合わせ**。したがって**配る相手は「いま購入できない会員」**。

> ⚠️ **禁止**: 取得条件に「その会員の再募集が開始済み」を入れないこと。
> 再募集の開始＝販売再開なので、**「買える人だけが取得できる」＝目的と正反対**になる。
> 2026-08-22 に一度この実装を入れて本番で壊した。

#### 取得できる条件（すべて満たすこと）

1. **Plus の対象会員**（`plusAudience.isPlusAudience`＝**販売停止を外したら**商品ページを見られる人）
2. **いま販売を停止している**（`plusRelease.salePaused`）＝買えない
3. まだ取得していない
4. 取得の記録を本番へ保存できる（gate 有効）

#### 使える条件（割引が乗る）

1. 取得済み
2. **その会員の再募集が開始済み**（`reopenStartsAt` が確定）
3. **その会員の期限内**（`reopenStartsAt + 14 日`）

⚠️ **未開始のうちは「使えない」**（予約 write も申込への適用も fail closed）。
   未開始の会員に 58,000円 の申込を作らせないため。
⚠️ **既に取得済みのクーポンは保持する**（未開始・期限切れでも保有の事実は消えない）。

#### 通常フロー

1. 会員が Premium Plus を買おうとする → **販売停止中で買えない**
2. 受付休止ページ / クーポンページ / マイページで **クーポンを受け取る**
3. admin で対象会員の「▶ この会員の再募集を開始する」
   → その会員だけ販売が再開し、`reopenStartsAt` が確定して 14 日が始まる
4. 取得済みのクーポンが**使えるようになり**、**58,000円**で申し込める
5. 開始後に緊急停止しても **`reopenStartsAt` と 14 日の期限は変えない**
   （購入だけが止まる。停止中は取得もまたできる）

#### 取得できる場所（4 面）

`/dashboard/`（マイページ）／ `/premium-plus-coupon/`（クーポンページ）／
`/premium-plus/`（販売停止中の受付休止ページ）／
**三連複会員ページの導線を押したその場**（下記）。**判定はすべて同じ単一源**で、
URL 直打ち・API 直呼びでもサーバーが再検証する。

#### 販売停止中の導線（2026-08-22）— **入口を塞がない**

停止すると `resolveUpsellForCustomer` の channel が `none` になり、三連複会員ページの
導線（`PremiumPlusStageTeaser`）ごと消えていた。**買おうとすることすらできず**、
「代わりにクーポン」に到達できない。したがって:

| いつ | 何が起きるか |
|---|---|
| 押す前 | **販売中とまったく同じ見た目**（`plusAudience.resumed` の文言・ラベル） |
| 押した瞬間 | **遷移しない**。「お申し込みが殺到しており、ただいまご案内できません」＋クーポンのご案内 |

- `/api/premium-plus-stage.json` は停止中でも **200**（`paused: true` ＋ `pausedNotice`）
- **存在秘匿は不変**。Plus の対象外・非会員には**停止中でも 404**
- 文言・停止判定・取得可否は**すべてサーバー**が返す。導線の script は `is:inline` で
  未ログイン者の HTML にも載るため、**コメントを含め**商品名・状態・条件を書かない
- `plusAudience.resumed` は**表示にだけ**使う。購入可否は従来どおり `release`

#### 管理画面との整合

管理画面の「いま受け取れるか」は**顧客側と同じ `resolveCouponAccess`** で解く。
`adminCouponConsistency.smoke.test.mjs` が**同一会員**に対して管理 Function の判定と
本物の取得 API の実挙動を**両方動かして一致を検査**する。
...
## 反応なし除外は**取り込みコホート**で運用する（2026-08-26 確定）

### 確定した仕様

**累計 10 通以上 delivered で開封が一度も無い相手は、その後のマーケティング配信から自動除外する。**
まず CSV 取り込み由来の約 15,000 名に確実に効かせ、**もとからの Airtable 顧客とは分けて数える**。

| 項目 | 確定内容 |
|---|---|
| 除外の条件 | delivered 10 通以上 かつ 開封 0（`engagementPolicy.js` の `inactiveDelivered`）|
| 数える期間 | 反応の集計が**記録を始めた時刻以降**だけ（記録していない期間を無反応と見なさない）|
| コホート | `Source` が `customer-import:` で始まれば取り込み由来。判断材料が無ければ**既存顧客**扱い |
| 影響しないもの | **購入・契約・重要通知メール**（決済確認 / 認証 / サポート / 期限通知）|
| 再判定 | **送信直前にも**外す（キュー登録後に閾値へ達した相手を送らない）|
| 表示 | 除外理由と人数を管理画面で確認できる（コホート別の内訳つき）|

### 開封 0 判定の正本と Apple MPP

- 開封は **「反応あり」へ倒すためだけ**に使う。Apple MPP や画像ブロックで落ちるため、
  **開封が無いことを理由に切るのは delivered が閾値に達してから**
- **購入・ログインは開封より強いシグナル**。開封 0 でも外さない
- click は provider 側で計測していないため**当てにしない**
- 二重計上は起きない。配信の事実は `DeliveryKey`（campaign × version × step × 受信者）で
  一意になり、開封はアドレス単位で 1 回に畳まれる

### 材料が欠けたら 1 人も除外しない（fail closed の向き）

「反応が無い」と「観測できていない」を取り違えると、開封している人を切ってしまう。
次のどれか 1 つでも欠ければ **誰も除外しない**: 緊急停止 / 集計を読めない /
開封計測が有効でない / 開封の記録が 1 件も無い / 受信が長く途絶えている /
集計の開始時刻が不明。

### 送信直前の再判定（なぜ一覧を経由するのか）

「累計 10 通」の判定には配信台帳の全履歴が要るが、**実送信の Function では読み切れない**
（`CampaignDeliveries` は実測 14,000 行超）。そこで

1. **計算できる場所**（下見・enqueue）で判定し、結果を Redis の一覧へ書く
2. **送る場所**（dispatcher）がその一覧を読み、送信直前にもう一度外す

判定そのものは `engagementGuard.js` が単一源で、一覧は結果の受け渡しだけを担う
（`engagementBlocklistStore.js`）。**古い / 空 / 読めない 一覧は使わない**（誰も除外しない）。
`applied:false` のときは**書かない**（観測できていない状態の結果を残さない）。

送信直前に再判定するのは、購入済み・配信停止・バウンス・退会・**この反応なし除外**の 5 つ。

### 適用するのは**取り込みコホートだけ**（2026-08-26 確定）

**自動除外は `Source` が `customer-import:` の会員だけに適用する。**
もとからの Airtable 顧客には適用しない。取り込み分は購入も接点も無いところから始まるため
「開封 0」がそのまま無反応を意味するが、既存顧客は購入・問い合わせなどメール外の接点を持つ。
**同じ数字でも意味が違う**ので、自動では切らない。

- 判断材料が無い会員は `existing` 扱い（＝除外しない側へ倒す）
- 既存顧客まで広げるときは `suppressionCohorts` を**明示的に**渡す（既定では広がらない）

### 完全自動運用（MK の毎日の操作を要求しない・2026-08-26 確定）

| 項目 | 確定内容 |
|---|---|
| 起点 | **Step1 の明示送信**（母集団が最大になるので初回だけ人が撃つ）|
| Step2 以降 | `delayDays` に従って**自動**。人の指示・承認・日付更新は要らない |
| 日付 ARM | **不要**。`MARKETING_SEQUENCE_ARMED` は未設定なら常時武装。
日付を入れればその日だけ動く（従来運用も残す）|
| cron | **10 分ごと**（従来は 1 日 1 回）|
| 1 tick の上限 | **500 名**（`MARKETING_SEQUENCE_MAX_PER_TICK` で下げられる）|
| 同日完走 | 500 × 6 回/時 = **3,000 通/時**。15,000 名でも 5 時間で配り切る |
| 止める手段 | `MARKETING_SEQUENCE_SCHEDULER_ENABLED` / `MARKETING_CAMPAIGN_DISPATCH_ENABLED`
を外す（即停止）。キャンペーン期間外は `getCampaign()` が null で送れない |

⚠️ **1 日 1 回・200 通の実装では 15,000 名に 75 日かかり、実質動かなかった。**
「200 名/日で何十日も掛かる実装は不可」（MK）。

### 連続配信の実績を管理画面で見る（2026-08-26 確定）

キャンペーン × ステップごとに、**配信台帳の実データ**から次を出す。

| 項目 | 意味 |
|---|---|
| 対象数 | その step で扱った全員（queue済み + 実送信 + 除外 + 失敗）|
| queue済み | **まだ届いていない**。キュー登録しただけ |
| 実送信数 | `Status='sent'` の行だけ |
| 除外数 | `skipped-*`（配信停止 / バウンス / 退会 / 反応なし など）|
| 失敗数 | `Status='failed'` |
| 未送信残 | queued のまま送られていない数。**0 が正常** |
| 二重送信数 | 同じ `DeliveryKey` が 2 行以上。**0 が正常** |
| 最終実行時刻 | 送信 / 登録 / 失敗 / 除外のうち最も新しい時刻 |

⚠️ **queue済みを「送信済み」として表示しない。** 混ぜると「送ったつもりで届いていない」を
見逃す（2026-08-25 に実際 488 通が queued のまま止まっていた）。

集計は **cron が台帳を走査するついでに作る**（追加の読み取りをしない）。
1 周読み切ったところで「確定」にし、それまでは「途中経過」と明示する
（0 と未集計を混同させない）。管理画面と管理 API は**読むだけ**で、何も送らない。

単一源: `marketing/sequenceMetrics.js`。検証: `sequenceMetrics.test.mjs`。

### 台帳は tick をまたいで走査する（2026-08-26 に本番で発見）

1 通目を 15,491 通送ったことで配信台帳が **4,000 行の読み取り上限**を超え、
連続配信の tick が例外で止まり **2 通目が 1 通も送れない**状態になっていた。

⚠️ **「先頭 N ページだけ読む」で打ち切るのは誤り。** Airtable のページ順は安定しているので
毎回同じ人しか見えず、後ろの人が永久に進まない。

**前回どこまで読んだか（`offset`）を保存し、次の tick が続きから読む。**
最後まで読んだら次の周回を先頭から始める。

- 1 tick が読むのは決まったページ数だけ（既定 20 ページ = 2,000 行。`MARKETING_SEQUENCE_SCAN_PAGES` で変更可）
- 周回を重ねれば**全員が必ず対象になる**
- 走査が重複しても**送信は重複しない**（`DeliveryKey` が保証）
- Redis が無ければ毎回先頭から読む（進まないだけで誤送信にはならない）
- 読み残しは `partial` で返す（黙って打ち切らない）

単一源: `marketing/sequenceLedgerScan.js`。
検証: `sequenceLedgerScan.test.mjs`（16,000 名で全員ちょうど 1 通・取りこぼし 0・重複 0）。

### 15,000 名規模の予約配信を完走させる

連続配信の 1 tick 上限（既定 200 名）を超えたとき、以前は**中止**していたため
15,000 名規模では **1 人も送れなかった**。いまは **上限まで送り、残りを次の tick へ持ち越す**。

- 誰に送ったかは配信台帳が持つので、持ち越しても取りこぼさない
- 同じ相手へ二度送らない（送信済みは次回 due から自動的に外れる）
- 残り人数（`carriedOver`）と総数（`dueTotal`）を返すので進捗を追える
- **黙って切り捨てない**

単一源: `marketing/importCohort.js` / `marketing/engagementBlocklistStore.js` /
`marketing/engagementPolicy.js` / `marketing/engagementGuard.js` /
...
- `/free-prediction/` に無料登録による全頭解放 CTA / ゲートを復活させること（2026-08-20 撤廃済み。CTA だけ・ゲートだけの復活も不可）。※ 2026-09-27 に確定した**無料会員特典（`/free/` の詳しい表示・ポイント）への登録案内**はこれに当たらない（ページの中身を登録で出し分けないため）
