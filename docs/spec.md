...
## 三連複 買い切りの閲覧権は**恒久**（2026-08-25 確定）

### 確定した仕様

**三連複を買い切りで購入した会員は、その閲覧権を恒久的に保持する。**
Premium（馬単）の期限が切れたあとも**三連複ページだけ**は閲覧できる。

| 項目 | 確定内容 |
|---|---|
| 期限切れ後に開くもの | **三連複ページのみ** |
| 期限切れ後に開かないもの | **馬単（Premium）/ Light / その他 Premium 権限**（復活させない）|
| 今後の新規購入 | 従来どおり恒久（入金確認が `LifetimeSanrenpuku` を立て、`有効期限` を触らない）|
| 権利の表し方 | **既存の正規権限 `LifetimeSanrenpuku`（Customers のチェックボックス）だけ** |
| 判定式 | **緩めない**。旧プラン名（`Premium Sanrenpuku` / `Premium Combo`）だけで
自動的に買い切り扱いにはしない |

### なぜ判定式を緩めないのか

旧プラン名だけの会員は、買い切りだったのか期間契約だったのかを**データから判別できない**
（本番実測: 有効期限が 2025-12〜2026-05 に散らばっており、期間契約の終了に見える）。
判定式を緩めると、**期間契約が終わっただけの会員にも恒久的な権利を配る**ことになり、
以後どの会員が買い切りなのかをコードからもデータからも復元できなくなる。

権利を与えるべき会員が居る場合の正しい直し方は、**その会員に `LifetimeSanrenpuku` を立てること**。
「誰に与えたか」がレコードに残り、取り消しも 1 フィールドで済む。

### 旧三連複会員（期限切れ）への付与（2026-08-25 MK 決定）

顧客維持・既存顧客優遇の方針として、**購入履歴の追加調査を復旧の条件にしない**。
対象者には `LifetimeSanrenpuku=true` を付与する（＝上の「正しい直し方」を全員に適用する）。

- これは**判定式の変更ではなく、レコードへの権利付与**である
- 付与後も契約は期限切れのまま。**馬単・Light は復活しない**
- 本番レコードの変更は高リスク境界。対象・件数・変更フィールド・影響・rollback・
  検証結果を提示して**明示承認を得てから**実行する

### 成立に必要な 3 層（1 つ欠けるとページが開かない）

| 層 | 期限切れ + 買い切りのとき | 正本 |
|---|---|---|
| ログイン判定 | `premium-sanrenpuku` の**有料セッションを発行する** | `src/lib/auth/memberResolution.js` |
| 権利判定 | 三連複=可 / 馬単=不可 / Light=不可 | `src/lib/entitlements/resolveEntitlements.js` |
| ページの入口 | 三連複ページのみ通す | `src/lib/auth/paidPageGate.js` |

⚠️ **ログイン判定が `free` に落ちるとセッション自体が発行されない**
（`sessionPayload.js` が free を拒否する）ため、権利判定が正しくてもページは開かない。
この 3 層のつながりは `src/lib/auth/sanrenpukuLifetimeAccess.test.mjs` が固定する。

### 退会・強制ログアウトは買い切りより優先

`WithdrawalRequested` / `ForceLogout` / 停止 Status は、買い切りを持っていても拒否する（安全側）。
