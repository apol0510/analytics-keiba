...
| 3 | **admin の付与 / 取消 / 誤取得訂正 / 再発行** | MK 要否判断 → 実装（**未着手・下記 3 参照**）|
...
⚠️ **付与 / 取消 / 誤取得訂正 / 再発行の 4 操作は未完了のまま。**
これらが揃うまで「**管理運用完成**」としない（2026-08-19 MK 指示）。
利用状態が読めるようになっただけでは、運営者は Airtable の直接編集から解放されていない。

#### 予約取消・誤取得訂正を実装するときの前提（実装前に読む）

- 予約の取消は**予約行だけ**を `revoked` にする。Customers 側の「クーポン取得済み」は消さない
  （純粋関数 `buildReservationRevokeFields()` は実装済み・**admin から呼ぶ経路が無い**）。
- 誤取得の訂正は Customers 側の 3 列を空へ戻す操作。これを入れると
  「取得していない会員に予約行が存在する」状態が作れてしまうため、
  **`describeCouponLifecycle` の `claimed=false` を「予約なし」と読み替える最適化を入れないこと**
  （現在も入れていない。台帳を読めないときは会員の取得有無に関わらず `unknown`）。
...
1. **MK に 2 点を確認する: ①実際の再募集開始日時（`reopenStartsAt`）
   ②admin の付与 / 取消 / 誤取得訂正 / 再発行の要否。**
   ①が決まれば有効期限が確定し、予約 write と `confirm-bank-payment` 配線へ進める。
   割引条件（10,000円OFF / 68,000→58,000）、期限ルール（開始日 + 14 日）、
   利用ライフサイクル、redeem 部分成功対策は**すべて確定・実装済み**。
2. **②が「要る」なら 3 / 3-B のとおり付与・取消・誤取得訂正・再発行を実装する。**
