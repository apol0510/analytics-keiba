...
現在の正本 `src/lib/premiumPlus/premiumPlusReopenCoupon.js` の値:
...
| `offerPrice` | **null** |
| `expiresAt` | **null** |
| `terms.determined` | **false** |

`promotions/promotionOfferCatalog.js`（価格の正本）に **Premium Plus 向け offer は 0 件**。
つまり**実質的な割引内容が未定**で、顧客に出しているのは
「優待の具体的な内容は、募集再開のご案内時に改めてお知らせいたします。」という文だけ。
**この状態は完成ではない。**

⚠️ **MK の承認なしに金額・割引率・有効期限を決めない。**
...
**PC / mobile 両方。** 2026-08-19 時点で受付休止ページ・クーポンページ・admin は
合成データでローカル表示できる状態まで用意したが、**MK はまだ目視承認していない**。
デザインの統一（BaseLayout 不使用・ヘッダー / フッター / 戻り導線が無い）も未判断。

### 6. 再募集時の実利用フロー（**offer 発行以降は未接続**）

| 工程 | 状態 |
|---|---|
| Premium Plus 用 `purchase_offer` をカタログに定義 | **未実装**（0 件）|
...
1. **現行 `/free-prediction/` は変更しない。** 有料版の内容・価値を見せる
   プレビュー／サンプルとして維持する。
...
- **新しい未検証の予想ロジックを勝手に作らない**（既存の印・指数・評価・特徴量から構成する）
- 新規ページの **URL・名称・具体的な公開項目を勝手に確定しない**
...
| | 状態 |
|---|---|
| 仕様の確定（上記 4 点）| ✅ 2026-08-19 MK 決定 |
| `docs/spec.md` / `decisions.md` / `progress.md` への固定 | ✅ 本コミット |
| 現行コード・既存データの調査（提供できる情報の棚卸し）| ⏳ 実施中 |
| 3 条件（無料でも役立つ／全レースを見る理由になる／有料版を食わない）での候補比較 | ⏳ 未報告 |
| URL・名称・公開項目の決定 | ❌ **未確定（MK 判断待ち）** |
| ページ本体の実装 | ❌ **未着手** |
| 計測（新規登録・有料転換への寄与）の設計 | ❌ 未着手 |

## 未完了任務

1. **調査**: 既存の予想 JSON・アーカイブ・特徴量・過去走データから、
   全レースに対して提供可能な情報を棚卸しする。
2. **比較報告**: 「無料でも役立つ」「全レースを見る理由になる」「有料版を食わない」の
   3 条件で候補を比較して MK へ報告する。
3. **MK 決定**: URL・名称・公開項目を確定する。
4. **実装**: 新規ページ。買い目を返す経路を配線として遮断する（公開DTO 経由を強制）。
5. **導線**: nav / 無料ページ / メール からの入口をどこに置くか（`/free-prediction/` の
   プレビュー価値を削らない置き方）。
6. **検証**: `npm run check:safety`、および無料へ買い目・pt・指数が漏れないことの guard。

## ▶ 次作業（次回セッションはここから）

1. 上記 1（調査）→ 2（3 条件での比較報告）を先に終える。**実装には入らない。**
2. MK が公開項目を確定してから 4（実装）へ進む。
3. 実装時は「新データを作らない」原則（`/results-showcase/` と同じく既存 JSON の
   再構成に留める）を守る。
