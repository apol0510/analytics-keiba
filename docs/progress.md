...
### 2. クーポンの具体的価値 — **割引条件は確定済み / 有効期限だけ未確定**
...
`expiresAt = null` / `expiresDetermined = false` のまま。
顧客画面には「**有効期限は未定です（募集再開のご案内時にお知らせいたします）**」と出す。
⚠️ **「30日間有効」等を勝手に補完しない。** 期限は MK が決める。
...
| 表示項目 | クーポン名 / 割引額 / 通常価格 / 適用後価格 / 有効期限（**現在は未確定表示**）|
...
#### ❓ 未決定（MK 判断待ち）: 「使用済み」にするタイミング

⚠️ **選択しただけ・申込フォームを開いただけで「使用済み」にしてはいけない。**
既存の申込・入金フローに照らすと候補は 3 つ:
...
##### ⚠️ 未完了: confirm-bank-payment への配線

上記ロジックは**まだ `confirm-bank-payment` に接続していない**（本番 write なし）。
接続には ①再募集開始日時の確定（＝期限が確定して予約が作れる）
②接続後の本番検証手順の合意 が要る。
...
1. **MK に 4 点を確認する: ①有効期限 ②（確定済み）利用ライフサイクル
   ③**再募集の開始日時**（決まれば期限が確定し、予約 write を有効化できる）
   ④admin の付与 / 取消の要否。**
   どれも実装を止めている決定事項。
