...
## 2026-08-27 — 【調査・計画】CSV 取り込み分を prospect プールへ戻す
...
| 実績の可視化 | ✅ 本番稼働（Light / Premium は確定・無料区分は走査中）|
| prospect 移行 | 🔵 **調査と計画のみ。本番データの削除は 1 件も実行していない** |
...
1. **打ち切り回数の確定** — `prospectPolicy.js` の **3（送信回数）** と
   MK 指示の **10（delivered 数）** が食い違っている。**意味が違うので揃え方の判断が要る**
2. **反応（開封）の一覧を計画へ当てる** — 開封記録は Redis にあり手元から読めない。
   当てずに実行すると**開封した人まで prospect へ落とす**
3. **移行の実行** — prospect 投入（Redis）→ 検証 → Customers 削除（Airtable）

### ▶ 次作業

1. 上の 1 と 2 を確定する（MK 判断 + 管理 API 側で開封を突合）
2. 下見をやり直し `engagementApplied: true` にする
3. prospect へ投入（1 回 2,000 件上限）→ 完全性を検証
4. **Customers 削除の直前で停止し、改めて承認を取る**

### 完成条件（これを満たすまでクローズしない）

1. 打ち切り回数が確定し `prospectPolicy` に反映されている
2. 反応の一覧を当てた計画で `engagementApplied: true` / 母数 = 判定の合計
3. prospect プールへ投入され、件数・状態・送信回数の引き継ぎが検証されている
4. Customers から対象が削除され、**残件数が想定と一致**している
5. 削除後も**配信が止まっていない**（2 通目・3 通目が届く / 実績が出る）
6. 巻き戻し手順が実データで確認されている
...
| 境界 | 状態 |
|---|---|
| 本番 Redis への書き込み（prospect 投入）| **未実行** |
| **Customers の削除** | **未実行** |
| production env の変更 | **未実行** |
| 実送信 | **未実行**（2 通目は自動配信が 8/31 に行う）|
