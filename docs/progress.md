...
### なぜ下限 25 秒なのか（**根拠なく下げていない**）

下限を `F` にすると、新しい campaign を始められる最も遅い時点は `予算 − F`。
そこから**最も重い campaign** が走って終わる時刻が打ち切り以内でなければならない:

```
(TICK_TIME_BUDGET_MS − MIN_NEXT_CAMPAIGN_ESTIMATE_MS) + MAX_CAMPAIGN_MS <= TICK_HARD_LIMIT_MS
(55 − 25) + 23 = 53 秒 <= 60 秒   ✅ 余裕 7 秒
```

- **20 秒**でも 1 tick の本数は同じだが理論最悪が **58 秒**（余裕 2 秒）まで詰まる
- **30 秒**にすると余裕は増えるが**本数が減る**（free の実行 10 → 8 回 / 14 tick）
- → **本数が変わらないなら余裕の大きい 25 秒**を採用した

不変条件は `sequenceTickBudget.test.mjs` が式ごと固定している。
**どの campaign も `MAX_CAMPAIGN_MS`（30 秒）を超えてはいけない**（超えたら式を見直す）。
...
| | 現行（固定 50 秒）| **修正（実測ベース）** |
|---|---|---|
| `campaign-discount-free` の実行回数 | 4 回 | **10 回**（約 2.5 倍）|
| 1 tick あたりの本数 | ほぼ 1 | **2〜4** |
| 実際の最長終了 | 42 秒 | **49 秒**（予算 55 秒以内）|
| 理論最悪終了 | — | **53 秒**（打ち切り 60 秒以内）|
