...
- 🛑 **期限つき文面の成立判定で不成立**（2026-09-18）。10 通のうち **01 / 02 / 03 / 10** に
  「2026年9月23日まで」が入っており、**10 通目が最後に出る**ため
  **最遅開始日は start-1 = 09-14 / start-2 = 09-15 / start-3 = 09-16**（いずれも既に過去）。
  09-19 開始なら 10 通目は 09-26〜09-28 で **3〜5 日超過**。04〜09 は日付なしで常に成立。
  → **期間を取り直す（A）／文面を差し替える（B）／期限つき 4 通を外す（C）** のいずれかを決めるまで
  **予約も作成もしない**（生成スクリプトが `--apply` でも 1 通も作らない）
