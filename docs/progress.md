...
### ① 第 2 期の入口判定を 2 段引きに（毎 tick 約 8 秒 → 約 4.5 秒）

> ### ⚠️ 訂正（2026-09-17 / deploy 後の実測）
>
> 当初この節は「**約 19 秒 → 約 6 秒**」と書いていたが、**過大評価だった**。
> Redis 1 往復を約 100ms と見積もって 178 往復 = 17.8 秒としたが、
> **実際の Upstash REST は約 45ms** で、入口判定の実コストは **約 8 秒**だった。
>
> deploy 後の実測（下見・2 回平均）:
>
> | campaign | 最適化前 | **最適化後** |
> |---|---|---|
> | `campaign-prospect-phase2` | 約 20.4 秒 | **約 16.7 秒** |
> | `campaign-discount-free`（比較用・入口判定なし）| 約 13.0 秒 | 約 13.7 秒 |
>
> 差分（＝入口判定のコスト）は **約 8 秒 → 約 3 秒**。
> **削減は約 3.7 秒**であって、13 秒ではない。
> 往復回数の削減（178 → 60 回・**66%減**）は設計どおりだが、
> **1 往復あたりの実コストを見誤った**。
...
### ③ 再ベンチマーク（契約は 30 秒 / 打ち切り 60 秒 / 余裕 5 秒を維持）

| 人数 | 往復 | 逐次(旧) | **並行(新)** | rate 下限 | 30 秒契約 | 60 秒打ち切り |
|---|---|---|---|---|---|---|
| 50 |  9 |  23 秒 | **19 秒** | 15 秒 | ✅ | ✅ |
| 75 | 14 |  28 秒 | **23 秒** | 16 秒 | ✅ | ✅ |
| **100** | 17 | 32 秒 | **24 秒** | 16 秒 | ✅ **余裕 5.6 秒** | ✅ |
| 200 | 34 |  50 秒 | **36 秒** | 20 秒 | ❌ | ✅ |
| 500 | 85 | 107 秒 | **70 秒** | 30 秒 | ❌ | ❌ |

**安全に使える最大同期人数を 75 → 100 に更新**（`SYNC_TICK_MAX_RECIPIENTS`）。
計算上は 140 まで収まるが**余裕が 1 秒を切る**ため、
`HARD_LIMIT_SAFETY_MARGIN_MS` と同じ考え方で**余裕 5 秒以上**を残す 100 を採る。

⚠️ **500 は並行化しても 70 秒**で打ち切りを超える。同期経路では到達できない。
...
|---|---|---|---|
| 現状（最適化前・N=50）| **4 / 7 tick（本番実測）** | **181 通/時（実測）** | 約 **2.6 日** |
| 最適化後・N=50（env 変更なし）| **実測待ち** | **実測待ち** | — |
| 最適化後・N=100（env 変更が要る・**未実施**）| — | — | — |

> ⚠️ **訂正（2026-09-17）**: 「最適化後は 5/7 tick = 214 通/時」は**見積りであって実測ではない**。
> 見積りに使った campaign 所要時間が実測とズレていた（上記①）ため、**実測に置き換えるまで空欄**にする。
> deploy 後の実測は「④-2 deploy 後の実測」に記載する。

### ④-2 deploy 後の実測（2026-09-17 / #566 反映後・read-only）

**`#566` は本番で正常に動作している。** live のみ通る upsert 経路も含めて確認した。

| 時刻 | step2 | 差 | その間の tick | free の実行 |
|---|---|---|---|---|
| 07:59Z | 6,138 | — | — | — |
| 08:14Z | 6,135 | −3（索引縮小と一致）| 08:00 / 08:10 | ❌ ❌ |
| 08:26Z | **6,184** | **+49** | 08:20 | ✅ |

⚠️ **下見（`sequenceTickPreview`）は窓で切られている**（`limit 2000` / `scanPages 2`）が、
live の tick は**索引と台帳を最後まで読む**。したがって
**下見の所要時間から live を見積もると必ず短く出る**。
この誤りで「最適化後 5/7 tick」と書いてしまった。**実測に置き換える。**

#### 安全条件（全サンプルで維持）

| 項目 | 実測 |
|---|---|
| `MARKETING_SEQUENCE_MAX_PER_TICK` | **50 のまま**（引き上げていない）|
| 1 run の対象人数 | **50**（`送る=50`・超過なし）|
| prospect 以外の混入 | **0**（`{prospect:50, Customers:0, 出所不明:0}`）|
| 第 2 期入口の 2 段引き | 正常（`entry {open:true, entered:0}` / `rows 0`）|
| duplicate | **0**（索引数 = delivered 分類合計）|
| claim 取り残し | **なし**（`byStopReason {}` / delivered が送信に追随）|
| queue 滞留 | **なし**（`jobs` 0〜1 で毎回捌けている）|
| partial failure / retry / rollback | **発生なし**（`delivery_rows_unconfirmed` 0 件）|
| suppression / unsubscribe / bounce | 正常（sink `recording: ok` / `reasons: []`）|
...
|---|---|---|
| `campaign-prospect-phase2` | 約 20.4 秒 | **約 16.7 秒** |
| `campaign-discount-free` | 約 13.0 秒 | 約 13.4 秒 |

⚠️ **これは窓で切った下見の値**。live の所要はこれより長い。**比較にだけ使う。**

#### 🔻 throughput が下がった — 原因は #566 ではなく **#565 の安全カット**

7 tick（08:00–09:00）の実測:

| tick | 08:00 | 08:10 | 08:20 | 08:30 | 08:40 | 08:50 | 09:00 |
|---|---|---|---|---|---|---|---|
| free の rotation 位置 | 5 | 4 | **3** | **2** | **1** | 7 | 6 |
| 実行 | ❌ | ❌ | ✅ +49 | ✅ +49 | ✅ +50 | ❌ | ❌ |

**3 runs / 7 tick ＝ 148 通 / 70 分 ＝ 約 127 通/時**（deploy 前は 4/7 ＝ 約 181 通/時）。

位置と実行を突き合わせると原因が分かる:

| free の位置 | 1 | 2 | 3 | 4 | 5 | 6 | 7 |
|---|---|---|---|---|---|---|---|
| deploy 前 | ✅ | ✅ | ✅ | ✅ | ❌ | ❌ | ❌ |
| **deploy 後** | ✅ | ✅ | ✅ | **❌** | ❌ | ❌ | ❌ |

**1 tick で到達できる campaign 数が 4 → 3 に減った。**
これは #566（軽量化）ではなく、**#565 で入れた `LATEST_START_MS = 25 秒`**
（新規開始の絶対カット）の効果。`60 − 30 − 5 = 25` と**契約値から**導いたもので、
先行 campaign の合計が 25 秒を超えると以降は始まらない。
live の所要は窓付き下見より長いため、このカットが効いて free が押し出されている。

⚠️ **安全強化が throughput を犠牲にした。** これは設計どおりの動作であって不具合ではないが、
**代償を正しく記録しておく**。

#### ⚠️ 第 2 期の calendar に対して余裕がほぼ無い

第 2 期を 27 日で配り切るのに必要な速度は **約 128 通/時**（11,826 人 × 7 通 ÷ 27 日）。
実測 **約 127 通/時**は**ほぼ境界**で、余裕が無い。

#### 2 サイクル目でも同じ（再現した）

| tick | 09:10 | 09:20 | **09:30** |
|---|---|---|---|
| free の位置 | 5 | 4 | **3** |
| 実行 | ❌ | ❌ | **✅ +50** |

**位置 4 で止まり、位置 3 から動く**という境界が 2 サイクルで再現した。
偶然ではなく `LATEST_START_MS` の効果と判断してよい。

#### 選択肢（いずれも未実施・要判断）

| # | 手段 | 効果 | 安全への影響 |
|---|---|---|---|
| A | `campaign-prospect-phase2` をさらに軽くする（現在 約 16.7 秒）| 到達 campaign 数が増える | **弱めない** |
| B | `MAX_CAMPAIGN_MS` の契約を実測に合わせて見直す（30 秒 → 25 秒等）| `LATEST_START_MS` が伸びる | **契約を緩める**（要慎重）|
| C | `MARKETING_SEQUENCE_MAX_PER_TICK` を 100 へ | 3 runs × 100 ＝ 約 257 通/時 | #566 で 100 は 24 秒と確認済み |

⚠️ **C は本番 env 変更**。#566 時点の想定（214 通/時）は実測（127 通/時）と違ったので、
「env を上げる必要は無い」という判断の前提は**変わっている**。
