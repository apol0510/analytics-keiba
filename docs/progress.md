...
> → **2026-09-20 19:00 JST から 27 通の Single Send が毎日配信中**（予約は #585 の記録どおり）。
> **最新の現在地は直下の「📡 選別配信の現在地（2026-09-26 実測）」節**を正本とする。
> 下の完成条件表は AK 自作経路での到達状況の記録として残す。

## 📡 選別配信の現在地（2026-09-26 09:06Z 実測 / 本番 read-only・書き込み 0）

**配信は予定どおり進行中。変更は加えない**（再送・list 変更・env 変更・週次有効化はしていない）。

### 配信実績（SendGrid 実測）

| 日（19:00 JST）| start-1 | start-2 | start-3 |
|---|---|---|---|
| 09-20 | m01 req 318 / dlv 25 | m02 2,586 / 2,561 | m03 8,777 / 8,774 |
| 09-21 | m02 144 / 16 | m03 2,497 / 2,486 | m04 8,608 / 8,607 |
| 09-22 | m03 21 / 14 | m04 2,461 / 2,459 | m05 8,534 / 8,534 |
| 09-23 | m04 14 / 14 | m05 2,444 / 2,444 | m06 8,464 / 8,464 |
| 09-24 | m05 14 / 14 | m06 2,427 / 2,426 | m07 8,425 / 8,425 |
| 09-25 | m06 13 / 13 | m07 2,411 / 2,411 | m08 8,402 / 8,400 |
| 残り | m07〜m10（09-26〜09-29）| m08〜m10（〜09-28）| m09〜m10（〜09-27）|

- 送信済み 18 通 / 予約 9 通 / 重複送信 0 / 旧 AK 経路は `engine=sendgrid` のまま
- list 人数: start-1 **12** / start-2 **2,408** / start-3 **8,361**（合計 10,781。開始時 11,698）
- AK: 送信候補 10,782 / 反応済み 762 / 抑止 427 / 継続 list `ak-drm-engaged` **774**
- 週次配信: 無効（`SENDGRID_WEEKLY_ENABLED` 未設定。**今回も有効化しない**）

### ① 毎日の自動点検 — 毎日動いているが、毎日「注意」メールが出ている（誤検知）

- 最終起動 = 最終実行 = **2026-09-25 11:20:15Z**（途中で落ちていない）/ 不整合 **9**（安定）
- ただし **最後に知らせた = 同時刻**＝ 09-25 の回は「【注意】」メールを出している（実測）。
  下の原因により**コード上は毎回発火する**（09-21 以降の各回の通知有無は記録が上書きされ未確認）。
- 原因（コード読解）: `RECIPIENT_GAP` が**過去すべての通**の requests を
  「**前日の list 人数**」と比べている（`selectionWatch.js`）。list は反応・bounce で毎日減るので、
  過去の通ほど差が開く。例: start-1 m01 requests 318 ÷ 前日 list 13 前後 → 5% を大きく超える。
  **同じ list への初日の通が残るかぎり、毎日必ず発火する。**
- 影響: 異常が無くても毎日届くので、**本物の異常メールが埋もれる**。
  不整合 9 = |AK 送信候補 10,782 − provider rejected 10 − list 10,781| で、開始以来ほぼ一定。
- 対処案（**未実施**・コード修正）: 比べる相手を「その list の**最新の 1 通**」だけにする。

### ② 無反応 10 通の除外 — **9/27 ではなく今夜 9/26 の配信から始まる**

打ち切りは「累計 **delivered 10** かつ開封・クリック 0」（`engagementPolicy.inactiveDelivered=10`、
env 上書きなし）。**累計には移行前の配信も入る**（prospect 移行時に `Status='sent'` の行数を
`delivered` として引き継いだため）。送信候補 10,782 名の delivered 分布（全窓走査 11 窓・missing 0）:

| delivered（09-26 配信前）| 人数 | 打ち切りに届く日（開封が無い場合）|
|---:|---:|---|
| 9 | **6,690** | **09-26 19:00 の通が届いた時点**（start-3 m09）|
| 8 | 3,249 | 09-27（start-3 m10）または start-2 は 09-27（m09）|
| 7 | 769 | 09-28（start-2 m10）|
| 6 | 61 | start-1（12 名）は 09-29（m10）で 10。残り約 49 名は start-2 で 1 回届かなかった人で **最後まで 9** |
| 5 | 1 | 届かない（最後まで 9）|
| 1 | 12 | **list に居ない人**と推定（provider rejected 10 ＋ 2。AK 10,782 − list 10,781 − rejected 10 = 不整合 9 と整合）。SendGrid からは送られない |

- 分布の内訳は「予想 +1」の山が 2 つある形（start-3 = 8 と 9、start-2 = 7 と 8）で、
  +1 は**移行前に 1 通届いていた人**。仕様どおり（累計 delivered で数える）。
- **打ち切りの判定そのもの（AK の EXHAUSTED 化）は webhook の `recordDelivered` で起き、
  list の状態とは独立に確定する。** AK 側の結果は SendGrid の list が残っていても正しい。

#### ⚠️ リスク: 打ち切った人を list から外す処理に **1 回 100 件の上限**がある

- `sendgridSelectionExit.js` の `MAX_EXIT_PER_CALL = 100`。超えた分は `上限超過` として数えるだけで
  **外さない**。しかも EXHAUSTED は「外し損ねても再送させない」側（`CRITICAL_EXIT_STATES` は
  ENGAGED / PROMOTED だけ）。
- **既に本番で起きた証拠**: start-1 は初日に bounce 90 ＋ bounce_drops 211 で約 300 名が
  SUPPRESSED になったが、翌日の m02 も requests **144**（bounce_drops 122）。
  1 日で外れたのは約 170 名で、数日かけて外れた（m03 で drops 7、m04 で 0）。
  SUPPRESSED は SendGrid 側の suppression で**実害は無い**が、
  **EXHAUSTED には provider 側の suppression が無い**ので、外れ残った人には**次の通が実際に届く**。
- 今夜は約 6,700 名が一度に打ち切りになる。webhook 1 回に 100 件を超えて届けば、
  外れ残った人は **9/27 の start-3 m10（最終通）を受け取る**。start-2 も 9/27〜28 で同様。
- 影響の上限: 打ち切り後に最大 1 通（start-3）〜2 通（start-2）余分に届く。
  **AK の判定（EXHAUSTED・抑止台帳）は変わらず、以後の配信母集団（`ak-drm-engaged`）にも入らない。**
- **今回は触らない**（list 変更禁止の指示）。確認方法は下の「9/27 の確認」。

### ③ start-1（318 名）の大量未達 — 原因は**もともと届かないアドレスの集まり**
...
| delivered | 25 |
| bounces | 90 |
| **bounce_drops**（SendGrid の bounce 抑止リストに既に載っていて送られなかった）| **211** |
| invalid_emails | 1 |

- start-1 は「次に送る番号 = 1」＝ **AK 経路で step1 が一度も届かなかった人**の集まり。
  AK 時代に `dueByStep 1: 328` が何週間も減らなかったのは、送っても届かない宛先だったため。
- bounce / drop は webhook で SUPPRESSED → list から除外 → 318 → **12** 名まで縮んだ。
  **仕組みは正しく動いている**（外れるまで数日かかったのは上の 100 件上限による）。
- 残る 12 名は m04〜m06 で bounce 1 以外は delivered。m10 は 09-29。

### 9/29 に 3 グループの結果を確定する手順（read-only）

| いつ | 何を見る | 期待 |
|---|---|---|
| 09-27 朝 | list 人数（start-3）| 今夜の打ち切りが外れていれば **約 1,700** まで減る。**8,000 台のままなら 100 件上限で外れ残り** |
| 09-27 朝 | `prospectSequenceCheck` 全窓の delivered 分布 | 9 の山（6,690）が消え、AK 抑止が約 6,700 増える |
| 09-30 朝（最終 09-29 19:00 ＋ webhook 遅延）| 同上 ＋ `mailOverview` | 送信候補に残るのは delivered 9 以下（1 回届かなかった組 約 50 名 ＋ list 外 12 名 ＋ 今後の未達分）だけ |

- 使うもの: `admin-marketing` の `mailOverview` と `prospectSequenceCheck`（どちらも read-only）。
  `netlify dev:exec --context production` で管理 secret を注入して叩ける（Upstash は secret 指定で
  ローカルに取り出せないため直接は読めない）。
- ⚠️ `sendgrid-migration-audit.mjs --via-admin` は第 1 段（全窓走査）は通るが、
  **後段の `prospectIndexAudit` で `admin_api_unavailable` になり出力が空**（2026-09-26 実測）。
  delivered 分布だけなら上の 2 action で足りる。
- ⚠️ 抑止の内訳（打ち切り EXHAUSTED ／ 配信停止・bounce SUPPRESSED）を**分けて返す read-only
  経路は無い**。今日の抑止 427 は delivered 最大 9 の時点なので**全員 SUPPRESSED**
  （打ち切りは 0）。確定時は「抑止の増分 − 残り通の bounce・配信停止・苦情」で打ち切り数を出す
  （近似）。正確に分けるには read-only の内訳を足すコード変更が要る（**未実施**）。

### 最後まで確定しない人（判断が要る）

- delivered が 10 に届かないまま全通が終わる人（1 回届かなかった start-2 組 約 50 名 ＋
  list 外 12 名 ＋ 今後の未達分）は、**EXHAUSTED にも ENGAGED にもならず送信候補に残る**。
  扱い（追加で送る／このまま据え置く／打ち切り扱いにする）は **MK 判断**。
...
| 3 | `delivered` を数える（打ち切りの分母） | ✅ **本番実証済み**（webhook で prospect レコードへ加算）。2026-09-16 の分布は `1:398 / 2:6,199 / 3:5,209 / 4:30`（**max 4**）|
| 4 | `open` / `click` を蓄積する | ⚠️ open は稼働中（`withOpens` 現在 0）。**click は `MARKETING_CLICK_TRACKING_ENABLED` 未設定で 0 のまま**（正本どおり当てにしない）|
| 5 | 反応ありを保持する（ENGAGED は打ち切らない・Customers へ昇格できる）| ✅ 実装済み ＋ **本番で ENGAGED 122 名を観測**（2026-09-16。09-15 の 6 名から増えており、反応の取り込みは生きている）|
...
## 🚚 マーケティング基盤の全面整理 — AK = 頭脳 / SendGrid = 配送（2026-09-18 MK 確定 / **本番切替 実施済み・2026-09-20 から配信中**）
...
- Single Send 27 通は **draft・予約 0・送信 0**（＝**実送信はまだ 0**）
  → 2026-09-19 に予約、**2026-09-20 から配信中**（最新は先頭の「📡 選別配信の現在地」節）
...
**送信は成功。ただし指定 50 名に対して 150 名へ配信された。**

| 確認項目 | 実測 |
|---|---|
| 実送信 | **150 通**（3 ジョブ × 50）|
| provider accepted | **150**（202 受理分だけを `sent` に数える）|
| SentCount | 25,612 → **25,762（+150）** |
| duplicate send | **0**（3 ジョブの宛先は重なり 0 / ユニーク 150）|
| failed | **0** |
| bounce / suppression | EmailBlacklist **402 のまま**（増加 0）|
| 配信行 | sent 15,537 → **15,687** / cancelled 3,854 → 3,704 / queued 1 |
| 無関係な campaign | 送信は `campaign-discount-free` step2 のみ。他は不変 |
...
上限は**1 起動あたり**に効くので 50×3 = 150。`cron-marketing-rollout` にはある
tick 鍵が `cron-campaign-sequence` に無かった。**→ tick 鍵を追加（PR 作成済み）**。

⚠️ 二重送信は起きていない。積む前に配信行を名指しで突き合わせるガードが効き、
3 回の起動が別々の 50 名を選んだため。

### 原因 ②：prospect が 1 人も含まれていない

150 名は**全員 Customers 由来**（配信行 `cancelled` がちょうど −150）。
`selectNextDueStep` は出所を見ないため、母数の並び順で偏った。
**#521 の prospect 送信経路は実配信で 1 度も通っていない。**
→ 出所で絞る仕組みと、送る前に確かめる下見を追加（PR 作成済み）。

### 誤読しかけた点：「150 通送ったのに step 分布が動かない」
...
> ⚠️ **2026-09-17 時点の状態へ更新**（下表は #548 を作った時点の記録だった）。

| 項目 | 状態 |
|---|---|
| PR merge | ✅ **完了**（#548 = `c587c025` が main に入っている）|
| production deploy | ✅ **完了**（第 2 期は catalog に載り、共有 cron の対象に入っている）|
| production env 変更 | **不要**（下記）|
| 第 2 期の実顧客送信 | **未実施**（1 通も送っていない）|
| 第 2 期が自動選択されたか | ⚠️ **選択されるが入口が開かない**。第 1 期 step3 が 0 通のため完了者 0 名。原因と修正は下の「🛑 進行が止まっていた」|
...
## 🛑 進行が止まっていた — step2 が 0 人で tick を使い切っていた（2026-09-16 実測 / 2026-09-17 修正）

> # ⚠️ この節の診断は**誤りだった**（2026-09-17 訂正）
>
> **「配信は 1 通も進まない状態で止まっていた」は成り立たない。**
> deploy 後に**全窓走査で測り直した**結果、`campaign-discount-free` の step2 は
> ずっと配信され続けていた（**約 170 通/時**）。
>
> | 時刻 | step1 | step2 | step3 |
> |---|---|---|---|
> | ~03:05Z（#562 deploy 前）| 11,508 | 5,465 | **0** |
> | 03:56Z（deploy 後）| 11,504 | **5,611**（+146）| **0** |
> | 04:01Z | 11,504 | 5,611 | **0** |
>
> 2026-09-15 の delivered 分布 `1:469 / 2:11,305 / 3:165 / 4:30` が 2026-09-17 に
> `1:398 / 2:6,199 / 3:5,209 / 4:30` へ動いていることからも、step2 が数日かけて
> 大量に届いているのが分かる。
>
> **step3 が 0 の本当の理由**: `selectNextDueStep` は**いちばん小さい due step** を返す。
> `dueByStep` は `{1: 328, 2: 5,893, 3: 5,611}` で **step2 がまだ 5,893 名残っている**ため、
> step3 はその後ろで順番待ちしているだけ。**不具合ではない。**
>
> **なぜ誤診したか**: deploy 前の下見が `step=2 / 候補 0 / 除外 36` を 3 回連続で返したのを
> 「step2 が永久に空振りする」と読んだが、これは**下見の窓の性質**だった。
> 下見は索引の先頭から見るので**窓 0（先頭 2,000 件）**を読み、そこは step3 待ちが濃い
> （`{1:48, 2:5, 3:1947}`）。live の tick は**自分のカーソル位置**を見ており、
> そこには step2 待ちが大量にいる。**両者は別の部分集合を見て別の step を選んでいた**だけ。
>
> ⚠️ **窓 0 だけを見て全体を判断しない。** 進行は**全窓走査して合算**する。
>
> **#562 の位置づけ**: 「0 人だった step を外して選び直す」は候補が 1 人も残らない
> 縁のケースのガードとしては**正しく無害**（テスト 16 件で固定）だが、
> **step3 = 0 の原因ではなかった**。以下の記述と #562 の commit メッセージは**誇張**。
>
> 以下は 2026-09-16 時点の記録として残す（歴史記録・**現況ではない**）。

**「継続観測へ」ではなく、配信は 1 通も進まない状態で止まっていた。**

### 本番実測（read-only・書き込み 0・送信 0）

prospect 索引 **11,836 件を全窓走査**（`missing 0` / `digest` 固定）:

| 項目 | 実測 |
|---|---|
| 状態 | 送信候補 **11,836** / 反応済み未登録（ENGAGED）**122** / 永久除外 **18** |
| `delivered` | **max 4** / `{1: 398, 2: 6,199, 3: 5,209, 4: 30}` / `withOpens` **0** |
| 第 1 期 `sentByStep` | `{1: 11,508, 2: 5,465, **3: 0**}` |
| 第 1 期 `dueByStep` | `{1: 328, 2: 6,043, **3: 5,465**}` |
| 第 2 期 `campaign-prospect-phase2` | 配信台帳 **0 行**（**1 通も送っていない**）|
| 送信待ちジョブ | **0 件** |

**第 1 期 step3 は 1 通も出ていない。** 5,465 名が step3 の期限を過ぎているのに 0 通。

### 原因 ①：0 人だった step で tick が終わる（**本体**）

`selectNextDueStep` は**いちばん小さい due step** だけを返す。その step の候補が
後段の安全条件（既に `queued` / `sent`・出所フィルタ・許可リスト）で**全部落ちる**と、
`targets.length === 0` で `no_due_recipients` を返して**tick が終わる**。
次の tick も**まったく同じ step を選ぶ**ので、後ろの step は永久に進まない。

live と同じ条件（絞り込みなし）の下見を 3 回連続で実行した結果は毎回同じ:

```
step=2 / この tick の候補=0 / 送る=0 / 登録済みのため除外=36
```

step2 に残った **36 件が `queued` のまま `sent` にならない**（対応するジョブは 0 件）ので、
step2 は**永久に due のまま**。その裏で step3 の 5,465 名が 1 通も受け取れない。

⚠️ 出所を `prospect` に絞ると同じ下見が **step3 / 送る 50** を返す。
   **step3 に送る相手は居る。選ばれないだけだった。**

**直し方**: その step が 0 人なら、**その step だけを外して次の due step を選び直す**
（`planSequenceTick({ skipSteps })`）。step 数ぶんで打ち切る。

- **安全条件は 1 つも迂回しない**（既登録の除外 → 出所フィルタ → 許可リスト を同じ順で通す）
- **1 tick の上限は据え置き**（送る人数は増えない。送る step が変わるだけ）
- **許可リスト / `expectedCount` が渡されたときは選び直さない**
  （canary と DRM 入口の「この step でこの人数」の約束を変えないため）
- 下見も同じ選び直しをする（**下見と live がズレたら下見の意味が無い**）
- `skipSteps` を渡さなければ**挙動は 1 ミリも変わらない**

### 原因 ②：第 2 期の入口は下見で**一度も評価されていなかった**

後段接続の組み立ては `if (priorDecl && !isDry)` の内側にあり、**下見では丸ごと飛ばしていた**。
そのため `priorEntryEmails` は下見で必ず空になり、`autoStartGate.open` は常に `false`。
下見は「入口は閉じている」としか答えられず、**live なら開くのかを送る前に確かめられなかった**。

組み立てに使うのは読み取りだけ（`filterDelivered` = `SMISMEMBER` / `planPhase2Entry` = 純粋関数）
なので、**下見でも評価する**ように変えた。予約・queue・配信行・送信は従来どおり
**下見では行わない**（下見の return は予約より手前のまま）。

guard も「**評価しないこと**」から「**書き込まないこと**」へ移した
（`prospectPhase2Entry.test.mjs`）。

### 第 2 期が始まらなかったのは ① の結果

第 2 期の入口は「**第 1 期 3 step を配り終えた人が居ること**」がそのままゲート。
① で step3 が 1 通も出ていないので**第 1 期の完了者は 0 名**、したがって入口は開かず、
`campaign-prospect-phase2` は**1 行も作られなかった**。
→ delivered 累計は **max 4 のまま 10 に到達できず、EXHAUSTED も起こらない**。

**第 2 期の定義・後段接続の判定そのものに不具合は見つかっていない**（テスト 38 件 green）。
止めていたのは ① の 1 点。

### テストで固定した（`sequenceStepAdvance.test.mjs` 16 件）

既定は最小 due step のまま／step1 は自動で撃たない／step1 しか居なければ従来どおり
`first_step_is_manual`／**step2 が 0 人なら step3 が選ばれる**／外した step の人は混ざらない／
使い切ったら `no_due_recipients`（`first_step_manual` と言わない）／ゲートが閉じていれば何も選ばない／
上限と持ち越しは不変／選び直しは 0 人のときだけ／許可リスト・`expectedCount` では選び直さない／
打ち切りがある（無限ループにならない）／安全条件の順序は不変／除外数を持ち越さない／
下見も同じ選び直しをする。

### まだ実配信では確かめていない（**本番未反映**）

この修正は**まだ production に出していない**。反映後に read-only で確かめること:

| 見るもの | 期待 |
|---|---|
| 第 1 期 `sentByStep{3}` | 0 から増える |
| 第 1 期の完了者 | 0 名から増える |
| 第 2 期 `autoStart.open` | 完了者が出たら `true` になる |
| 第 2 期の配信台帳 | 0 行から増える |
| `delivered` max | 4 から増える |
| EXHAUSTED | delivered 10・無反応で増える |

⚠️ **人工的に進めない**方針は維持（期限の書き換え・手動 enqueue・canary・Redis 補正はしない）。

## ⏱️ 反映後の実測 — 1 tick で 1 campaign しか進まない（2026-09-17 / read-only）

#562（`a7383d60`）を production へ反映し、**下見で修正が効いていることを確認**した。

| | 反映前 | **反映後** |
|---|---|---|
| live 同条件の下見（`campaign-discount-free`）| `step=2 / 候補 0 / 送る 0 / 除外 36` | **`step=3 / 候補 100 / 送る 50 / 除外 0`** |
| 第 2 期の入口（下見）| 評価されず（常に `open: false`）| **評価される**（`open: true` / `entered: 0`）|

ゲートは 4 つとも開いている（`ARMED` は未設定＝常時武装で正本どおり）。

### ⚠️ 訂正 — 「step3 が 0 なのは停滞のせい」は**誤りだった**（2026-09-17）

当初ここに「tick が step2 を 0 人で空振りし続けるので step3 が永久に出ない」と書いたが、
**全窓走査で測り直した結果この診断は成り立たない**。
...
**step2 はずっと配信され続けていた**（実測 約 170 通/時）。
2026-09-15 の delivered 分布 `1:469 / 2:11,305 / 3:165 / 4:30` が
2026-09-17 に `1:398 / 2:6,199 / 3:5,209 / 4:30` へ動いていることからも、
step2 が数日かけて大量に届いているのが分かる。**配信は止まっていない。**

### step3 が 0 の本当の理由 — **step2 が先に捌かれる**

`selectNextDueStep` は**いちばん小さい due step** を返す。
`dueByStep` は `{1: 328, 2: 5,893, 3: 5,611}` で、**step2 がまだ 5,893 名残っている**。
したがって step3 は step2 の後ろで順番待ちしているだけで、**不具合ではない**。

### なぜ誤診したか（下見の窓の読み違え）

deploy 前の下見は `step=2 / 候補 0 / 除外 36` を 3 回連続で返した。これを
「step2 が永久に空振りする」と読んだが、実際は**下見の窓の性質**だった。

- 下見は索引の先頭から見る。**窓 0（先頭 2,000 件）は step3 待ちが濃い**（`{1:48, 2:5, 3:1947}`）
- live の tick は**自分のカーソル位置**を見ており、そこには step2 待ちが大量にいる
- 両者は**別の部分集合を見て別の step を選んでいた**だけ

⚠️ **窓 0 だけを見て全体を判断しない。** 進行を測るときは**全窓走査して合算**する。

### #562 の位置づけ（訂正）

#562 の「0 人だった step を外して選び直す」は、**候補が 1 人も残らない縁のケースの
ガードとしては正しく無害**（テスト 16 件で固定）だが、
**今回の step3 = 0 の原因ではなかった**。commit メッセージと当初の記述は誇張だった。

### 影響（完成条件への到達時間）

**実測は 約 170 通/時**（step2 が 51 分で +146）。
「70 分に 1 回 = 約 43 名/時」は **1 tick 1 campaign を前提にした最悪値**で、
実際は軽い campaign（2〜3 秒）が先頭に来た tick では 2 本目が始まるため、
`campaign-discount-free` の番はもっと頻繁に回ってくる。

| 区間 | 実測 約 170 通/時 での目安 |
|---|---|
| 第 1 期 step2 の残り 5,893 名 | 約 35 時間 |
| 続いて step3 の 5,611 名 | さらに 約 33 時間 |
| 第 1 期 3 通の完了 → 第 2 期の入口が開く | その後 |
| 第 2 期 7 通 → delivered 累計 10 → EXHAUSTED | さらに後 |

⚠️ **遅さの原因は #562 でも「停滞」でもない。** 1 tick の上限 50 名と、
   step を 1 つずつ捌く（最小 due step）設計の組み合わせ。

### 未判断（別タスク・**本 PR では触っていない**）

`MIN_MS_FOR_NEXT_CAMPAIGN`（50 秒）が `TICK_TIME_BUDGET_MS`（55 秒）に近すぎるため、
#545 が意図した「1 tick で複数 campaign に番を回す」がほぼ成立していない。
直すなら候補は次のいずれかだが、**送信量が増える変更なので独立して判断する**:

- 予算と最小値の差を広げる（例: 予算 55 秒のまま最小を 15〜20 秒へ）
- tick の間隔を詰める
- campaign ごとの所要を下げる（第 2 期の入口が毎 tick 約 35,500 鍵を引いている）

## 🔧 1 tick 1 campaign を直す（2026-09-17 / 実測ベースの時間予算）

上の「1 tick で 1 campaign しか進まない」を恒久修正した。**送る人数は増やしていない。**

### 何が間違っていたか

`MIN_MS_FOR_NEXT_CAMPAIGN = 50 秒` の根拠はコード上のコメント
「**実測 1 campaign ≒ 45 秒**」だったが、2026-09-17 の実測は次のとおりで**桁が違う**。

| campaign | 所要（下見）|
|---|---|
| `sanrenpuku-upsell-sequence` | 2 秒 |
| `light-to-premium-sequence` | 3 秒 |
| `free-signup-onboarding` | 7 秒 |
| `campaign-discount-premium` | 8 秒 |
| `campaign-discount-light` | 11 秒 |
| `campaign-discount-free` | 13 秒 |
| `campaign-prospect-phase2` | **21 秒** |
| **合計** | **65 秒**（予算 55 秒には全部入らない）|

予算 55 秒に対して下限が 50 秒なので、**1 本目が 5 秒を超えた時点で 2 本目以降は必ず止まる**。

### 直し方 — 固定値をやめ、**同じ tick の実測**から見積もる

`estimateNextCampaignMs()`（純粋関数）を新設し、
**その tick で実際に掛かった時間の最大 × 安全率 1.5**（下限 25 秒）を次の 1 本の見積りとする。
`observedMs` を渡さなければ**従来どおり固定 50 秒**（既存の呼び出しは挙動不変）。

### 安全余裕の決め方（**契約値だけで計算する**）

⚠️ **実測値と契約値を混同しない。** 実測の最遅は 23 秒だが、
**契約（`MAX_CAMPAIGN_MS`）は 30 秒**。安全余裕は**契約の 30 秒**で計算する。
実測だけを根拠に安全上限を狭めると、次に重い campaign が増えた瞬間に前提が崩れる。

安全は**見積りとは独立の絶対条件**が持つ:

```
LATEST_START_MS = TICK_HARD_LIMIT_MS − MAX_CAMPAIGN_MS − HARD_LIMIT_SAFETY_MARGIN_MS
                =        60          −        30       −             5
                = 25 秒
```

つまり **経過 25 秒を過ぎたら新しい campaign を始めない**。
最も遅く始めた campaign が契約上の最大 30 秒まで掛かっても:

```
LATEST_START_MS + MAX_CAMPAIGN_MS = 25 + 30 = 55 秒
打ち切り 60 秒 − 55 秒 = 余裕 5 秒   ✅
```

| 値 | 意味 |
|---|---|
| `TICK_HARD_LIMIT_MS` = **60 秒** | 打ち切りの実測（60,000 / 60,340 ms）|
| `HARD_LIMIT_SAFETY_MARGIN_MS` = **5 秒** | 必ず残す余裕（**ぎりぎりを安全と呼ばない**）|
| `MAX_CAMPAIGN_MS` = **30 秒** | **契約**（実測 23 秒ではない）|
| `LATEST_START_MS` = **25 秒** | 上の式から導出。**新規開始の絶対カット** |
| `TICK_TIME_BUDGET_MS` = **55 秒** | 予算 |
| `MIN_NEXT_CAMPAIGN_ESTIMATE_MS` = **25 秒** | 見積りの下限（**安全の根拠ではない**）|

### 判定は 3 条件すべて

1. **契約上の絶対条件** … `elapsed <= LATEST_START_MS`（見積りと独立）
2. **予算内に終わる見込み** … `elapsed + 見積り <= TICK_TIME_BUDGET_MS`
3. **打ち切りまで余裕を残す見込み** … `elapsed + 見積り <= TICK_HARD_LIMIT_MS − HARD_LIMIT_SAFETY_MARGIN_MS`

⚠️ 3 を `<= TICK_HARD_LIMIT_MS` にすると**余裕 0 秒を許してしまう**。
これらの式は `sequenceTickBudget.test.mjs` が**そのまま機械固定**している。

### ⚠️ 以前ここに書いていた式は誤りだった（2026-09-17 訂正）

当初は `(55 − 25) + 23 = 53 秒 <= 60 秒`（余裕 7 秒）と書いていたが、
**23 は実測値**であり、テスト契約は `MAX_CAMPAIGN_MS = 30 秒`を許容していた。
契約値で計算すると `(55 − 25) + 30 = 60 秒` で**余裕は 0 秒**。
**実測と契約を混同した誤りだった**ため、上記の契約ベースの式へ差し替えた。

### なぜ打ち切りを恐れるか

予約（`claimDelivered`）を取ったあと、キュー登録の前で打ち切られると、
その鍵は配信済み集合に残り **その人へは二度と送られない**（送信漏れ）。
だから「だいたい足りる」ではなく**実測の 1.5 倍**を要求する。

### 効果（実測所要でのシミュレーション / 14 tick）

| | 現行（固定 50 秒）| **修正（実測ベース＋契約カット）** |
|---|---|---|
| `campaign-discount-free` の実行回数 | 2 回 | **4 回**（2 倍）|
| 1 tick あたりの本数 | ほぼ 1 | **1〜4** |
| 実際の最長終了 | 23 秒 | **45 秒**（予算 55 秒以内）|
| 実際の最遅開始 | — | **23 秒**（カット 25 秒以内）|
| **契約上の最悪終了** | — | **55 秒**（打ち切りまで**余裕 5 秒**）|
| 飢える campaign | あり | **なし**（全 7 本に番が回る）|

### 触っていないもの

1 tick の送信上限（`MAX_RECIPIENTS_PER_TICK`）／campaign ごとの安全条件
（購入停止・配信停止・バウンス・`DeliveryKey` の重複防止・出所フィルタ・許可リスト）／
rotation の決め方（時刻ベースの round-robin）／送信経路。
**変わるのは「次の 1 本を始めてよいか」の判定だけ。**

### 次の改善余地（**本 PR では触らない**）

`campaign-prospect-phase2` の 21 秒は、**入口の判定で毎 tick 約 35,500 鍵**を
`filterDelivered` に掛けているため。第 1 期の完了者が 0 名でも毎回全件引いている。
ここを軽くできれば 1 tick の本数はさらに増えるが、入口の判定そのものに触るので別途。

## 📐 1 tick の人数を増やせるか — 構造解析（2026-09-17 / read-only）

**結論: 正本の設計値 500 は「同期 tick」では物理的に入らない。env だけ上げてはいけない。**

### キュー登録は逐次で、往復回数が人数に比例する

tick が作るのは `ScheduledEmails` の PENDING 行と `CampaignDeliveries` の queued 行だけで、
**provider（SendGrid）は呼ばない**（送信は `cron-marketing-dispatch`）。
したがって **tick 側に provider の rate limit / timeout リスクは無い**。

一方、書き込みは**すべて逐次**（`await` をループ内で回す）:

| 処理 | 分割 | 500 名なら |
|---|---|---|
| ジョブ作成（`ScheduledEmails`）| `RECIPIENTS_PER_JOB` = **50** | 10 回 |
| 配信行 upsert（`CampaignDeliveries`）| **10 件**ずつ | 50 回 |
| 書き込み後の**読み戻し検証** | 20 件ずつ | 25 回 |
| **逐次往復 合計** | | **85 回**（50 名なら 9 回）|

### 人数別の予測所要

予測式 = **読み取り 13 秒 ＋ 書き込み 10 秒 ×（往復比）**。
13 秒は下見の実測、23 秒は live のモデル値（**7 tick 連続で run / no-run を的中**させた値）。

| 人数 | 逐次往復 | 予測所要 | 1 campaign 30 秒契約 | 60 秒打ち切り |
|---|---|---|---|---|
| **50**（現行）| 9 | **23 秒** | ✅ | ✅ |
| **75** | 14 | **29 秒** | ✅（ぎりぎり）| ✅ |
| 100 | 17 | **32 秒** | ❌ 超過 | ✅ |
...
### なぜ危険か — 予約はキュー登録の「前」にある

`claimDelivered`（予約）→ ジョブ作成 → 配信行 upsert → 読み戻し、の順。
**登録の途中で打ち切られると鍵だけが配信済み集合に残り、その人へは二度と送られない**（送信漏れ）。
人数を上げるほどこの窓が広がる。**#563 で塞いだリスクを、env 変更で開け直すことになる。**

⚠️ 正常系には回復手段がある（`delivery_rows_unconfirmed` でジョブ取消＋予約解放）。
   しかし**打ち切りは回復コードごと殺す**ので、その保険は効かない。

### 恒久対策（本 PR）— env だけで上限を超えられなくする

`SYNC_TICK_MAX_RECIPIENTS = 75` を新設し、`resolveMaxRecipientsPerTick()` が頭打ちにする。

- **現在の本番設定（50）は 1 ミリも変わらない**
- env に 500 を入れても **75** で止まる（打ち切り → 送信漏れを防ぐ）
- Background など 15 分枠を持つ経路だけが `cap` を明示して超えられる
- `MAX_RECIPIENTS_PER_TICK`（500）は**正本の設計値として据え置き**（意味を変えない）

検証は `syncTickRecipientCap.test.mjs`（予測式・分割サイズ・契約との整合を機械固定）。

### 他の安全条件への影響

**なし。** duplicate 防止（`DeliveryKey`）／suppression・unsubscribe・bounce／
prospect 限定（`audienceSource`）／step 順序（最小 due step 優先）／
`dispatchLock` はいずれも触っていない。

⚠️ **「50×3=150」の多重起動事故**（2026-09-14）は、`cron-campaign-sequence` に
tick 鍵（`tick:campaign-sequence` / TTL 240 秒）が入って以降、
同一スロットでの多重起動を `tick_busy` で弾く。人数を変えてもこの防御は不変。

## 🚦 現実的な完走プラン（env 変更なしで到達できる範囲と、その先）

### いまの速度と内訳

実測 **約 181 通/時**（7 tick サイクルで free が 4 回 × 50 名）。

> ⚠️ **訂正（2026-09-17）**: 下表は **backlog を捌くのに必要な時間**であって、
> **完走日数ではない**。第 2 期は `delayDays` の合計で**最低 27 日**かかり、
> 送信速度では縮まない。詳細は「④ 速度の見通し」を参照。

| 区間 | 通数 | 181 通/時 で捌く時間 |
|---|---|---|
| 第 1 期 step2 の残り | 約 5,600 | 約 31 時間 |
| 第 1 期 step3 | 約 5,900 | 約 33 時間 |
| 第 2 期 7 通（約 11,800 名）| 約 82,600 | 約 19 日ぶんの**送信量** |
| 合計 | 約 94,100 | — |

⚠️ 第 2 期の 82,600 通は **27 日かけて少しずつ due になる**ので、
「19 日ぶんの送信量」を一度に捌く必要は無い（必要速度は約 128 通/時で、現行で足りている）。

### 段階案（安全条件を 1 つも緩めない）

| 段 | 内容 | 到達速度 | 必要な操作 |
|---|---|---|---|
| **0（現状）** | 50 名 / 4 runs per 7 tick | 181 通/時 | — |
| **1** | env を **75** へ（本 PR の上限まで）| 約 257 通/時 | **本番 env 変更（要承認）** |
| **2** | `campaign-prospect-phase2` の入口判定を軽くする（毎 tick 約 35,500 鍵 → 窓分割）| 約 270〜320 通/時 | コード変更のみ |
| **3** | 配信行 upsert を**並列化**（逐次 50 回 → 束ねる）| 500 名でも 30 秒台が視野 | コード変更（書き込み経路）|
| **4** | 一括登録を **Background**（15 分枠）へ移し、正本どおり **500 名/tick** | **3,000 通/時**（**完走が 1.3 日になるわけではない**）| 設計変更＋要承認 |

- **段 1 は即効だが env 変更**なので停止しています（1.4 倍）
- **段 2 はコードだけ**で、送信量を増やさずに frequency を上げる（本 PR には含めていない）
- 段 4 は正本の設計値 500 に到達する道筋のひとつ。既存パターン
  （`drm-entry-background` / `marketing-campaign-dispatch-background`）に倣える。
  ⚠️ ただし**完走日数は `delayDays` が決める**ので、段 4 でも最短は約 37 日で変わらない

⚠️ どの段でも **1 tick の安全条件・重複防止・suppression・prospect 限定は不変**。
⚠️ 速度を上げると**単位時間あたりの送信量**が増える。deliverability（苦情率・バウンス率）は
   別途見る必要がある。現行 181 通/時 ≒ 4,300 通/日 は穏当だが、3,000 通/時 ≒ 7.2 万通/日 は
   **送信レピュテーションの観点から段階的に上げるべき**。

## 🔎 時間を消費している経路を特定した（2026-09-17 / live 実測）

**「phase2 が原因」ではなかった。** 窓を 1 軸ずつ変えて切り分けた実測:

### ① prospect 索引の読み込み（**支配項**）

`campaign-discount-free` / 台帳 2 ページ固定:
...
件数に比例して伸びる（約 3.1ms/件）。**どの窓でも `送る` は 50 のまま。**
...
| 6 | 17.7 秒 | 6 | 50 |
| 10 | 20.4 秒 | 10 | 50 |
| 20 | 20.9 秒 | **10**（それ以上のページが無い）| 50 |

約 **0.86 秒/ページ**。`DEFAULT_PAGES_PER_TICK = 20` だが、実際に読めるのは 10 ページ。

### ③ 第 2 期の入口判定（#566 で軽量化済み）

約 **3 秒**（2 段引き後）。**支配項ではない。**

### ⚠️ live の全件は測れない

下見は `Math.min(4000, …)` で**4,000 件が上限**なので、
**live の全件（11,799）の所要時間は測定できない**。
ここで「約 N 秒」と外挿してはいけない（2026-09-17 に外挿で 3 回続けて見積りを外した）。

確かなのは 2 点だけ:

1. 索引の件数に比例して伸びる（500 → 4,000 で 8.2 → 19.0 秒）
2. **live は測定できた最大（4,000）の約 3 倍を無制限に読む**

## 🪟 live の prospect 走査を窓で切る（2026-09-17）

**「測っていない無制限」を「測った有限」にする。速くなる幅は主張しない。**

live だけ `maxRecipients` を渡しておらず、索引を**無制限に**読んでいた
（`loadProspectSequenceInputs` の既定は全件）。これを**有限の窓＋回転カーソル**にする。

仕組みは配信台帳の走査（`sequenceLedgerScan.js`）と**同じ**:

- 1 tick が読むのは決まった件数だけ（既定 **2,000**＝実測 12.7 秒）
...
| 項目 | 状態 |
|---|---|
| 1 tick の送信人数 | **50 のまま**（`SYNC_TICK_MAX_RECIPIENTS`）|
| 二重送信の防御 | **不変**（`DeliveryKey` の名指し確認＋予約）|
| `MAX_CAMPAIGN_MS` / `LATEST_START_MS` / 安全余裕 | **不変** |
| env | **変更なし** |
| `delayDays` | **不変** |
| 下見の窓 | **不変**（4,000 上限のまま）|

⚠️ カーソルが読めない・書けないときは**先頭から読むだけ**で、**送信は止めない**
（重なっても `DeliveryKey` が二重送信を防ぐ）。

### まだ確かめていないこと

**この変更が throughput を上げるかは未検証。** deploy 後に実測して記録すること。
窓を 2,000 から動かすときも **live の実測を添える**（外挿で決めない）。

### ⚠️ 窓は step 順序を変えてはいけない（2026-09-17 のレビュー指摘・修正済み）

`selectNextDueStep` は「**全体で**いちばん小さい due step」を選ぶ。
窓で切ると progress が**窓の中だけ**から作られるので、次が起こり得た:
...
実際、本番の窓 0 は `{1:48, 2:5, 3:1947}` と **step3 待ちが濃い**ので、これは現実に踏む。
...
| 窓の最小 due step | 判断 |
|---|---|
| ＝ 選べる最小（step1 除外時は **2**）| **窓のままで安全** |
| それより大きい | **全体を読み直す**（従来どおりの全件）|
| 窓に due が 1 人も居ない | **全体を読み直す**（「全体も 0」とは限らない）|

判定の単一源は `prospectWindowStepSafety.js`。
読み直しに失敗したら**窓のまま進めない**（`prospectInputs = null`・fail closed）。
全件を読んだ tick では**窓のカーソルを進めない**。

⚠️ **性能のために step 順序を変えない。** 遅くなっても順序を優先する。

#### 2 つ目の穴 — 後段フィルタで 0 人になったとき（同日に追加修正）

事前の窓判定は「窓に最小 step の **due** が居るか」しか見ない。
ところが実送信の手前で、**既に `queued` / `sent`・出所フィルタ・許可リスト**によって
...
カーソルが窓 A のとき、既存の `emptySteps` による選び直し（#563）が働くと
...
許されないのは「永久に入れない」「二重に入る」「他 prospect が混ざる」で、
`prospectScanWindow.test.mjs` が窓を回すシミュレーションで固定している
（第 1 期完了者 334 名が全員入口へ到達・二重入口 0・第 1 期未完の混入 0・再入口 0）。


## ⚡ 同期経路の最適化（2026-09-17 / env 変更なし）

**Background 移行を前提にせず、まず同期経路を速くした。** 本番 env・送信量は変えていない。

### ① 第 2 期の入口判定を 2 段引きに（毎 tick 約 19 秒 → 約 6 秒）

従来は「**全 prospect × 全 step**」を 1 回で引いていた:
...
| 実測所要 | 約 **19 秒**（campaign 全体 22 秒）|

第 1 期の完了者は **0 名**なので、この 19 秒は**毎 tick ゼロ件のために**使われていた。

**直し方**: 全 step が揃っているかを知りたいとき、**どれか 1 つでも欠ければ不成立**。
シーケンスは順番に配るので、**最後の step が無い人は必ず未完了**。

1. 各 prospect の**最後の step の鍵だけ**引く（1 人 1 鍵 ＝ 約 11,826 → 往復 **約 60 回**）
2. ①を通過した人だけ、**全 step の鍵**を引く（通常ごく少数・いまは **0 件**）

⚠️ **結論は 1 ミリも変わらない。** ①は必要条件で、最終判定は従来どおり
`planPhase2Entry` が全 step 揃っているかで行う。等価性は
`prospectPhase2EntryProbe.test.mjs` が**従来の引き方と選ばれる人を突き合わせて**固定する。

> ### ⚠️ `delivered` の累計で足切りしてはいけない
>
> 当初「`delivered >= 3` でない人は除外する」案を検討したが、**不正確**だった。
> この集合は `claimDelivered`（＝**キュー登録時の予約**）で作る「送った鍵」で、
> webhook で確認した `delivered` とは**別物**。バウンスすれば
> 「鍵は 3 つあるが `delivered` は 1」もあり得る。**採用しない。**

### ② 配信行 upsert を上限つき並行に（逐次 → 並行度 3）

`CampaignDeliveries` の upsert は 10 件ずつ**完全逐次**だった。
1 往復 **約 1.1 秒**で**待ちが支配的**（Airtable の上限 5 req/秒 ＝ 1 往復 0.2 秒よりずっと遅い）
ため、並行にすれば縮む。

⚠️ **速さより安全**（`boundedBatchWrite.js`・18 件のテストで固定）:

- 並行度は既定 **3**（Airtable の 5 req/秒 に余裕を残す）
- **429 / 5xx は再試行**（従来は 1 回の 429 で tick 全体を巻き戻していた）／4xx は再試行しない
- **締め切り（`MAX_CAMPAIGN_MS`）を越えたら新しい batch を始めない**
  — 予約はキュー登録の**前**なので、途中で打ち切られると鍵だけが残る
- **1 つでも失敗したら全体を失敗**として既存の巻き戻し（ジョブ取消＋予約解放）へ進む
- 冪等性は `performUpsert`（`DeliveryKey` 突合）が担保。再試行しても行は増えない

### ③ 再ベンチマーク — **この表は安全根拠にならない（2026-09-17 訂正）**

> ## ⚠️ 訂正 — 下の表は **窓付き下見**から作った未検証値
>
> この表の秒数は `sequenceTickPreview`（下見）の実測から作ったが、**下見は窓で切られている**:
>
> | | 下見（`sequenceTickPreview`）| **live の tick** |
> |---|---|---|
...
> 実際、この表を根拠に「1 tick に 4〜5 本入る」と見積もったが、
> 本番は **3 本**しか入らなかった（2 サイクルで再現）。
>
> したがって **「100 名で 24 秒」「50 名で 19 秒」「安全に使える最大は 100」は
> いずれも本番で検証されていない。安全根拠として使ってはいけない。**
>
> **本番実測で安全が確認できている人数は `50` だけ**なので、
> `SYNC_TICK_MAX_RECIPIENTS` は **50 へ戻した**。

参考（**構造の比較にだけ使う。秒数は信用しない**）:

| 人数 | 逐次往復 | 下見ベースの逐次(旧) | 下見ベースの並行(新) |
|---|---|---|---|
| **50**（本番実測あり）| 9 | 23 秒 | 19 秒 |
| 75 | 14 | 28 秒 | 23 秒 |
| 100 | 17 | 32 秒 | 24 秒 |
| 200 | 34 | 50 秒 | 36 秒 |
| 500 | 85 | 107 秒 | 70 秒 |

**往復回数が人数に比例する**という構造は変わらない（これは実装から導ける）。
**秒数へ換算して上限を決めるには live の full-index 実測が要る。**

⚠️ 500 が同期 tick に入らないという結論自体は変わらない
（往復 85 回・正本の設計値は Background 前提）。

#### 上限を上げるときの必要条件

1. その人数で **live の tick** が `MAX_CAMPAIGN_MS` に収まることを**本番で実測**する
2. 下見・窓・シミュレーションだけを根拠にしない
3. `syncTickRecipientCap.test.mjs` が「下見ベースの秒数を安全根拠に書かない」ことを固定している

### ④ 速度の見通し — **throughput と完走日数は別物**

> ## ⚠️ 訂正（2026-09-17）— 「N=50 で 18 日 / N=100 で 9 日 / Background で 1.3 日で完走」は**誤り**
>
> これらは **94,100 通を配り切るのに必要な時間（＝ backlog 消化）**であって、
> **完走（delivered 10 / EXHAUSTED 到達）までの日数ではない。**
>
> `sequenceProgress` の正本挙動は
> **「次回予定 ＝ 直近の送信時刻 ＋ *次 step* の `delayDays`」**。
> 1 人が step1 から最終 step まで受け取るには `delayDays` の合計ぶん
> **カレンダー日数が必ずかかり、送信速度では 1 日も縮まない。**

#### カレンダーの下限（catalog の実値）

| campaign | `delayDays` | step1 → 最終 step |
|---|---|---|
| 第 1 期 `campaign-discount-free` | `0 / 5 / 6` | **最低 11 日** |
| **第 2 期 `campaign-prospect-phase2`** | `0 / 3 / 4 / 4 / 5 / 5 / 6` | **最低 27 日** |

**第 1 期を今日終えた人でも、第 2 期の 7 通目までに 27 日**かかる。
`delayDays` は**ユーザー承認なしに変更しない**。
この計算は `sequenceCalendarSpan.test.mjs` が catalog の実値から固定する。
...
| **calendar（完走日数）** | 1 人が最終 step に到達するまで | **縮まない** |

#### throughput（ここが高速化の効き所）

第 1 期の backlog は**一度きりの山**（2026-09-17 実測: step2 due 5,593 ＋ step3 due 5,900）。

| 構成 | free の実行 | 速度 | 第 1 期 backlog 消化 |
|---|---|---|---|
| 現状（最適化前・N=50）| 4 / 7 tick | 171 通/時 | 約 **2.8 日** |
| 最適化後・**N=50**（env 変更なし）| **5 / 7 tick** | **214 通/時** | 約 **2.2 日** |
| ~~最適化後・N=100~~ | — | **未検証**（下見ベースの外挿。`SYNC_TICK_MAX_RECIPIENTS` は 50 へ戻した）| — |

第 2 期が calendar どおり進むのに必要な速度は
**11,826 人 × 7 通 ÷ 27 日 ＝ 約 3,066 通/日（約 128 通/時）**。
**現行の 171 通/時（4,104 通/日）で既に足りている。**

⚠️ つまり**第 2 期のためには速度を上げる必要がない**。高速化の価値は次の 3 点:

1. **第 1 期の backlog（一度きりの山）を早く捌く**
2. **滞留を防ぐ**（due が積み上がったまま進まない状態を作らない）
3. **同じ日に大量の cohort が due になっても処理できる**
   — 第 1 期の完了が重なると、その人たちの第 2 期 step1 が**同日に集中**する。
   平均 3,066 通/日 に対してピークはそれを大きく超え得るので、**余力が要る**

#### 最短の完成時期（calendar 支配）

| 段階 | 目安 |
|---|---|
| 第 1 期 step2 の残りを捌く | 約 1〜3 日 |
| その人の step3 は **+6 日**（`delayDays`）| |
| 第 1 期の完了が出そろう | 約 **8〜10 日後** |
| 第 2 期 step1 → step7 | **＋27 日** |
| **delivered 10 / EXHAUSTED** | **最短でも約 37 日後** |

⚠️ **速度を上げてもこの 37 日は縮まない。** 縮むのは最初の 1〜3 日の部分だけ。
⚠️ Background（3,000 通/時）にしても**完走は 1.3 日にはならない**。同じく約 37 日が下限。
...
5. **正本/公開 DTO と実画面の既存不一致（枠・通算成績）の確認が必要**（下記）

### 5. 正本/公開 DTO と実画面の既存不一致（枠・通算成績）— 未確認

2026-09-15 の本番確認で見つかった、**メールとは独立の既存問題**。

| 項目 | `docs/spec.md` | `buildFreePublicRows()` | 実画面 |
|---|---|---|---|
| **枠** | 無料公開側に記載 | `frame` を DTO に持たせている | per-horse 表示**無し** |
| **通算成績** | 無料公開側に記載 | コメントで公開事実として扱う | 表示**無し** |

確認すべきこと:

- 実画面に出ていないのは**意図的な非表示**か、**表示漏れ**か
- 意図的なら `docs/spec.md` と `freePublicView.js` の記述を実態へ合わせる
- 表示漏れなら、無料ページ側に表示を足すかを判断する
  （⚠️ ただし `/free-prediction/` の公開範囲を広げる話になるため、
   `docs/spec.md` §触ってはいけないこと「第 2 層を兼ねさせる目的で公開範囲を広げる」に
   抵触しないかを先に確認すること）

⚠️ **PR #552 では解消していない**（メール文面を実画面に合わせただけ）。
   公開範囲・UI・正本は 1 つも変更していない。
...
### ⚠️ 本番確認で見つかった 2 種類のズレ（PR #552）

本番の実ページと突き合わせた結果、**性質の違う 2 種類**のズレが見つかった。

#### (a) 語彙の違い（公開範囲は一致している）

| メールの語（旧）| 画面の実際 | DTO のフィールド |
|---|---|---|
| 厩舎 | **調教師** | `trainer` |
| 過去走 | **近走**（「近走詳細データ」）| `recent` |

同じものを別の語で呼んでいるだけ。メール側を画面の言い方へ揃えた。

#### (b) 正本・公開 DTO と実画面の**不一致**（語彙の問題ではない）

| 項目 | `docs/spec.md` | `buildFreePublicRows()` | 本番の実画面 |
|---|---|---|---|
| **枠** | 無料公開側に**記載あり** | `frame` を DTO に**持たせている** | per-horse の表示**無し**（「場状態・天候・距離適性・枠順」の説明文中のみ）|
| **通算成績** | 無料公開側に**記載あり** | コメントで公開事実として**扱っている** | 表示**無し**（`N戦M勝` 等のパターンを検出できず）|

⚠️ **「語彙だけのズレ」ではない。** 正本と DTO は公開対象として扱っているのに、
実画面に出ていない。**どちらが正しいかは未確定**。

PR #552 では**メールの文面だけ**を現在の実画面に合わせ、
**公開範囲・UI・正本は一切変更しない**（勝手に仕様変更して解消しない）。
...
同じ「テストの固定日付が腐る」問題を、**2 つのセッションが正反対の方針で**直した。

| PR | 方針 |
|---|---|
| **#537**（先着 `e26accc6`）| 「滞留していない予約」の fixture を**実時計基準**（いま − 1 日）にする |
| **#538**（当方 `af81eed7`）| fixture は固定のまま、**テスト側の時計を固定**する |

当方の branch は #537 より前から切っており、**#538 の CI は #537 を含まない状態で green**。
git は競合なく自動マージしたが**意味的に衝突**し、固定日付の `ClaimedAt` と
実時計基準の `StartsAt` が固定時計の下で前後逆転して **main が 8 件 red** になった。

⚠️ 2026-09-14 の #525 / #526（`gates.allOpen` の文字列一致 guard）と**同じ型**。
   1 日で 2 度踏んだ。

### 是正

**#539（`4d6a268e`）** で #537 を正とし、ファイル全体の時計固定を外した。
境界を見るテストは `StartsAt` と `nowMs` を**両方その場で明示**する自己完結型へ。
#537 の fixture 定義行は 1 行も変えず、revert もしていない。

副産物として `premiumPlusCouponAdmin.test.mjs` に残っていた
**2026-10-02 に発火する別の時限爆弾**（`ClaimedAt` 固定 × 予約が実時計基準）も閉じた。

### 運用ルールとして確定（MK 指示）

> **PR merge 直前には必ず最新 `origin/main` を fetch し、
> 同じ変更領域に別セッションの変更が入っていないことを確認する。
> 入っていれば追随・再検証してから merge する。**

正本は [`docs/decisions.md`](./decisions.md) の
「2026-09-15 — PR は merge 直前に必ず最新 origin/main を取り直す」。
`CLAUDE.md`「停止して確認を取る操作」の直後からも辿れる。

⚠️ **「自分の PR の CI が green」は「main が green のままでいる」ことを保証しない。**
   branch の CI は相手の変更を含まない状態で走っている。
...
<!-- 並び順: 最上位は「反応で選別する大規模マーケティング配信基盤」（約 15,000 件 / PR #521）。
     ...
     先頭を奪わないこと。 -->
# 🔍 自動点検が動かなかった原因（2026-09-19）— **特定・修正済み / 反映は未実施**

## 何が起きたか

2026-09-19 11:20 UTC（20:20 JST）の自動点検が**記録を残さなかった**。
Netlify の関数ログを取ったところ、**定期実行は予定どおり起動していた**。

```
11:20:12.613Z {"fn":"sendgrid-selection-watch","ok":false,"reason":"sendgrid_http_400"}
11:20:14.731Z Duration: 1586.52 ms
```

## 原因

`GET /v3/marketing/stats/singlesends` の **`page_size` は 1〜50**。
`page_size=100` を渡していたため **400** が返り、点検が**最初のデータ取得で終了**していた。

```
page_size=100 → 400 {"message":"must be integer between 1 and 50","parameter":"page_size"}
page_size=50  → 200
```

## もう 1 つ見つかった問題（こちらの方が危ない）

管理画面向けの `overview` は同じ 400 を **握り潰して 0 を返していた**。
「読めなかった」と「0 件」が区別できず、**送っていないのに送信実績 0 と表示**されうる状態だった。

## 直したこと

- `page_size` を **50** にする（定数 `STATS_PAGE_SIZE` に集約・テストで 50 超を禁止）
- 点検は**実績が読めなくても止まらない**。読めなかったことを `stats_unavailable` として
  **異常に数える**（黙って「異常なし」にしない）
- `overview` は読めなかった素材を `取得できなかったもの` として返し、実績は **0 ではなく null**
- 管理画面は「**取得できず**」と表示し、「0 件という意味ではありません」と添える

## 副産物（`#591` で入れた切り分け）

点検は**走り出した時刻を最初に**記録するようになった（`最終起動`）。
今回のように途中で落ちても、次からは「動いていない」と「途中で落ちた」を外から区別できる。

# 🤖 メール配信を完全自動運用へ（2026-09-19）— **実装済み / 有効化は未実施**

**MK が毎日触らない**ことを目的に、足りていなかった 3 つを埋めた。**新しい配送基盤は作っていない。**
...
- 異常時に**自動では止めない**（通知のみ。止める判断は人が 1 回）

## 残る人手（これだけ）

1. ~~`ak-drm-engaged` list を 1 回作る~~ → **2026-09-19 に作成済み**（空・既存 list は不変）。
   ただし**作成前に反応した 237 名は入っていない**（一括投入は要承認）
2. **`SENDGRID_WEEKLY_ENABLED=true` を 1 回開ける**（選別が終わってから / 承認事項）
3. cutover のときだけ reconcile を 1 回承認する

## 管理画面（2026-09-19 追加）

`/admin/premium-plus-eligibility/` に「📮 メール配信」タブを追加（**読むだけ**）。
選別の進み / 反応と次の導線 / 週次 / **自動点検の最終実行**を表示する。
画面は `admin-marketing` の `mailOverview` を叩き、数の作り方は `marketingOverview.js` に集約。

未実装: 有料転換・売上・DRM 段の表示（次の増分）。

## 2026-09-19 に直したこと

- **`no_main_race` の原因**: 週次の文面組み立てが showcase の形を取り違えていた
  （`venues` は**会場名の配列**で、明細は `venueGroups`）。実物から組むテストを足して固定。
  実データで文面が組め、品質基準も通ることを確認済み
- **過去に反応した人の取りこぼし**: `reconcile`（`scope:'excluded'`）が
  ENGAGED / PROMOTED を継続 list へ入れるようにした（**すでに居る人は数えるだけ**・
  配信停止/bounce/苦情/打ち切りは入れない・**メールは送らない**）

# 📆 選別配信 27 通を予約した（2026-09-20 19:00 JST 開始）— **予約済み / 初回配信はこれから**

## 予約の内容

| 宛先 list | 通数 | 初回 | 最終 |
|---|---:|---|---|
| `ak-prospect-select-start-1` | 10 | 2026-09-20 19:00 JST（m01）| 2026-09-29 19:00（m10）|
| `ak-prospect-select-start-2` | 9 | 2026-09-20 19:00 JST（m02）| 2026-09-28 19:00（m10）|
| `ak-prospect-select-start-3` | 8 | 2026-09-20 19:00 JST（m03）| 2026-09-27 19:00（m10）|

**1 人が 1 日に受け取るのは 1 通**。27 通すべて `status=scheduled`・宛先 list 1 本・
日時は毎日 19:00 JST であることを **GET で 27 件すべて検証済み**。

## 予約の前に満たした 5 点（read-only）

| # | 確認 | 実測 |
|---|---|---|
| ① | reconcile actionable | **0**（active 24 窓 / excluded 11 窓）|
| ② | 総数 | AK active 11,708 − provider rejected 10 = **11,698** ＝ list 合計 11,698 |
| ③ | 通し番号ごと | n1 318/318・n2 2,587/2,587・n3 8,793/8,793 |
| ④ | Single Send | 27 通 draft・予約 0（予約前）|
| ⑤ | 旧 AK | `engine=sendgrid`（prospect 宛は停止）|

判定と停止条件は `scripts/sendgrid-schedule-single-sends.mjs`（既定は下見・
`--apply --confirm` の両方でだけ予約・1 つでも欠ければ予約しない）。

## 初回配信後に確認すること（**未実施**）

1. 実際の送信人数（list の人数と一致するか）
2. 反応者が**次の号から外れている**か（webhook の list 除外）
3. **二重送信 0**（同じ人に同じ号が 2 回出ていない）
4. 旧 AK prospect 配送が止まったままか
...
⚠️ 本番での実行は **書き込みゲート開放**が要る（未実施）。下見は deploy 済みで動く。

**予約の可否は「反応が止まること」では決めない**（止まらないので無期限延期になる）。
予約直前に全件照合して **actionable 0** なら予約してよい。理由と 5 点の条件は
[`SENDGRID_MC_MIGRATION.md`](./SENDGRID_MC_MIGRATION.md) §15。
...
> メールアプリ内のブラウザで開くと、別のブラウザでは再度ログインが必要になる場合があります。
> 以下も削除→リンクを開いただけでは使われません（確認画面で「ログインする」を押したときに使われます）。
> その他のwellcomeメールや他に上記の文章が含まれていたら削除もしくは調整して
> 理由は現在必要ない文言だと思われる
...
| 場所 | 対応 |
|---|---|
| ログインメールの 📱 ブラウザ案内ブロック | **削除**（ブロックごと）|
| ログインメールのコピー用 URL の補足「リンクを開いただけでは使われません…」| **削除**。案内文（コピーして貼り付け）は残す |
| `/login/` の `no_session` 文面 | **調整**。別ブラウザの説明 2 文を削除し「もう一度ログインしてください」だけに |
| `/auth/verify` 確認画面・成功画面 | **簡素化**（下記）|

## 調べた範囲

顧客向けメールを送る Function を全部見て、同じ文言があるのは
`send-magic-link.js` **だけ**だった（welcome メールという名前の Function は存在しない）。
他の「ブラウザ」の語はコード内コメントで、顧客には出ない。

## `/auth/verify` の簡素化（MK 追加指示）

> 確認画面は「ログインするには、下のボタンを押してください。」、
> 成功画面は「ログインしました。まもなくマイページへ移動します。」程度に

| 画面 | 前 | 後 |
|---|---|---|
| 確認 | ログインの確認 / 下の「ログインする」を押すと、このブラウザへのログインが完了します。/ 押すまでリンクは使われません… | **ログイン** / **ログインするには、下のボタンを押してください。** |
| 処理中 | 認証中... / トークンを確認しています。しばらくお待ちください。 | **ログインしています** / **そのままお待ちください。** |
| 成功 | ログイン成功 / ✅ このブラウザへのログインが完了しました。次回から同じブラウザの… / 6秒後に /dashboard/ へ移動します。 | **ログインしました** / **まもなくマイページへ移動します。** |
| 失敗 | 認証失敗 / 無効なトークンです。 | **ログインできませんでした** / **このリンクは使用できません。** |

読ませる文章が無くなったので自動遷移は **6 秒 → 2.5 秒**。
（旧 guard は「案内を読み切れるよう 5000ms 以上」を強制していた。前提が消えたので
1500〜5000ms の範囲チェックへ替えた。）

## 確定方針を正本へ（2026-09-10 MK）

> ユーザー向け画面・メールには、token消費、ブラウザ差、長押し、先読み、セッション等の
> 内部事情を原則出さず、「ユーザーが今すること／結果」だけを表示する

`docs/spec.md`「仕組みの説明も出さない」＋ `docs/decisions.md` 37 として固定。
**理由の説明はソースのコメントと docs に残す**（消さない）。

## 固定

- `userFacingNoInternals.guard.test.mjs`（新規）… `/auth/verify` の表示文言 /
  `/login` の理由表示 / ログインメール本文を、禁止語で検査する。
  **コメントは対象外**（理由はコメントに書いてよい）
- `scripts/e2e-magic-link-verify.mjs` … **実際に描画された文字**を確認 / 成功 / 失敗の
  3 画面で検査（21 項目へ拡張）
- `loginReasonNotice.guard.test.mjs` を「案内がある」→「**戻していない**」へ反転。
`verifyRequiresUserAction.guard.test.mjs` も同様（メールの補足は戻さない / 画面側の
「押すまで使われません」は維持）。
...
| 内容 | 結果 |
|---|---|
| `loginReasonNotice` / `verifyRequiresUserAction` guard | 14 pass / 0 fail |
| `e2e:magic-link` | **21 pass / 0 fail** |
| `e2e:admin-plus`（回帰）| 56 pass / 0 fail |
| `check:safety` | exit 0 |
| `build` | exit 0 |

# 🔑 ログインリンクが「コピーしただけ」で使用済みになる — **修正（2026-09-10）**

> `/auth/verify` の起動条件のみ。トークンの単回性・有効期限・会員判定（サーバー側）は**変更していない**。
...
`/auth/verify` は**開いた瞬間に** JS が `verify-magic-link` を呼び、トークンを使用済みにしていた
（`(async () => { ... await fetch ... })()`）。
iOS はリンクを長押ししただけで**プレビューを描画し、その中で JS を実行する**ため、
コピーしようとしただけで消費される。メールのリンク検査ボット・先読みでも同じことが起きる。
利用者からは「コピーして貼ったら必ず認証エラー」に見える。

## 直し方（MK 指示）

> GETで /auth/verify を開いただけではtokenを絶対に消費せず、
> 「ログインする」の明示操作時だけ消費する仕様にしてください。

| 変更 | 内容 |
|---|---|
| `/auth/verify` | 自動検証をやめ、「ログインする」ボタンの **click でだけ** 検証する |
| 画面 | 「押すまでリンクは使われません。コピーや長押しでは消費されません」を明示 |
| 通信エラー時 | トークンは消費されていないため、**もう一度押せる**ようにする |
| ログインメール | 「開いた画面で『ログインする』を押してください」「リンクを開いただけでは使われません」を追記 |

サーバー側（`verifyMagicLinkFlow.js`）は**無変更**。単回使用・60 分の有効期限・
Cookie 発行後に消費する順序はそのまま。

## 固定（ソース検査 + 実 DOM の両方）

| | 何を守るか |
|---|---|
| `verifyRequiresUserAction.guard.test.mjs` | 消費する通信が 1 か所だけ / それが click 用関数の中にある / `DOMContentLoaded` `load` `pageshow` `visibilitychange` `focus` で自動起動しない / async IIFE へ戻していない |
| `scripts/e2e-magic-link-verify.mjs`（新規・実ブラウザ）| **本物の markup と script をそのまま読み込み**、`fetch` を差し替えて**呼ばれた回数**を数える |

E2E で確認していること（18 項目）:

- 開いただけでは**通信 0 回**
- 読み込み・可視化・フォーカス・**長押し相当（mousedown / contextmenu）**でも**通信 0 回**
- ボタンが見えていて押せる（`elementFromPoint` で他要素に覆われていないことまで）
- 押したときだけ **1 回だけ**消費し、トークンと `credentials: 'include'` を送る
- **連打しても 1 回**
- 使用済みなら「既に使用済み」＋再送導線を出し、押せないボタンを残さない
- token 無しでは 1 回も通信しない

## 検証

| 内容 | 結果 |
|---|---|
| `npm run e2e:magic-link`（新規・実 DOM）| **18 pass / 0 fail** |
| `npm run e2e:admin-plus`（回帰）| 56 pass / 0 fail |
| `check:safety`（新 guard は `test:auth-session` の glob で自動的に入る）| exit 0 |
| `build` | exit 0 |

CI へ step を追加（`E2E（ログインリンクは押したときだけ使われる・実 DOM）`）。
ビルド成果物は要らないので、`dist` の有無に依存しない。
...
払戻は 1 桁違うと実績（最高払戻・的中時平均払戻）まで狂う。

## 変更

| 箇所 | 変更 |
|---|---|
| 払戻金額（`#payout`）| `type="number"` → `type="text" inputmode="numeric" autocomplete="off"` |
| 払戻単価（`#unitPayout`）| 同上 |
| レース番号（`#raceNumber`）| 同上（`maxlength="2"`。MK 依頼で追加）|
| 入力の受け取り | 数字以外は入力時に落とす（`599,000` を貼っても桁を落とさず `599000`）|
| プレビューの払戻単価 | `parseInt('599,000')` = 599 になる解釈をやめ、数字だけ取り出してから解釈 |

スマホでは `inputmode="numeric"` で数字キーパッドが出る。見た目は変わらない
（CSS は `input[type=text]` も同じ扱い）。

## 検証

| 内容 | 結果 |
|---|---|
| `adminResultsUiGuard.test.mjs`（type が戻っていないこと・サニタイズ・桁落ち防止を固定）| **18 pass / 0 fail** |
| `check:safety` | exit 0 |
| `build` | exit 0 |
...
| # | 内容 |
|---|---|
| 1 | E2E を **`dist` の静的配信ベース**へ作り替え（Edge 認証を触らない・CI で動く）|
| 2 | 検証項目を **56 項目**へ拡充（MK 指定の 14 観点をすべて含む）|
| 3 | スタブを**書き込みで状態が実際に変わる**形にし、「押したのに表示が変わらない」型を検出可能に |
| 4 | `safety-check.yml` に**必須ステップ**として追加（依存パッケージは追加しない）|
| 5 | 正本に「ロジックテスト＋実 DOM E2E の両方で守る」を固定 |

## E2E が検出した実バグ

| 検出 | 判定 |
|---|---|
| 一覧の行に「一時停止中」チップと「販売停止中」バッジが二重に出る | **実バグ → 修正**（主状態が 4 分類になったので重複）|
| 停止後に一覧が更新されない（ように見えた）| **誤診 → 撤回**。`refreshOne()` が内部で `render()` 済みで製品は正しく、E2E スタブが `lookup` を書き込み扱いにしていたのが原因 |

⚠️ 一度は製品側に `render()` を足したが、原因が E2E 側と判明したので**元に戻した**。
（`refreshOne` が読み直して描き直す設計は正しい）

## 結果

| 内容 | 結果 |
|---|---|
| `e2e:admin-plus` | **56 項目 pass / 0 fail**（未認証チェック含む）|
| `test:premium-plus-admin` | 89 pass / 0 fail |
| `check:safety` / `build` | exit 0 / exit 0 |

## 未実施

- **merge / 本番反映**
- 実管理画面での目視 / 本番での停止・再開の実押下

# 🐛 「今すぐ販売可なのに 9/19 から購入可」— **修正・E2E 導入（2026-09-10）**

> 表示のみの修正。顧客データ・env・送信は**一切変更していない**。

## MK 報告

> Audenki99@gmail.com にクーポン14日にしたら19日から購入可の表示
> 今すぐ販売可にしても9日の販売可の表示に変わらない

## 本番データで確認した事実（read-only）

| 項目 | 値 |
|---|---|
| recordId | `receX3HniQySnbt0g` |
| `PremiumPlusEligibility` | eligible |
| `PremiumPlusEligibleAt` | **2026-09-09T08:54:04Z（今日）** |
| `PremiumPlusReleaseOverride` | **phase4（今すぐ販売可は適用済み）** |
| 判定の結果 | `phase 4 / overrideApplied true / purchaseEnabled true` = **いま買える** |

**判定は正しかった。** 今日 eligible にしたため段階公開の予定が
「購入解禁 = 10 日後 = 9/19」になり、**画面の見出しだけ**がその未来日を
「2026-09-19 から購入できる状態です」と出していた。

## 直したこと

| # | 内容 |
|---|---|
| 1 | 買える会員に**未来の予定日を購入開始日として出さない**（`isFutureJst` で判定）|
| 2 | 待機日数を飛ばしているときは「段階公開の予定（購入解禁 9/19）は**適用されません**」と書く |
| 3 | 見出しは「**いま購入できる状態です**」へ |

## E2E を導入（MK 指摘「e2eでテスト確認検証しないの？」）

**ユニットは 89 件すべて通っていたのに本番で間違って見えていた。**
実ページを Chromium で開いて DOM を検証する E2E を追加した。

`astro-site/scripts/e2e-admin-plus.mjs` / `npm run e2e:admin-plus`

- 管理 API はブラウザ側の `fetch` 差し替えで合成データ。**本番へは触れない**
- **E2E が別の実バグを検出**: 全行から「詳細・操作」ボタンが消えていた
  （PR #509 の編集で `c8.appendChild(btn)` が落ちていた。生成はしているので
  ソース検査のガードでは通ってしまう）→ 修正し、ガードも append まで見るよう強化

## テスト

| 内容 | 結果 |
|---|---|
| `test:premium-plus-admin` | **89 pass / 0 fail**（回帰 5 ケース追加）|
| `e2e:admin-plus` | **全項目 pass** |
| `check:safety` / `build` | **exit 0 / exit 0** |
...
⚠️ E2E のためローカルで edge 認証を一時退避したが、**戻したことを差分ゼロで確認済み**
（`.env` などの混入も 0 件）。

# 🖱️ 管理画面 UX 第 2 弾 — **実装・検証完了 / Draft PR（2026-09-10）**

> 表示と一覧の分類のみ。顧客データ・env・送信は**一切変更していない**。

## 最優先の不具合（修正済み）

**停止した会員を再開する手段が画面から消えていた。**

2026-09-09 に販売の切替をトグル 1 つへ集約した際、`describeLaunchAction` の
「未開始 + 停止中 → 再開スイッチを出さない」（旧設計では
クーポン期間の開始が同時に再開する前提）を残したままだったため、
**停止中の会員に押せる再開操作が存在しなかった**。

| 直した点 | 内容 |
|---|---|
| ボタン名 | 「販売停止中」→ **「販売を再開」** / 「販売中」→ **「販売を停止」**（状態名をボタンにしない）|
| 再開スイッチ | 停止中は**クーポン期間がどの状態でも**出す（未開始 / 開始済み / 途中 / 確認不能）|
| Redis 不通時 | 再開を塞がない（販売の停止/再開は Airtable だけの操作）|
| 同時表示 | 停止と再開を同時に出さない（常にどちらか 1 つ）|

## 一覧・視認性

| # | 内容 |
|---|---|
| 1 | 上部を**大きな 4 カード**へ（購入可能 / 販売停止中 / 段階表示中 / 対象外）。押すと絞り込む |
| 2 | 「購入可能 4名 **0**」の意味不明な 0 を解消（件数をラベルと別要素に）|
| 3 | 会員行は**どこを押しても**詳細・操作を開く（メール・バッジ・行全体・キーボード）|
| 4 | 行内のボタンは行クリックとして扱わない（誤操作防止）|
| 5 | 検索を**常時展開の主入口**へ（完全一致 1 件で自動的に詳細を開く）|
| 6 | 細かい条件 7 つを「**詳細条件を開く**」へ（既定は閉じる・削除はしない）|
| 7 | 本文 16px / 表 1rem / 入力 1rem / カードの件数 2.1rem へ拡大、薄い文字を明るく |

効果測定は 2026-09-09 の折りたたみのまま（集計データは削除していない）。

## テスト

| 内容 | 結果 |
|---|---|
| `test:premium-plus-admin` | **81 pass / 0 fail** |
| 停止 ⇄ 再開の往復・停止中はどの状態でも再開できる | 追加 2 ケース |
| 行クリック / 検索の主入口 / 詳細条件の折りたたみ / 文字サイズ | 追加 4 ケース |
| `check:safety` | **exit 0** |
| `build` | **exit 0** |

旧挙動を固定していた既存ガード 6 件を確定仕様へ更新
（`adminReopenStart.smoke` / `premiumPlusReopenLaunch` / `premiumPlusAdminStatusSummary` /
`adminListUxWiring` / `adminUiGuard` / `adminMultiFilterUi`）。
うち `adminMultiFilterUi` はマーケ / カムバックの検索は従来どおり畳んだままにし、
**Plus 一覧の検索だけ**主入口へ変えたことを固定。

## 未実施

- **merge / 本番反映**
- 実管理画面での目視（Basic 認証のため本番画面は開けない）
- **停止 ⇄ 再開の本番実押下**（ゲートは有効なので押せば実際に切り替わる）

# 📋 Premium Plus 管理画面 一覧を「状態・次の操作」中心へ — **実装・検証完了 / Draft PR（2026-09-09）**

> 表示ロジックと一覧の分類のみ。顧客データ・env・送信は**一切変更していない**。
...
旧一覧の `classify()` は**内部の軸をそのまま並べていた**。

| 旧表示 | 実際の意味 |
|---|---|
| 即時販売 | override を当てた（**操作の名前**。状態ではない）|
| 販売可 | 資格が eligible（**まだ買えない段階公開中も含む**）|
| 販売中 | PHASE 4（**停止中でも同じ表示**）|

`classify()` は **salePaused を一切見ておらず**、override 無しで PHASE 4 に達した
odamoto は「即時販売」で拾えなかった。「いま買える会員」を探す入口が無かった。

## 直したこと

| # | 内容 |
|---|---|
| 1 | 主状態を **4 分類**へ（購入可能 / 販売停止中 / 段階表示中 / 対象外）|
| 2 | **「購入可能」は `purchaseEnabled` だけで判定**（override / PHASE で分岐しない）|
| 3 | 停止を**最優先**で判定（停止中を購入可能・段階表示中に混ぜない）|
| 4 | 状態フィルタを 4 分類へ。**「即時販売」を選択肢から削除** |
| 5 | 運営サマリーを「購入可能 N 名 / 販売停止中 N 名 / …」へ |
| 6 | 一覧から内部の列 7 つを外す（6 列に。横スクロール削減）|
| 7 | 行に**大きな色バッジ** + **次の操作 1 個**（販売を停止 / 販売を再開）+ 詳細・操作 |
| 8 | **メール完全一致 1 件なら詳細パネルを自動で開く** |
| 9 | 効果測定を**既定で閉じた折りたたみ**へ |
| 10 | 異常のある会員だけ警告。要対応は**対象会員を開けるボタン**に |
| 11 | 「対象外」の**理由**（販売対象外 / 資格保留）を一覧のバッジに短く添える |

実行処理は詳細パネルの 1 か所へ委譲（確認ダイアログ・履歴記録・その場更新を二重実装しない）。

## テスト

| 内容 | 結果 |
|---|---|
| `test:premium-plus-admin`（6 ファイル）| **71 pass / 0 fail** |
| 一覧の分類・サマリー・完全一致・行操作 | 14 ケース |
| 一覧の配線（単一源委譲・折りたたみ・内部用語なし）| 9 ケース |
| `check:safety` | **exit 0** |
| `build` | **exit 0** |

旧一覧の設計を固定していた既存ガード 8 件は、確定仕様へ更新した
（`adminUiGuard` / `plusNotifiedWiring` / `premiumPlusImmediateSale`）。
案内・実閲覧・表示判定は**判定そのものは残し**、一覧の列から外したことを固定。

## PR の依存関係（2026-09-09 確認）

| PR | 状態 | 関係 |
|---|---|---|
| #508 | **MERGED**（squash `d7a76228`）| 詳細パネル + 停止の実効化 |
| #509 | OPEN / Draft・base = `main`・CLEAN | **#508 を含む**（`d7a76228` から分岐）|

`git merge-base --is-ancestor d7a76228 feat/admin-list-ux` = 真。
**merge 順は #508 → #509 で、#508 は反映済み。#509 はそのまま merge できる。**

重複・取りこぼしの確認:
- 両方が触るファイルは 6 件あるが、#509 は #508 の版の**上に**足しているだけ（revert なし・CLEAN）
- #508 の成果物（要約パネル / 折りたたみ / media の停止ゲート / クーポン期間の改称 /
  guard 3 ファイル）が #509 ブランチに**すべて健在**であることを実体で確認

## 未実施

- **merge / 本番反映**
- 実会員・実管理画面での目視（Basic 認証のため本番画面は開けない）

# 🖥️ Premium Plus 管理画面を「状態 → 次の操作」型へ刷新 — **実装・検証完了 / Draft PR（2026-09-09）**

> 顧客データへの書き込み・送信・付与・env 変更は**一切していない**。
> 変更したのは管理画面の表示と、停止中の可視性を閉じるサーバー判定のみ。

## きっかけ（MK 指摘）

> 再募集を開始するボタンが押せそうだからと、今すぐ販売可も押せそうで、矛盾している
> なぜ矛盾がないようにテスト確認してくれないの？
> 内部処理感が強く、人間にはわかりずらい、読みずらい
> （追加）眼精疲労を抑えるため「細かい文章を読まなくても運営できる」ことを完成条件に

## 本番実データで確認した対象会員の状態（read-only）

| 項目 | 値 |
|---|---|
| 会員 | odamoto@gmail.com（`rec1zljY6Ozb23gd1`）|
| 三連複 | 保有（`LifetimeSanrenpuku: true`）|
| 販売資格 | eligible（確定 2026-08-25）|
| 段階 | **PHASE 4 = 購入可能（2026-09-04 から）** |
| 停止 | していない |
| クーポン | 未取得 |
| クーポン期間 | 未開始 |

つまり**すでに購入できる会員**なのに、画面には「今すぐ販売可」と
「この会員の再募集を開始する」が同時に押せる形で並んでいた。

## 直したこと

| # | 内容 |
|---|---|
| 1 | 会員詳細の先頭に **色バッジ + 大きな 1 行 + 段階 1 行**（購入可否・停止・段階・クーポン・期間）|
| 2 | その下に **次に可能な操作だけ**を大きなボタンで表示 |
| 3 | 従来の詳細セクション 10 個は**削除せず**下部の**折りたたみ**へ移動（既定は閉じる）|
| 4 | 販売状態は**トグル 1 つ**（「販売中」→ 停止 / 「販売停止中」→ 再開）|
| 5 | **すでに購入できるなら「今すぐ販売可」を出さない** |
| 6 | 段階表示を人間向けに（「段階表示 5 日目 ・ 予告のみ表示中 ・ あと 2 日で商品ページ表示」）|
| 7 | 主表示から **PHASE / override / eligibility / 再募集** を排除 |
| 8 | 「再募集を開始する」→「**クーポンの利用期間（14日）を開始する**」へ改称 |
| 9 | **停止中は実績画像も 404**（`premium-plus-media` GET に会員ゲート）|

実行処理は既存ボタンへ**委譲**（click）。確認ダイアログ・履歴記録・再描画を二重実装していない。

## 停止の実効化（URL 直打ち対策）

| 面 | 停止中 |
|---|---|
| 商品ページ | 受付休止の案内のみ（商品内容なし・従来どおり）|
| 実績画像 `premium-plus-media` GET | **404（今回追加）**。fields が読めない / Plus 対象外 / 停止中は返さない |
| クーポンページ | 見せる（停止中こそ配る相手）|

判定は既存の単一源 `resolveUpsellForCustomer()` のみ。面ごとに停止判定を再実装していない。

## テスト

| 内容 | 結果 |
|---|---|
| `test:premium-plus-admin`（新規 4 ファイル）| **48 pass / 0 fail** |
| うち「状態 × 操作の全組み合わせで矛盾ゼロ」| eligibility 3 × PHASE 4 × 停止 2 × override 2 × 期間 5 = **480 通り** |
| `check:safety` | **exit 0** |
| `build` | **exit 0** |

矛盾として固定した観点: 買えるのに「今すぐ販売可」が出ない / バッジと段階表示が一致 /
停止中に「買える」と出ない / 販売切替は常に 1 つでラベルが現状を表す /
押せない操作には必ず理由がある / 主表示に内部用語が出ない。

## 目視確認（ローカル・本番非接触）

管理画面は Basic 認証の背後なので、**同じ CSS と同じ判定**で描画するハーネスを作って確認。

| パターン | バッジ | 出たボタン |
|---|---|---|
| 購入可能（実データ相当）| 購入可能（緑）| 販売中 / クーポンの利用期間（14日）を開始する |
| 段階表示中 | 段階表示中（黄）| 販売中 / 今すぐ販売可にする / クーポン期間 |
| 販売停止中 | 販売停止中（赤）| **販売停止中 のみ** |

購入可能の会員に「今すぐ販売可」が出ないこと、停止中はトグル 1 つだけになることを確認。

## 未実施（判断待ち）

- **merge / 本番反映**
- 実会員での目視（本番の会員セッションはこちらで作れない）
- 停止トグルの本番実行（`PREMIUM_PLUS_SALE_PAUSE_READY` の env 状態は未確認・未変更）

## rollback

| 変更 | 戻し方 |
|---|---|
| 画面・文言・要約 | `git revert`（表示のみ。データは触らない）|
| 停止中の実績 404 | 同じ revert で従来の挙動へ戻る |
| 停止フラグ | admin のトグルを押し戻す（Airtable 1 行・他会員に影響なし）|
| クーポン期間の開始 | `HDEL ak:pp:reopen:v1:members <recordId>` |

# 🧹 顧客向け画面から内部運用の文言を除去 — **実装・検証まで完了（2026-09-09）**

> コード変更は**表示文言と 1 本のリンク削除のみ**。ロジック・データ・env・顧客データへの
> 書き込みは一切していない。

## きっかけ

MK 報告「内部情報が堂々とユーザーに公開されてしまってます」（`/premium-plus-v2/` の注記）。

## 直した箇所（顧客向け 149 ページを横断監査）

| ページ | 直した内容 |
|---|---|
| `premium-plus-v2.astro` | 注記から「控えを保存していなかった」「集計外」「自動集計」を削除 |
| `archive-sanrenpuku-all/index.astro` | 内部ルール名 `AI_SANRENPUKU_AXIS_V1` の直書きを削除 |
| `archive/jra/[year]/[month]/index.astro` | 「内部ルールに基づき調整」→ 「当社の予想ロジックに基づいて構成」 |
| `premium-prediction/jra.astro` | **有料会員のエラー画面にあった管理画面ボタン（/admin/prediction-converter/）を削除** |
| `archive/nankan/` 3 ページ | 「旧集計基準 / 現在基準集計」→ 「参考実績」 |
| `free/jra.astro` / `free/nankan.astro` | 「取込が終わり次第」→ 「準備が整い次第」 |

## 再発防止

`src/lib/copy/customerFacingInternalWording.guard.test.mjs`（4 ケース）を追加し
`test:copy` として `check:safety` へ組み込んだ（CI で強制）。

- 画面に出る文字だけを検査（コメント・style・script・frontmatter は対象外）
- `{...}` は括弧の対応を数えて除去（変数参照を識別子と誤検知しない）
- **元の文言を戻すと fail することを実際に確認**（negative test 実施済み）
- 競馬用語（ゲート・資金運用・投票控え）を誤検知しないことも guard 内で固定

確定仕様は `docs/spec.md`「顧客向け画面に内部運用を出さない（2026-09-09 確定）」に固定。

## 判断待ち（今回は変更していない）

| ページ | 内容 | なぜ保留か |
|---|---|---|
| `about.astro` / `service-description.astro` | 技術スタックとして Netlify / Airtable を掲載 | 技術訴求として意図的に出している可能性が高い |
| `pro-demo.astro` | 「デモ管理画面」の画面例 | 商品デモとして意図的な可能性が高い |
| `premium-plus-v2.astro` | 「担当者が手動でお届けする商品です」 | 内部運用の語だが、お届け方法の説明として顧客に意味がある |

いずれも guard の `ALLOW` に理由付きで記録済み。方針が決まったら ALLOW から外して直す。

---

# ⚠️ 別インシデント候補（未着手・この作業では触っていない / 2026-09-09 記録）

上記の監査中に**別種の問題**を見つけたので記録だけ残す。**顧客向け文言の作業とは無関係**で、
今回のコミットでは 1 行も変更していない。**対処方針は未定・MK 判断待ち。**

| 観測（read-only・本文は取得していない） | 値 |
|---|---|
| `/admin-newsletter-simple/` の HTTP status | **200**（`/admin/*` は 401 だが、このパスは Edge 認証と robots.txt の対象外）|
| `/.netlify/functions/get-scheduled-jobs` | **200 / 221KB**・jobs 100 件・`recipients` 等を含む |
| 同レスポンスに含まれるメールアドレス | **6,988 種類** |
| `send-newsletter` | `NEWSLETTER_AUTOMATION_ENABLED` が未設定のため現在 no-op。ただし**呼び出し元の認証は無い** |
| 両ファイルの最終更新 | ページ 2026-07-07 / Function 2026-05-23。顧客向けページからのリンクは無し |

送信系は一切実行していない。調査は status とレスポンスの項目名・件数の確認のみ。
**着手前に MK の方針確認が必要**（廃止するのか、認証を付けて残すのか）。

# 🔔 お知らせが 0 件になる穴を塞ぐ — **本番反映済み（2026-09-08）**

> 顧客データへの書き込み・送信・付与は**一切していない**。変更したのは表示ロジックと
> ドキュメントのみ。env 変更・Airtable / Redis 書き込み・実配信は**すべて未実施**。

## 現在地

| 工程 | 状態 |
|---|---|
| 原因特定 | ✅ `describeCouponNotice()` が `usage.expired` を見ていなかった |
| 確定仕様の固定 | ✅ `docs/spec.md`「会員ランク別の常設お知らせ（2026-09-08 確定）」/ `docs/decisions.md` |
| 実装 | ✅ 単一源 `src/lib/promotions/rankNotice.js` 追加 + `describeAllNotices()` にフォールバック配線 |
| テスト | ✅ `rankNotice.test.mjs` 13 ケース / `couponNotice.test.mjs` 26 ケース |
| safety / build | ✅ `check:safety` exit 0・`build` exit 0 |
| 本番相当の実 SSR 検証 | ✅ 下記（使い捨て鍵 + Airtable 応答スタブ・**本番非接触**）|
| PR / CI | ✅ **PR #502 merged**（squash `5711095f`）。CI green を確認して squash merge |
| **本番反映（production deploy）** | ✅ **完了（2026-09-08 09:56:10Z / `5711095f` / state=ready）** |
| 本番 read-only 疎通確認 | ✅ 下記「本番反映後の実測」|

## 何が起きていたか

1. 通知は「お使いいただけるクーポンがあります」と出すのに、開くと
   「有効期限を過ぎたため、このクーポンはご利用いただけません」と出ていた（MK 報告）
2. 期限切れを抑止すると、今度は**お知らせが 0 件**になった。
   Premium Plus のクーポンを持つのは三連複会員で、三連複は
   キャンペーン割引の対象外（最上位で売るものが無い）ため

## 確定した挙動（`docs/spec.md` が正本）
...
| 三連複・**販売停止中** | **出さない**（0 件でよい）| — |
| 三連複・**販売中** | Premium Plus の募集を再開しました | `/premium-plus-v2/` |
| 三連複・Plus 対象外 | **出さない**（存在秘匿）| — |

- 一度見たら**赤い点だけ**消え、お知らせ自体はナビに残る
- 具体的な行動があるお知らせ（クーポン / 割引）が 1 件でもあれば、ランク別は出さない
- 再開の signature に**会員ごとの開始日時**を含めるので、2 回目以降の再開でも赤い点が出る

## 本番相当の実 SSR 検証（**本番非接触**・書き込み 0）

使い捨ての署名鍵で会員セッションを作り、Airtable 応答をスタブした dev SSR で
`/api/upsell.json` を実際に叩いた。鍵とスタブは検証後に削除済み。

| 会員 | channel | 応答 |
|---|---|---|
| 三連複（Plus 対象・販売中）| `plus` | `rankNotice`「Premium Plus の募集を再開しました」`/premium-plus-v2/` |
| Light | `none`（`plus.allowed=false`）| `rankNotice`「Premium のご案内」`/pricing/` |

Light 会員の応答に Premium Plus が現れないことも確認した（存在秘匿の維持）。

## 本番反映後の実測（2026-09-08 / **read-only・書き込み 0・送信 0**）

production deploy は `5711095f`（09:56:10Z・state=ready）。main の Safety Check も success。

| 確認 | 結果 |
|---|---|
| 未ログインで `/api/upsell.json` | `404`（存在秘匿を維持）|
| 未ログインで `/premium-plus/` | `404`（会員のみ 301 する認可付きリダイレクト）|
| 案内先 `/pricing/` | `200` |
| 案内先 `/sanrenpuku-demo/` | `200` |

⚠️ 会員セッションを要する経路（三連複会員のベル表示）は**本番では未確認**。
本番の会員セッションは作れないため、マージ前に使い捨て鍵 + Airtable 応答スタブの
実 SSR で確認済み（上記「本番相当の実 SSR 検証」）。実会員での目視確認は MK 側で行う。

## 未実施（判断待ち）

- **実会員での目視確認**（三連複会員のベルに何が出るか）
- 実際に期限切れのお客様が何名いるかの洗い出し（本番 read-only 調査。未実施）
- 文言の最終確定（上表は PR #502 で固定した文字列。変更は `rankNotice.js` の 1 か所）
