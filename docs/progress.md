...
- **強い操作語**（即時販売 / 送信 / 昇格 / 販売可）は
  **管理操作 → 保存値 → 公開判定 → CTA 表示 → `purchaseEnabled`** を **E2E で確認**する
- **「即時販売」= 確定時点で即 PHASE 4 相当・CTA 表示・購入可能**（eligible 化でも段階公開開始でもない）
- dry-run / preview は**「この操作後に顧客から何が見えるか」を明示**する設計を優先
- 恒久的な回帰条件: **「管理画面で即時販売を選ぶと、その顧客の公開側 CTA が即座に表示され購入可能になる」**
