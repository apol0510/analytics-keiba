# 🔔 お知らせが 0 件になる穴を塞ぐ — **実装・検証まで完了 / Draft PR（2026-09-08）**
...
| PR / CI | 🔵 Draft PR 作成・CI 確認中 |
| merge / 本番反映 | 🔵 **未実施・ユーザー判断待ち** |
...
- Draft PR の merge / 本番反映
- 実際に期限切れのお客様が何名いるかの洗い出し（本番 read-only 調査。未実施）
- 文言の最終確定（上表はこの PR で固定した文字列。変更は `rankNotice.js` の 1 か所）
