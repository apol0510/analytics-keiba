...
### ④ 速度の見通し（94,100 通を配り切るまで）

| 構成 | free の実行 | 速度 | 完走 |
|---|---|---|---|
| 現状（最適化前・N=50）| 4 / 7 tick | 171 通/時 | 約 **22 日** |
| 最適化後・**N=50**（env 変更なし）| **5 / 7 tick** | **214 通/時** | 約 **18 日** |
| 最適化後・**N=100**（env 変更が要る）| 5 / 7 tick | **429 通/時** | 約 **9 日** |
| 参考: Background で N=500 | 7 / 7 tick | 3,000 通/時 | 約 **1.3 日** |

**env を変えなくても 22 日 → 18 日**、env を 100 にすれば **9 日**。
Background は**唯一の選択肢ではない**が、日単位まで詰めるなら差は大きい。
どこで止めるかは**送信レピュテーション**（3,000 通/時 ＝ 7.2 万通/日）との兼ね合いで決める。
