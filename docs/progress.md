...
| **「販売再開 ＋ 再募集開始」の 1 操作化**（2 段階運用は廃止 / partial success の検出と復旧つき）| ✅ **実装・テスト完了 / Draft PR**（2026-08-22）。**本番ではまだ押していない** |
...
| # | 未完了 | 誰が |
|---|---|---|
| 0 | **1 操作化を本番へ反映する**（Draft PR `feat/premium-plus-reopen-launch` の merge → deploy）| **MK の承認** |
...
6. 実顧客で **取得 → 申込 → 入金確認 → redeem** が 1 件通っている
...
1. **1 操作化の本番反映** — Draft PR（`feat/premium-plus-reopen-launch`）の
...
## 2026-08-22 — 【仕様変更】「販売再開」と「再募集開始」を **1 操作**に統合（Draft PR・**本番では未押下**）
...
- Redis の `members` HASH は空のまま。Customers / PromotionalOffers / 履歴への書き込みも 0 件
