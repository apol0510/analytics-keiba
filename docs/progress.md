...
技術実装は下の「完成済み」まで到達している。**残っているのは次の 6 つだけ**。

| # | 未完了 | 誰が |
|---|---|---|
| 1 | **実際の再募集開始日時（`reopenStartsAt`）の決定** | MK |
| 2 | **production の予約 write / `confirm-bank-payment` 配線・有効化** | 1 の決定後 |
| 3 | **admin の付与 / 取消 / 誤取得訂正 / 再発行** | MK 要否判断 → 実装 |
| 4 | **MK の最新 PC / mobile 目視確認** | MK |
| 5 | **PR #377 / #379 / #380 の merge** | MK 承認 |
| 6 | **production 反映と本番確認** | 5 の後 |
