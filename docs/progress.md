...
## 2026-08-27（追記5）— 実 Redis で検証。**8/31 が止まる不具合を発見し修正**（未 deploy）

### 実 Redis の検証結果（read-only・窓 6 分割・指紋一致）

| | 実測 |
|---|---:|
| prospect プール（送信候補）| **11,975** |
| 台帳から復元できた既送信 | **11,647** |
| いま送れる | 328（**すべて step1**）|
| **8/31 09:00 JST に送れる** | **5,863**（step1 328 / step2 **5,535**）|
| 停止（除外）| 0 |
| 開封あり | **0**（開封者は移していない）|
| delivered 分布 | 1 通 470 名 / 2 通 11,505 名（最大 2）|

### 🔴 発見 1: 8/31 の 2 通目が **1 通も出ない**状態だった（私の作り込み）

`selectNextDueStep` は**いちばん小さい due ステップ**を返す。初回接触（step1）は
自動で撃たない決まりなので、`planSequenceTick` は「いちばん小さい due が 1 なら中止」
していた。

prospect プールを cron の対象へ含めた結果、**配信台帳に行が無い人（＝step1 待ち）が
328 名対象に入り**、tick が毎回 `first_step_is_manual` で中止 →
**step2 の 5,535 名が永久に送られない**。実測で再現を確認した。

移行前は cron の対象が「この campaign の配信行がある人」だけだったので、
step1 待ちは対象に入らなかった。**移行によって初めて表面化した。**

**直した**: step1 を飛ばして次に小さい due ステップを選ぶ。
step1 待ちは従来どおり手動でしか送られず、人数は `firstStepPending` /
ログの「初回接触待ち」で必ず出す。

### 🔴 発見 2: レコードはあるが索引に無い行が 1 件（投入 11,976 / 索引 11,975）

1 ページ目が gateway timeout で切れたとき、`SET` は通ったが索引への `SADD` が
走らなかったレコードが 1 件残り、次の実行では「既にある」と判定されて
**索引へ載らないまま**になった（＝配信候補から永久に外れる・送信漏れ）。

**直した**: `addManyIfAbsent` が**既存ぶんも索引へ載せ直す**（`SADD` は冪等）。
状態が配信候補（NEW / SENDING）のものだけが対象で、除外済みは復活させない。

⚠️ **この 1 件はまだ索引に載っていない。** 修正を deploy し、
`PROSPECT_MIGRATION_ENABLED` を一時的に開けて投入を再実行すると解消する
（`addManyIfAbsent` は冪等なので既存レコードは上書きされない）。

### 数字の突き合わせ

| | 下見 | 投入 | 実 Redis | 差の理由 |
|---|---:|---:|---:|---|
| 移行対象 | 11,979 | 11,976 | 11,975 | 下見→投入で **3 名が新たに開封**／投入→索引で **1 件が索引未登録** |
| 8/31 step2 | 5,537 | — | 5,535 | 上の 3 名のうち 2 名が step2 対象だった |

取り込み由来 Customers **14,489 行 / 一意アドレス 14,489**（重複 0 組）を実測で確認。
