...
| **第 2 期 7 通**（約 11,800 名）| **約 82,600** | **約 19 日** |
| 合計 | 約 94,100 | **約 22 日** |

**支配項は第 2 期の 82,600 通**。ここを縮めない限り「3 週間」は動かない。
...
| **4** | 一括登録を **Background**（15 分枠）へ移し、正本どおり **500 名/tick** | **3,000 通/時**＝**約 1.3 日** | 設計変更＋要承認 |

- **段 1 は即効だが env 変更**なので停止しています（1.4 倍）
- **段 2 はコードだけ**で、送信量を増やさずに frequency を上げる（本 PR には含めていない）
- **段 4 が正本の設計値 500 に到達する唯一の筋**。既存パターン
