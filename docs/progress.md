...
<!-- 常設ブロック: 売上減少への対応（無料導線）。完成条件を満たすまで消さない -->
# 📉 売上減少への対応 — 無料登録の入口とファーストビュー（2026-09-27）— **実装済み・Draft PR / 本番未反映**

正本: [`decisions.md`](./decisions.md) 2026-09-27 / [`spec.md`](./spec.md)「第 1 層 `/free-prediction/`」

## 現在地（2026-09-27 売上減少調査 / Airtable read-only）

| 期間 | 入金確認（`PaidAt`）| 自然流入の新規登録 |
|---|---|---|
| 7/13〜8/16（5 週）| 平均 約 3.4 件/週 | 77 → 40 → 29 → 20 → 14（既に減少傾向）|
| 8/17〜9/26（6 週）| 平均 約 1.0 件/週（1, 1, 1, 0, 3, 0）| 6, 6, 6, 9, 15, 15 |

- 落ち始めは **2026-08-20 の `/free-prediction/` 有料版プレビュー化**（#384。無料登録の全頭解放 CTA とゲートを撤廃
  → このページから無料登録へ進む入口が消えた）と一致する。#384 自身も「無料会員獲得経路」を未確定として残していた。
- 翌 8/21 に狭い画面の左右余白を詰める変更も入っており、スマホで情報が詰まって見える状態だった。
- 元々の会員（約 1,500 名）へのメールは 8/24 週と 9/14 週には届いているが、**届かない週がある**
  （8/31・9/7 週は 0 名）。売上が立ったのは大量配信のあった週（8/10・9/14）。→ 別件調査（下記）。
- 件数が週 0〜6 件と少なく、`PaidAt` は再入金で上書きされるため、数字は傾向として読む。

## 今回確定した方針（MK / 2026-09-27）

**プレビューは維持し、昔の無料予想へは戻さない。** そのうえで、

1. `/free-prediction/{jra,nankan}` から**無料登録へ進む入口を復活**（無料会員特典への案内。全頭解放ゲートではない）
2. **ファーストビュー**を「結論・注目馬 → 短い理由 → 無料登録 CTA → 詳しい分析」の順にする（スマホ優先）
3. 詳細分析は下に残す
4. GA4 の有料化ファネル計測を壊さない（イベントは増やさない）

## 未完了任務と完成条件

| # | 完成条件 | 状態 |
|---|---|---|
| 1 | 2 ページのファーストビューに、各会場メインレースの ◎ と短い理由（公開事実のみ）が出る | ✅ 実装・テスト済み（ローカル build で表示確認）|
| 2 | 未ログインにだけ無料登録 CTA が出て `/free-signup/` へ進める。ログイン済み・有料会員には出ない | ✅ 実装・テスト済み（未ログイン表示 / ログイン済み非表示をスマホ幅で確認）|
| 3 | 有料項目（pt / AI総合指数 / 役割 / 買い目）が静的 HTML に出ない（既存 guard 維持＋新 guard）| ✅ `freePreviewFirstView{,.guard}.test.mjs` / check:free-mask 合格 |
| 4 | GA4 既存計測（`funnel-analytics.js` / 6 段ファネル）に変更なし | ✅ `test:analytics` 合格・計測コード無変更 |
| 5 | 本番反映後、GA4 で `/free-prediction/` → `/free-signup/` の遷移と新規登録数の推移を確認する | 未着手（反映後）|

## 別件（調査のみ・このブロックの完成条件ではない / 2026-09-27 read-only）

### 元々の会員（約 1,500 名）へ「送らない週」がある理由

**毎週届く定期配信は設計上 1 本も無い。** この層に届くのは期間つきの有限連続配信
（割引キャンペーン: free 3 通 / light・premium 2 通）だけで、その期間中も不具合で止まっていた。

| 週 | 原因 | 区分 |
|---|---|---|
| 8/31・9/7 | 第 1 期 step2/3 が 1 通も出ていない（停止判定と宛先条件の衝突 / カーソル固着 / tick 全体中止 / 期限で無効化。#499 で修正）| 不具合 |
| 9/7〜9/13 | 第 2 期開始直後の暴走 enqueue（JobId 欠落で全員 skip。9/14 に全件取消→修正）| 不具合 |
| 9/21 | step2 の遅れで step3 の期日が期間終了（9/24 00:00 JST）と重なった | 設計＋期間 |
| 9/28〜 | 割引 campaign 終了。native 向けに自動で動くのは DRM の少数だけ → **対策しなければ 0 名が続く見込み** | 設計 |

- light-trial 系は CSV 取込コホート限定（しかも 9/08 から kill 中）、SendGrid MC の選別配信は prospect 限定で、native は対象外。
- ライフサイクル自動配信（期限前・失効後・free→light）は `MARKETING_AUTOMATION_SCHEDULER_ENABLED` / `MARKETING_AUTOMATION_DISPATCH_ARMED` 未設定で動いていない。
- 反応なし除外・頻度上限は原因ではない。
- 対策案（**未実施・MK 判断**）: A. native 向け定期配信を正式に定義する / B. ライフサイクル自動配信のゲートを開けるか判断する / C. native を SendGrid MC の list に入れる / D. 期間つき連続配信の期間を最終 step 期日から取る。

### Premium Plus 販売停止の影響

- **現在は販売停止されていない**（`PremiumPlusSalePaused=true` は 0 名）。停止は会員ごとの操作で、
  過去に停止されていたのは**最大 3 名・2026-08-18〜09-09 の約 3 週間**だけ。
- この間の機会損失の推定は **0〜68,000 円**（0〜1 件）。**現在の停止による損失は 0 円/月**で、売上減少の主因ではない。
- 購入できる対象は最大 6 名（うち 2 名は運営者の確認用アカウントの可能性。未確認）。売れていない場合の原因は、停止ではなく母数の小ささと導線。
- 停止前の販売件数・売上は**既存データから確定できない**（Plus は Customers を書き換えない設計）。確定するには、管理者宛の注文通知メールを数えるか、admin の funnel 表示（Redis）を読み取り専用で確認する。

## 次作業

Draft PR の CI 確認まで完了。**merge・本番 deploy は MK 判断**。反映後は完成条件 5（GA4 で `/free-prediction/` → `/free-signup/` の遷移と新規登録数の推移）を見る。
