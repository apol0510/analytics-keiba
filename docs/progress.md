...
## 2026-08-25 — 【仕様確定】三連複 買い切りの閲覧権は恒久 / 旧三連複会員へ付与する

### 確定（MK）

**三連複を買い切りで購入した会員は恒久的に閲覧権を保持する。Premium 期限切れ後は
三連複ページだけ閲覧できる。** 旧プラン名で期限切れの対象者には、購入履歴の追加調査を
復旧条件にせず、顧客維持・既存顧客優遇の方針で `LifetimeSanrenpuku` を付与する。

正本: `docs/spec.md` §三連複 買い切りの閲覧権は恒久 ／ `docs/decisions.md` §2026-08-25 ／
`docs/MEMBER_TIERS.md` §2。

### 調べて分かったこと

**実装は既にこの仕様どおりだった。** 挙動の変更は不要。

| 層 | 期限切れ + 買い切り | 状態 |
|---|---|---|
| `memberResolution` | `premium-sanrenpuku` の有料セッションを発行 | ✅ 実装済み |
| `resolveEntitlements` | 三連複=可 / 馬単=不可 / Light=不可 | ✅ 実装済み |
| `paidPageGate` | 三連複ページのみ通す | ✅ 実装済み |
| `buildConfirmationFields` | `LifetimeSanrenpuku` だけ書き `有効期限` を触らない | ✅ 実装済み |

⚠️ ただし**層をまたいだテストが無かった**。ログイン判定が `free` に落ちると
`sessionPayload` が free を拒むためセッション自体が出ず、権利判定が正しくても
ページは開かない。この経路を `sanrenpukuLifetimeAccess.test.mjs` で固定した。

⚠️ `docs/MEMBER_TIERS.md` が「Premium 有効期間内のみ閲覧可能」と**実装と逆**を
書いていた。これを信じてコードを直すと買い切り購入者を締め出すため訂正した。

### 本番データの実測（読み取りのみ・書き込み 0）

| 区分 | 件数 |
|---|---|
| `LifetimeSanrenpuku=true` | 1 |
| 旧プラン名のみ（フラグ無し）| 20 |
| └ うち期限切れ = **いま三連複を見られない** | **18** |
| └ うち有効（有効期限 2098 / PlanType=Lifetime）| 2 |

18 名の有効期限は 2025-12〜2026-05 に散らばっており、期間契約の終了に見える
（買い切りなら期限は入らないはず）。**買い切りかどうかはデータから判別できない**。
だからこそ判定式は緩めず、レコードへ明示的に付与する方式を採る。

### ⚠️ 付与すると起きる副作用（要判断・未対応）

`campaignOffers.resolveCampaignOfferIdsFor` は **「三連複を見られる人＝最上位、売るものが無い」**
として空配列を返す。付与すると 18 名は

- 付与前: Light / Premium 年額 / Premium 買い切り の**割引 3 件が見えている**
- 付与後: **割引が 1 件も見えなくなる**

馬単は期限切れなので「最上位」ではない。顧客維持のための付与が、
**そのまま割引の非表示につながる**。`campaignOffers.js` は別作業が進行中のため本 PR では触らない。

### 本番レコードの変更は未実行（高リスク境界）

対象 18 名への付与は**実行していない**。対象・件数・変更フィールド・影響・rollback・
検証結果を提示して停止する。
