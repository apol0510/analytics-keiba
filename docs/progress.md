...
### ただし 21 分（tick 2 回）観測して step3 は 0 のままだった

原因は**この修正とは別**で、**rotation と時間予算の組み合わせ**にある。

| 定数 | 値 |
|---|---|
| `TICK_TIME_BUDGET_MS` | **55 秒** |
| `MIN_MS_FOR_NEXT_CAMPAIGN` | **50 秒** |
| 差（2 本目を始められる余裕）| **5 秒** |

1 本目が 5 秒を超えて終わると、`hasTimeForAnother` が false になり **2 本目以降は始まらない**。
実測所要は `free-signup-onboarding` **7 秒** / `campaign-prospect-phase2` **21 秒** /
`campaign-discount-free` **14 秒** なので、**実質 1 tick = 1 campaign**。

`resolveTickCampaignIds` は現在 **7 本**（DRM 3 ＋ 割引 3 ＋ 第 2 期）を返し、
`rotateCampaigns` が tick ごとに先頭をずらす。したがって
**`campaign-discount-free` の番は 70 分に 1 回**（7 本 × 10 分）しか来ない。

観測した 03:10Z / 03:20Z の tick では free は 6 番目・5 番目で、**一度も始まっていない**。
→ **step3 が 0 のままなのは修正の失敗ではなく、順番が来ていなかったため。**

### 影響（完成条件への到達時間）

1 回 50 名 × 70 分に 1 回 = **約 43 名/時**（50 × 60 ÷ 70）。

| 区間 | 目安 |
|---|---|
| 第 1 期 step3 の待機 5,465 名 | **約 5.3 日** |
| 第 1 期 3 通の完了 → 第 2 期の入口が開く | その後 |
| 第 2 期 7 通 → delivered 累計 10 → EXHAUSTED | さらに後 |

⚠️ **これは #562 が作った遅さではない**（#562 以前は step3 が **永久に 0**）。
   ただし「15,000 件を選別する」任務の速度としては足りない。
