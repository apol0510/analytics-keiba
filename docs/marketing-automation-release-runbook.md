> ## ⚠️ 2026-09-08 追記 — **連続配信は「開放済みだが進んでいなかった」**
>
> 本 runbook は「env を開けるまで何も起きない」段階の手順書だが、割引キャンペーン
> （2026-08-24〜09-07）は **env を開けた状態で 1 通目 15,509 通を配信済み**であり、
> **2 通目以降がコード側の 4 つの原因で 1 通も出なかった**。
> 原因と不変条件は `astro-site/docs/CAMPAIGN_SEQUENCE.md` §11、決定は `docs/decisions.md`
> （2026-09-08）、実測は `docs/progress.md` 冒頭。
>
> ### 再開する前に必ず確認する（この runbook の合格条件へ追加）
>
> | # | 確認 | 満たすまで再開しない |
> |---|---|---|
> | 1 | 修正 `fix/sequence-stall-rootcauses` が **main に入り production deploy 済み** | 未修正のまま期間を延ばすと、また 1 通目だけで止まる |
> | 2 | **キャンペーン期間**（`CAMPAIGN_WINDOW`）が、最終 step まで配り切れる長さか | `sequenceWindowFit` のテストが根拠 |
> | 3 | **PENDING 滞留ジョブ**（`recqqadkAZvDkORZ8` / 50 名）を送るか取り消すか決めてある | 修正後は追跡が片付くので**再開と同時に出る可能性がある** |
> | 4 | prospect の **step0 328 名**に 1 通目を出すのか出さないのか決めてある | 自動では出ない（初回接触は手動のまま）|
> | 5 | `rollout` の `stage` を動かすかどうか（体験シーケンス 24 通の再開判断）| 割引 3 本とは**別の判断** |
> | 6 | 直前に `sequenceMetrics` / `prospectSequenceCheck` を read-only で取り、件数を記録した | 再開後の差分を数えられるようにする |
>
> ### 再開時に出る通数の見積り（2026-09-08 実測ベース）
>
> | 宛先 | step | 通数 |
> |---|---|---:|
> | prospect（step1 済み）| step2 | **11,648** |
> | 同上 | step3（step2 の 6 日後）| **11,648** |
> | prospect（step1 未送信）| step1 | 328（**手動判断が要る**）|
> | Light 会員 | step2 | 5 |
> | Premium 会員 | step2 | 13 |
> | 滞留 PENDING（体験シーケンス step4）| — | 50 |
>
> 1 tick 500 通・10 分間隔 = **3,000 通/時**。step2 だけで約 4 時間。
