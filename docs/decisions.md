...
## 2026-08-25 — 三連複 買い切りの閲覧権は恒久 / 旧三連複会員への付与

### Status

**Accepted**（2026-08-25 / MK）。

> 旧 Premium Sanrenpuku / Premium Combo 等で現在期限切れとなっている対象については、
> 購入履歴の追加調査を復旧条件にしない。顧客維持・既存顧客優遇の方針として、
> 対象者には三連複の恒久閲覧権を付与する。

### 決めたこと

- **Premium 期限切れ後も三連複のみ閲覧可能**（馬単・Light・その他 Premium 権限は復活させない）
- **今後の新規 Sanrenpuku 買い切りも従来どおり恒久閲覧**
- **旧プラン名だけで全員を自動 Lifetime 扱いするよう判定式は緩めない**
- 対象者へは**既存の正規権限 `LifetimeSanrenpuku` を明示的に付与する方式**を使う

### なぜ「判定式を緩める」ではなく「権利を付与する」なのか

旧プラン名だけでは買い切りか期間契約かを判別できない。判定式を緩めると
期間契約が終わっただけの会員にも恒久的な権利が配られ、**誰が買い切りなのかを
以後どこからも復元できなくなる**。レコードへ付与すれば「誰に与えたか」が残り、
取り消しも 1 フィールドで済む。

### 実装は既にこの仕様どおりだった（挙動の変更なし）

調査の結果、3 層（ログイン判定 / 権利判定 / ページの入口）と入金確認の書き込みは
**すでに恒久前提で実装済み**で、コード変更は不要だった。今回加えたのは

1. 層をまたいだ回帰テスト（`sanrenpukuLifetimeAccess.test.mjs`）
2. **逆のことを書いていた `docs/MEMBER_TIERS.md` の訂正**
   （「Premium 有効期間内のみ閲覧可能」→ 実装と逆。これに合わせて直すと買い切り購入者を締め出す）

### 本番レコードの変更は別承認

対象 18 名への `LifetimeSanrenpuku` 付与は**この決定では実行しない**。
対象・件数・変更フィールド・影響・rollback・検証結果を提示し、明示承認を得てから実行する。
