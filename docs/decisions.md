...
1. **`/free-prediction/` は変更しない。** 有料版の内容・価値を見せる
   **プレビュー／サンプル**として現状のまま維持する。
...
4. 提供するのは「**全レースについて、買い目以外で無料でも実際に役立ち、
   毎日見に来る価値がある情報**」。
5. **新しい未検証の予想ロジックを勝手に作らない。** 既存の印・指数・評価・特徴量など、
   すでに確定済みの出力から候補を構成する。
...
- 新規ページのために**新しい予想データを生成しない**前提で候補を選ぶ
  （既存 JSON の再構成・再表示に留める）。`/results-showcase/` と同じく
  「新データを作らない」原則を引き継ぐ。
...
- `/free-prediction/` は「変更しない」ため、UI 改善要望が来ても
  本件を理由に解放範囲を広げてはならない。
