...
「利用予約を使用済みにする」は理由が常に同じ（入金を確認したから）ため、
**既定理由**（`COUPON_OPERATION_DEFAULT_REASON`）を持たせて手入力を不要にした。

- 既定理由は**サーバーが配る**（`action.defaultReason`）。画面で文言を作らない
- 履歴は空にしない。未入力なら既定理由をそのまま記録する
- **既定を持つのは使用済み化だけ**。付与・訂正・再発行・予約取消は
  「なぜそうしたか」が毎回違う＝そこが監査の中身なので、従来どおり理由が必須
