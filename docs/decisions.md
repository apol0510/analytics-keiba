...
| 35 | **数値の入力欄は `type="number"` にしない**（レース番号 / 払戻金額 / 払戻単価）。スピナー・ホイール・上下キーで勝手に増減し、1 桁違うと実績まで狂う。`type="text" + inputmode="numeric"` で直接入力し、数字以外は入力側で落とす | `src/pages/admin/premium-plus-results.astro` / `adminResultsUiGuard.test.mjs` |
