...
## 4. 対象件数（本番 read-only 実測 2026-08-27 / **開封適用後が確定値**）
...
Redis へ入れる `DeliveryKey` は **11,651 鍵**。
**8/31 の 2 通目**: Customers 5,865 = prospect **5,865**（step2 5,537）／片側だけ 0。
Airtable の増加: 現行 **+5,865 行** → 移行後 **0 行**。

⚠️ 開封を当てないと migrate は 12,872 になり、**893 名の開封者まで移してしまう**。
`engagementApplied:false` で実行してはいけない（投入側が fail closed で拒否する）。
