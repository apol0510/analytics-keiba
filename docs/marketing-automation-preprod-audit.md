...
> **2026-08-06 追記 — C-2 と B-3 は解消済み（PR #241 / `37090c0`）。**
> 以下は**監査時点**の記録として残す。現在の未解決は **B-4 / B-5** の 2 件。
>
> | # | 現状 |
> |---|---|
> | **C-2** | ✅ 解消。走査を Scheduled Function へ移し、同期側は Redis の写しを読む |
> | **B-3** | ✅ 解消。`runs` を直近 30 日（最大 90 日）へ |
> | **B-4** | ✅ 解消（PR #242）。本体と索引を **1 回の Lua** で更新し、tick で収束させる |
> | **B-5** | ✅ 解消（PR #242）。run 本体に **120 日 TTL** + **TTL の無い墓標**で二重開始を防ぐ |
> | B-6 | 記録のみ（開放前の基準点） |
...
| 順 | 項目 | 状態 |
|---|---|---|
| ~~1~~ | ~~C-2~~ | ✅ **解消**（PR #241 / `37090c0`）— Scheduled Function + Redis の写し |
| ~~3~~ | ~~B-3~~ | ✅ **解消**（PR #241）— `runs` を直近 30 日（最大 90 日）へ |
| ~~1~~ | ~~B-4~~ | ✅ **解消**（PR #242）— `markActive` を先にする案より強く、**CAS と索引を 1 回の Lua** にした |
| ~~2~~ | ~~B-5~~ | ✅ **解消**（PR #242）— run 本体に 120 日 TTL、二重開始の判定は **TTL の無い墓標** |
| ~~3~~ | ~~B-4 の tick 掃除~~ | ✅ **解消**（PR #242）— `reconcileActiveIndex()` を tick の先頭で実行 |

**監査で挙げた項目はすべて対応済み。**

## C-2 / B-3 の解消内容（PR #241 / `37090c0`）

| # | 何をしたか |
|---|---|
| **C-2** | 全件走査を **`cron-prospect-worker`（Scheduled Function・10 分ごと）**へ移し、同期側は Redis の写し（`ak:customer-snapshot:`）を読むだけにした。chunk 2,000 件・**meta は最後に更新**・無い / 古い（6 時間）/ 壊れているときは **fail-closed**。**公開 URL からは起動できない**（Netlify 層で 403）。管理 API は Redis に**依頼札を立てるだけ** |
| **B-3** | `runs` を **直近 30 日（最大 90 日）**へ。runId が `<automationId>#<YYYY-MM-DD>` と決定的なので**索引を増やさずに**日付を組み立てて引く |


---

# B-4 / B-5 の解消（2026-08-06 / PR #242）

## B-4: 本体と索引を 1 回の Lua で更新する

`saveDefinition`（CAS）→ `markActive`（SADD）の **2 段**が、途中で落ちると
**`get` は ACTIVE なのに `list` に出ない**（かつ scheduler も拾わない）食い違いを生んでいた。

**採った対策**（監査時の推奨より一段強い）:

| | |
|---|---|
| 原子性 | CAS の Lua を **KEYS 2 本**（def キー + `index:active`）に拡張し、**同じ Lua の中で** `SADD` / `SREM` する。Redis の Lua は単一のアトミック実行なので、**片方だけ進むことがない** |
| 索引の導出 | 索引は **`status` から導出**（`status === 'ACTIVE'` なら入れ、それ以外は外す）。呼び出し側が別途 `markActive` を呼ぶ必要が無くなった |
| 収束 | `reconcileActiveIndex()` を新設し、**tick の先頭**で毎回実行。ACTIVE でない / 本体が無い member を索引から外す |
| fail-closed | 収束は **索引から外す方向だけ**。ACTIVE なのに索引に無いものを足しはしない（送る側へ倒さない）。そうした定義は次の保存・状態遷移で自動的に入る |
| 後方互換 | `markActive` / `unmarkActive` は**冪等な補助として残す**（既存の呼び出しはそのまま動く）。キーの形も変えていない |

### 監査で挙げた「`markActive` を先にする」案を採らなかった理由

順序を入れ替えても **2 段であること自体は変わらず**、「索引にあるが DRAFT」という
別の食い違いが残る。Lua で 1 回にすれば**どちらの食い違いも起きない**。

## B-5: run の保持期間と、TTL 切れ後の二重開始防止

`saveRun` / `createRun` は TTL 無しの `SET` で、`lock` 300 秒 / `recipient` claim 7 日と
一貫していなかった。単に TTL を付けると、**TTL 切れの後に同じ runId で二重開始できてしまう**
（`createRun` の二重開始判定が run 本体の `SET NX` だったため）。

**採った対策**:

| | |
|---|---|
| 保持期間 | `RUN_TTL_SEC = 120 日`。表示は既定 30 日 / **最大 90 日**なので、**表示・監査に要る期間より短くしない**。更新のたびに張り直す |
| 二重開始の判定 | **`run-mark:<runId>` の `SET NX`（TTL 無し）**へ移した。run 本体の有無に依存しないので、TTL 切れでも二度目は通らない |
| 墓標の中身 | 値は `1` だけ。`runId` は `<automationId>#<YYYY-MM-DD>` で **PII を含まない** |
| 本体の SET | 墓標を取れた後なので **NX を外した**（TTL 切れで本体だけ消えていた場合も作り直せる） |
| 後方互換 | 旧データ（TTL 無し・墓標無し）はそのまま読める。更新すれば TTL が付き、移行は自然に進む。過去日の runId が再び due になることは無い（runId が暦日を含むため） |

### TTL の整合

```
lock 300 秒  <  recipient claim 7 日  <  run 本体 120 日  <  run 墓標（無期限）
                                  表示 30 日（最大 90 日） < run 本体 120 日
```
