...
> | B-4 | ⏳ 未対応（索引更新が 2 段。誤送信にはならないが表示が食い違う） |
> | B-5 | ⏳ 未対応（run キーに TTL が無い） |
> | B-6 | 記録のみ（開放前の基準点） |
...
| 1 | **B-4** の 3（`markActive` を先に）| ⏳ 未対応。1 行で安全側に倒せる。A-1 と同種の事故を塞ぐ |
| 2 | B-5（run キーの保持期間）| ⏳ 未対応。`lock` 300 秒 / claim 7 日と一貫していない |
| 3 | B-4 の 2（tick での索引掃除）| ⏳ 未対応。1 の後 |

**残り 3 件はいずれも未着手。** 着手は別途承認を得てから。
