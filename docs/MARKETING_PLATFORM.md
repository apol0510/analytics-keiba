...
### 現在地（2026-09-26 実測 / read-only）

詳細と数値の正本は [`progress.md`](./progress.md) 先頭の「📡 選別配信の現在地」節。

| 項目 | 状態 |
|---|---|
| 27 通の配信 | ✅ 09-20 から毎日 19:00 JST に配信中（送信済み 18 / 予約 9 / 重複 0）。最終は 09-29 |
| 反応者の除外と継続 list | ✅ 動いている（`ak-drm-engaged` 774 名）|
| delivered 10 通・無反応の打ち切り | ⏳ **09-26 19:00 の通から始まる**（delivered 9 の 6,690 名）。累計には移行前の配信も入る |
| 打ち切った人を list から外す | ⚠️ **1 回の webhook で 100 件まで**（`MAX_EXIT_PER_CALL`）。超過分は外れ残り、EXHAUSTED には provider 側 suppression が無いので**次の通が届く**。AK 側の判定は変わらない |
| 毎日の点検 | ⚠️ 毎日動いている（最終 09-25 11:20Z・途中停止なし）が、`RECIPIENT_GAP` が過去の通まで前日 list 人数と比べるため**毎日「注意」メールが出る誤検知**。本物の異常が埋もれる |
| 週 2 回配信 | 無効のまま（`SENDGRID_WEEKLY_ENABLED` 未設定）|
...
- delivered 10 通・無反応の**自動打ち切り**（EXHAUSTED → list からも外れる。⚠️ 外すのは 1 回 100 件まで＝上の「現在地」）
- 反応者を `ak-drm-engaged` へ渡す（継続配信の母集団づくり）
- 毎日の点検と異常通知（⚠️ 2026-09-26 時点で誤検知により毎日通知＝上の「現在地」）

**まだ人手が要るもの（残件）**

| # | 残件 | いま必要な人手 | 無くす方法 |
|---|---|---|---|
| 1 | ~~`ak-drm-engaged` list の作成~~ | **完了**（2026-09-19 / 空で作成）| 以後は webhook が自動で入れる |
...
### 継続 list に**既に反応済みの人**をどうするか（2026-09-19 時点 / **未実施**）

`ak-drm-engaged` を作ったのは 2026-09-19 で、**それ以前に反応した人（237 名）は入っていない**。
以後の反応者は webhook が自動で入れるが、過去分は入らない。

| 選択肢 | 中身 | 承認 |
|---|---|---|
| A. 何もしない | 次に反応したときに入る（無反応なら入らないまま）| 不要 |
| B. 過去分を 1 回だけ入れる | AK の反応済み索引から 237 名を継続 list へ upsert | **要承認**（一括投入）|

**B は一括投入にあたるため、MK の承認が要る。** 週 2 回配信を始める前に A / B を決める。
...
### 点検が動いたことをどう確かめるか（2026-09-19 の取りこぼしを受けて）

2026-09-19 の 11:20 UTC に点検の記録が残らず、**「動かなかった」のか「動いたが途中で落ちた」のか
外から区別できなかった**。区別できないと、次の一手（登録を直すのか、中の処理を直すのか）が決まらない。

- 点検は**走り出した時刻を最初に**残す（`最終起動`）。最後まで行ったら `最終実行` を残す
- `最終起動` だけ新しい ＝ **途中で落ちている**。どちらも無い ＝ **動いていない**
- 管理画面の「メール配信」タブに両方出す（片方だけだと誤読する）
- 定期実行の式は **`netlify.toml` にも書く**（コード側の `export const config` と**同じ値**）。
  食い違いはテストで落とす
...
### 管理画面（`/admin/premium-plus-eligibility/`）の強化 — **タブ 1 枚を実装済み（2026-09-19）**

「📮 メール配信」タブを足した。**読むだけ**で、送信・予約・停止の操作は持たせていない。

| 見えるもの | 中身 |
|---|---|
| 選別配信 | 次の配信日時 / 予約・送信済みの通数 / 宛先 list の人数 / 送信・到達・開封・bounce・配信停止 |
| 反応と次の導線 | AK 送信候補 / 反応済み / 継続配信 list の人数 |
| 週 2 回の一斉配信 | 有効か / 次の配信 / 予約・送信済み / 実績 |
| 毎日の自動点検 | **最終実行時刻**（1 回も動いていなければそう出す）/ 最後に知らせた時刻 / 不整合 |

⚠️ 画面は **`admin-marketing` の `mailOverview`** を叩く。
管理画面にはメール基盤の固有名詞を持ち込まない guard（`stagedReleaseGuard`）があるため、
送信基盤の名前が入った関数を画面から叩かない。数の作り方は `marketingOverview.js` に 1 つだけ置き、
移行用 API（`overview`）と画面用 API で**割れないようにする**。
