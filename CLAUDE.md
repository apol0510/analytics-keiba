...
### 言われなくても必ずやること（毎回の指示を待たない）

9. **送信を「押して終わり」にしない** — 申し込み・問い合わせフォームは、送信成功後に
   **お礼と次にすること**を出す完了画面へ画面を差し替える。入力欄だけ隠して周囲（金額・
   口座情報・手順・注意事項）を残さない。モーダル内なら先頭までスクロールして見せる。
10. **同じ画面に出る金額を 1 か所だけ更新しない** — 割引・クーポン適用時は内訳・振込金額・
    手順の金額を**すべて同時に**サーバー確定値へ差し替える。1 つでも取り残すと
    「お支払い金額」と「振込金額」が食い違ったまま客に出る。
11. **運用操作で毎回の手入力を必須にしない** — 監査に残す理由・操作者などは既定値を用意し、
    特別な事情があるときだけ上書きさせる。毎回タイプさせる設計にしない。
