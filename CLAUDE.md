...
- **メインレースのみ買い目公開**: 本命→相手5頭=**5点**（`1↔2.3.6.8.9(抑え4.5)` を抑え除去し
  ...
  - **裏目的中の表示畳み込み**: 配信買い目は双方向馬単（本命⇄相手・表裏両取り）。
    的中が「相手が1着・本命が2着」の**裏目**（例 買い目 `1↔2.3.6.8.9` / 結果 `2-1`）だと、
    `本命 → 相手5頭` の `→` 表記では結果と食い違って見える。**この裏目的中のときだけ**、
    勝った1組に畳んで `本命 ⇄ 勝った相手`（例 `1 ⇄ 2`）を `⇄` で表示する
    （相手5頭は出さない）。判定は `resultsShowcase.js` の `buildMainRace`：
    `umatan.combination` の2着が本命なら `displayArrow='⇄'` / `displayPartners=[1着相手]`。
    順目的中（本命が1着）・不的中は従来どおり `本命 → 相手5頭`（`→`）。
