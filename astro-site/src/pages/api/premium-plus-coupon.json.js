 ...
 *
 * > 買おうとした → **いまは売っていない** → 代わりにクーポンをどうぞ
 *
 * つまり配る相手は **「Plus の対象会員」かつ「いま購入できない（販売停止中）会員」**。
 * 判定は 1 か所（`resolveCouponAccess` / `resolveClaimDecision`）に集約する。
 *
 * ⚠️ 取得の条件に「再募集が開始済み」を入れてはいけない。開始＝販売再開なので、
 *    **買える人だけが取得できる**という正反対の挙動になる（2026-08-22 に一度そうなった）。
 *    開始済みかどうかは「**使えるか**（割引が乗るか）」の側の条件。
 *
