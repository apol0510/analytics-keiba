  ...
  globalThis.fetch = async (url, init = {}) => {
    ...
    if (u.includes('/Customers')) {
      ...
      if (method === 'PATCH') {
        ...
        store.writes.push({ method, records: body.records });
...
test('offers は 3 オファーと gate 状態を返す（既定は書き込み無効）', async () => {
  const { status, body } = parse(await call({ action: 'offers' }));
  assert.equal(status, 200);
  assert.deepEqual(body.offers.map((o) => o.offerId), ['light_lifetime', 'premium_trial_30d', 'comeback_full']);
  assert.equal(body.fieldsReady, false);
  assert.equal(body.writeEnabled, false);
  assert.equal(body.trialDays, 30);
});

test('一覧は特典状態と付与可否を返し、退会・停止は付与不可として出る', async () => {
  ...
  assert.equal(byId.rec6.grantable, false, '退会者が付与可能になっている');
...
test('特典の有無で絞り込める', async () => {
  const none = parse(await call({ action: 'customers', promo: 'none' }));
  assert.equal(none.body.matchedCount, BASE_CUSTOMERS.length);
  const any = parse(await call({ action: 'customers', promo: 'any' }));
  assert.equal(any.body.matchedCount, 0);
  const expired = parse(await call({ action: 'customers', contract: 'expired' }));
  assert.ok(expired.body.matchedCount >= 2);
});
...
test('dry-run は書き込みゼロ。理由別内訳と before/after を返す', async () => {
  const { status, body } = parse(await call({
    action: 'dryRun', offerId: 'comeback_full',
  ...
  assert.equal(body.willGrant, 3, '期限切れ2 + 有効Premium(Lightのみ付与)');
  assert.equal(body.skipped, 2);
  assert.ok(body.operationId, 'operationId が返っていない');
  ...
  // 有効 Premium は trial が「変更不要」として部分スキップされる
  const p3 = body.preview.find((p) => p.recordId === 'rec3');
  assert.equal(p3.partial[0].label, '有料 Premium が優先で変更不要');
});

test('dry-run は gate が閉じていても使える（確認だけ）', async () => {
  const { status, body } = parse(await call({ action: 'dryRun', offerId: 'light_lifetime', recordIds: ['rec1'] }));
  assert.equal(status, 200);
  assert.equal(body.writeEnabled, false);
  assert.equal(store.writes.length, 0);
});
...
test('フィールド未作成なら付与要求は 503 で書き込みゼロ', async () => {
  const dry = parse(await call({ action: 'dryRun', offerId: 'comeback_full', recordIds: ['rec1'] }));
  const { status, body } = parse(await call({
    action: 'grant', offerId: 'comeback_full', recordIds: ['rec1'],
...
test('有効化すると特典フィールドだけを書く（課金・契約・三連複は不変）', async () => {
  ...
  const dry = parse(await call({ action: 'dryRun', offerId: 'comeback_full', recordIds: ['rec1', 'rec8'] }));
  const { status, body } = parse(await call({
    action: 'grant', offerId: 'comeback_full', recordIds: ['rec1', 'rec8'],
    operationId: dry.body.operationId, planFingerprint: dry.body.planFingerprint,
  }));
  assert.equal(status, 200);
  assert.equal(body.mode, 'granted');
  assert.equal(body.granted, 2);
  assert.equal(body.emailSent, false);
  assert.equal(store.mailCalls, 0);

  // 書いたフィールドが allowlist だけであることを PATCH payload で検証
  ...
  assert.equal(after[PROMO_FIELDS.LIGHT_GRANTED], true);
  assert.ok(after[PROMO_FIELDS.TRIAL_UNTIL]);
});

test('複合オファーは 1 顧客 1 PATCH（片方だけ付く状態が作れない）', async () => {
  enableWrites();
  const dry = parse(await call({ action: 'dryRun', offerId: 'comeback_full', recordIds: ['rec1'] }));
  await call({
    action: 'grant', offerId: 'comeback_full', recordIds: ['rec1'],
  ...
  const patched = store.writes.filter((w) => w.method === 'PATCH');
  assert.equal(patched.length, 1);
  assert.equal(patched[0].records.length, 1);
  const fields = patched[0].records[0].fields;
  assert.ok(fields[PROMO_FIELDS.LIGHT_GRANTED]);
  assert.ok(fields[PROMO_FIELDS.TRIAL_UNTIL]);
});

test('同じ operationId で再実行しても二重付与にならない', async () => {
  enableWrites();
  const dry = parse(await call({ action: 'dryRun', offerId: 'comeback_full', recordIds: ['rec1', 'rec2'] }));
  const op = dry.body.operationId;
  await call({ action: 'grant', offerId: 'comeback_full', recordIds: ['rec1', 'rec2'], operationId: op, planFingerprint: dry.body.planFingerprint });
  const firstUntil = store.customers.find((c) => c.id === 'rec1').fields[PROMO_FIELDS.TRIAL_UNTIL];
  const writesAfterFirst = store.writes.length;

  // 同じ operationId で再 dry-run → 対象 0 件
  const dry2 = parse(await call({ action: 'dryRun', offerId: 'comeback_full', recordIds: ['rec1', 'rec2'], operationId: op }));
  assert.equal(dry2.body.willGrant, 0);
  assert.equal(dry2.body.skippedDetail.find((d) => d.reason === 'already_applied').count, 2);

  const again = parse(await call({ action: 'grant', offerId: 'comeback_full', recordIds: ['rec1', 'rec2'], operationId: op, planFingerprint: dry2.body.planFingerprint }));
  assert.equal(again.status, 400, '0 件でも書き込もうとしている');
  assert.equal(store.writes.length, writesAfterFirst, '再実行で追加の書き込みが発生した');
  assert.equal(store.customers.find((c) => c.id === 'rec1').fields[PROMO_FIELDS.TRIAL_UNTIL], firstUntil,
    '再実行で無料期間が延長された');
});
...
test('dry-run 後に対象状態が変わったら 409 で全体停止（書き込みゼロ）', async () => {
  enableWrites();
  const dry = parse(await call({ action: 'dryRun', offerId: 'comeback_full', recordIds: ['rec1', 'rec2'] }));
  // 別経路で rec2 に特典が入った状況を作る
  store.customers.find((c) => c.id === 'rec2').fields[PROMO_FIELDS.LIGHT_GRANTED] = true;
  store.customers.find((c) => c.id === 'rec2').fields[PROMO_FIELDS.TRIAL_UNTIL] = new Date(Date.now() + 86400000).toISOString();
  const writesBefore = store.writes.length;

  const { status, body } = parse(await call({
    action: 'grant', offerId: 'comeback_full', recordIds: ['rec1', 'rec2'],
    operationId: dry.body.operationId, planFingerprint: dry.body.planFingerprint,
  }));
...
test('operationId / planFingerprint が無い付与要求は拒否', async () => {
  enableWrites();
  assert.equal(parse(await call({ action: 'grant', offerId: 'comeback_full', recordIds: ['rec1'] })).status, 400);
  assert.equal(parse(await call({ action: 'grant', offerId: 'comeback_full', recordIds: ['rec1'], operationId: 'op-x' })).status, 400);
  assert.equal(store.writes.length, 0);
});

test('上限超過・未知オファー・空選択は拒否', async () => {
  enableWrites();
  assert.equal(parse(await call({ action: 'dryRun', offerId: 'nope', recordIds: ['rec1'] })).status, 400);
  assert.equal(parse(await call({ action: 'dryRun', offerId: 'comeback_full', recordIds: [] })).status, 400);
  const many = Array.from({ length: 201 }, (_, i) => `rec${i}`);
  assert.equal(parse(await call({ action: 'dryRun', offerId: 'comeback_full', recordIds: many })).status, 400);
});
...
test('書き込み失敗は途中で止め、適用済み件数と復旧手順を返す', async () => {
  enableWrites();
  const dry = parse(await call({ action: 'dryRun', offerId: 'light_lifetime', recordIds: ['rec1', 'rec2'] }));
  store.failNextPatch = true;
  const { status, body } = parse(await call({
    action: 'grant', offerId: 'light_lifetime', recordIds: ['rec1', 'rec2'],
    operationId: dry.body.operationId, planFingerprint: dry.body.planFingerprint,
  }));
  ...
  // 同じ operationId でやり直せる
  const dry2 = parse(await call({ action: 'dryRun', offerId: 'light_lifetime', recordIds: ['rec1', 'rec2'], operationId: dry.body.operationId }));
...
test('reconcile は operationId の適用状況を read-only で返す', async () => {
  enableWrites();
  // rec1 だけ付与し、同じ operationId で rec1+rec2 を突合する
  const dry = parse(await call({ action: 'dryRun', offerId: 'light_lifetime', recordIds: ['rec1'] }));
  const op = dry.body.operationId;
  const granted = parse(await call({
    action: 'grant', offerId: 'light_lifetime', recordIds: ['rec1'],
    operationId: op, planFingerprint: dry.body.planFingerprint,
  }));
  ...
  assert.equal(rc.body.total, 2);
  assert.equal(rc.body.applied, 1);
  assert.equal(rc.body.missing, 1);
  assert.deepEqual(rc.body.missingRecordIds, ['rec2']);
  assert.equal(store.writes.length, writes, 'reconcile が書き込んだ');
});

// ═══ 取り消し ════════════════════════════════════════════════════════

test('取り消しは promotional grant だけを消す（有料契約・三連複は不変）', async () => {
  enableWrites();
  const dry = parse(await call({ action: 'dryRun', offerId: 'comeback_full', recordIds: ['rec8'] }));
  await call({ action: 'grant', offerId: 'comeback_full', recordIds: ['rec8'], operationId: dry.body.operationId, planFingerprint: dry.body.planFingerprint });
  const before = { ...store.customers.find((c) => c.id === 'rec8').fields };

  const rdry = parse(await call({
    action: 'revokeDryRun', grantTypes: ['light_lifetime', 'premium_trial_30d'], recordIds: ['rec8'],
  }));
  assert.equal(rdry.body.sideEffects, 'none');
  assert.equal(rdry.body.willRevoke, 1);

  const out = parse(await call({
    action: 'revoke', grantTypes: ['light_lifetime', 'premium_trial_30d'], recordIds: ['rec8'],
    planFingerprint: rdry.body.planFingerprint, reason: '誤付与',
  }));
  assert.equal(out.status, 200);
  assert.equal(out.body.revoked, 1);
  const after = store.customers.find((c) => c.id === 'rec8').fields;
  assert.equal(after[PROMO_FIELDS.LIGHT_GRANTED], false);
  assert.equal(after[PROMO_FIELDS.TRIAL_UNTIL], '');
...
test('取り消しも gate が閉じていれば 503（書き込みゼロ）', async () => {
  const rdry = parse(await call({ action: 'revokeDryRun', grantTypes: ['light_lifetime'], recordIds: ['rec1'] }));
  assert.equal(rdry.status, 200);
  const out = parse(await call({
    action: 'revoke', grantTypes: ['light_lifetime'], recordIds: ['rec1'],
    planFingerprint: rdry.body.planFingerprint,
  }));
...
test('どの action でもメールを 1 通も送らない', async () => {
  enableWrites();
  const dry = parse(await call({ action: 'dryRun', offerId: 'comeback_full', recordIds: ['rec1'] }));
  await call({ action: 'grant', offerId: 'comeback_full', recordIds: ['rec1'], operationId: dry.body.operationId, planFingerprint: dry.body.planFingerprint });
  await call({ action: 'customers' });
  await call({ action: 'offers' });
  assert.equal(store.mailCalls, 0);
  // ScheduledEmails / CampaignDeliveries へ触れていない
  assert.equal(store.writes.every((w) => !w.table || w.table === 'Customers'), true);
});
