...
//
// 開始日は MK 確定。終わらせるときは `endsAtIso` を過去にする。
//
// ── 第 2 期（再募集 / 2026-09-10 〜 09-24）────────────────────────────
// 第 1 期（2026-08-24 〜 09-07）は 1 通目 15,509 通を配ったところで、連続配信の不具合
// （`CAMPAIGN_SEQUENCE.md` §11）により 2 通目以降が 1 通も出ないまま期間が終了した。
// 不具合は修正・本番反映済み（PR #499）なので、**同じ割引条件で期間を取り直す**。
//
// ⚠️ **この 1 か所が、メールの期限表記と購入画面の割引適用の両方の正本**。
//    メールへ日付を書き写さない（`DISCOUNT_DEADLINE` が必ずここから導出される）。
// ⚠️ 期間の長さは**連続配信を配り切れる長さ**にすること。
//    最終 step までの日数（`delayDays` の総和）が期間を超える定義は
//    `sequenceWindowFit` のテストが落とす（2026-09-08 に追加）。
export const CAMPAIGN_WINDOW = Object.freeze({
  startsAtIso: '2026-09-10T00:00:00+09:00',
  /** 開始 + 14 日（JST の 9月10日 0:00 から 9月24日 0:00 まで＝**表示は 9月23日まで**） */
  endsAtIso: '2026-09-24T00:00:00+09:00',
});
