 ...
 * 理由を入力しなくてよい操作の既定理由。
 *
 * ## なぜ全操作に用意するか（2026-08-23 / MK 指摘）
 *
 * 当初は使用済み化だけに既定理由を置き、他は手入力を必須にしていた。
 * だが**運営者は毎回それを打たされる**ことになり、実際に手が止まった
 * （「クーポン再発行　また操作理由求められる」）。
 *
 * 監査に必要な「誰が・いつ・何を」は **actor / 時刻 / 操作種別**が既に持っている。
 * そこへ意味の無い文字（「.」など）を打たせても監査の質は上がらず、作業だけ止まる。
 * **既定理由で必ず何かが残る**ようにし、書きたいことがあるときだけ入力してもらう。
 *
 * 入力があればそちらを優先する（補足を書きたいときは書ける）。
 * ⚠️ 既定理由は**その操作で常に真であること**だけを書く。事実でない理由を残さない。
 */
export const COUPON_OPERATION_DEFAULT_REASON = Object.freeze({
  grant: 'クーポンをお渡しするため',
  reissue: 'クーポンをお渡しし直すため',
  correct: '誤って取得された記録を取り消すため',
  revokeReservation: '入金確認の前に利用予約を取り消すため',
