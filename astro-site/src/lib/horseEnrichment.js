 ...
 *
 * JRA 形式: "2中3.15" → "中山 25/03/15"
 *   - 先頭の数字: 開催回（読みやすさのため非表示）
 *   - 次の漢字: 競馬場略号
 *   - 末尾の "M.D": 月.日
 *
 * JRA 紙面表記の慣用に基づくマップ:
 *   - 「中」=中山（紙面で中京は別文字「名」を使うため、「中」は常に中山）
 *   - 「名」=中京（JRA 紙面の固定慣用。名古屋地方競馬とは区別。
 *      地方の名古屋は新聞紙面で plain「名」表記にならない）
 *   - その他: 東=東京 / 京=京都 / 阪=阪神 / 新=新潟 / 福=福島 / 小=小倉 / 札=札幌 / 函=函館
 *
...
export function generateMinorHorseComment(role, gapFromHonmei) {
  ...
  // R1d-comment-v1: ユーザー可視文言を安全表現へ統一（admin docs §34.25・B案）。
  // ロジック条件・role 判定・gap 計算は不変。返却文言のみ変更。
  if (role === '連下' || role === '連下最上位') {
    if (gap <= 10) return '上位候補との差は小さく、展開が向けば評価を上げたい一頭です。';
    if (gap <= 20) return '上位候補との差はあるものの、相手候補として確認しておきたい一頭です。';
    return '能力面では一定の評価があり、展開次第で見直し余地があります。';
  }
  if (role === '押さえ' || role === '抑え' || role === '補欠') {
    if (gap <= 25) return '本線評価までは届かないものの、展開が向けば相手候補として確認したい一頭です。';
    if (gap <= 40) return 'スコアは控えめですが、上位候補に不安が出た場合は確認したい一頭です。';
    return '上位候補との差はありますが、展開次第では見直し余地があります。';
  }
  // 不要 / 無印 / その他（A〜Cいずれにも入らなかった馬）
  if (gap <= 40) return '上位候補と比べると評価は下がりますが、展開次第では確認の余地があります。';
  if (gap <= 60) return 'スコア面では上位候補に届かず、今回は評価を控えめにしています。';
  return '展開待ちの面が強く、今回は評価を抑えています。';
}
