...
test('管理画面の文言が「即時に購入できる」ことを明示している', () => {
  ...
  // ⚠️ 2026-09-09 確定仕様: 一覧の状態フィルタから「即時販売」を外した。
  //    「即時販売」は override **操作の名前**であって状態ではなく、
  //    これを状態フィルタに置いたため「いま購入できる会員」を取り逃していた。
  //    一覧は主状態 4 分類（購入可能 / 販売停止中 / 段階表示中 / 対象外）で絞り込む。
  assert.ok(!PAGE.includes('<option value="immediate">即時販売（CTA表示・購入可）</option>'),
    '「即時販売」が一覧の状態フィルタに残っている');
  assert.match(PAGE, /listStateFilterOptions\(\)/, '状態フィルタが単一源から作られていない');
  // 確認ダイアログは従来どおり残す
