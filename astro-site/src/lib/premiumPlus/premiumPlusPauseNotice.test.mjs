...
test('購入を煽る表現を含めない', () => {
  ...
  // ⚠️ 「いま買わないと損」と思わせる希少性の演出は入れない。
  //    ここは**買えなかった方へのお詫び**の画面であって、販促面ではない。
  assert.doesNotMatch(html, /好評|大反響|満席|完売|残りわずか|今だけ|お急ぎ/);
});

test('売れない理由は「申込の集中」であり、購入を促す文脈で使わない', () => {
  // MK 確定文言（2026-08-22）。「お申し込みが殺到しており、ただいまご案内できません」。
  // 事実確認できない表現を一律禁止していたが、**売れない理由そのもの**なので例外とする。
  // 代わりに、この語が販促（購入 CTA・価格）と同じ画面に出ないことを機械的に固定する。
  const html = renderPauseNoticeHtml({ coupon: notClaimed });
  assert.match(html, /殺到/, 'ご案内できない理由が伝わらない');
  assert.doesNotMatch(html, /今すぐ|お申し込みはこちら|購入する/, '断りの画面で購入を促している');
});
