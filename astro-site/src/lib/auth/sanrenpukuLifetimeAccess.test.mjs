...
// ── 5. 旧三連複会員は「付与」しない（2026-08-25 MK 方針変更）────────────
//
// ⚠️ 旧 Premium Sanrenpuku / Premium Combo で期限切れの 18 名には
//    **LifetimeSanrenpuku を付与しない**。Light 永久無料会員として再スタートさせる。
//    その仕様と検証は `entitlements/legacySanrenpukuNormalization.test.mjs`。
//    ここでは「判定式を緩めていないこと」だけを固定する。

/** 旧プラン名・フラグ無し・期限切れ（＝正規化の対象になる形）*/
