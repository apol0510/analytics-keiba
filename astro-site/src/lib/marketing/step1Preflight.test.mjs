...
// ══════════════════════════════════════════════════════════════
//  ジョブ一覧が「新しい順に一部だけ」になった後の判定（2026-08-15〜）
//
//  `jobs` は台帳が大きく全件返せないため上位 N 件しか返らない。
//  **見えている範囲に無いこと**を「無い」と読み替えると、古いジョブを
//  見落として二重にキュー登録しうる。確認できないものは ok にしない。
// ══════════════════════════════════════════════════════════════

test('【重要】一覧が絞られていて対象ジョブが見えないときは ok にしない', () => {
  const r = run({ jobs: okJobs({ jobsTotal: 152, jobsShown: 30, jobsTruncated: true }) });
  assert.equal(r.ok, false, '見えないだけかもしれないのに通している');
  const row = r.failures.find((f) => f.label.includes('既存ジョブ'));
  assert.ok(row, JSON.stringify(failed(r)));
  assert.match(row.detail, /確認できません/);
});

test('絞られていても対象ジョブが見つかれば「ある」と言い切れる', () => {
  const r = run({
    jobs: okJobs({
      jobs: [{ jobId: 'mkt-x', campaignId: CAMPAIGN, status: 'SENT' }],
      jobsTotal: 152, jobsShown: 30, jobsTruncated: true,
    }),
  });
  assert.equal(r.ok, false);
  const row = r.failures.find((f) => f.label.includes('既存ジョブ'));
  assert.match(row.detail, /既存=1 件/);
});

test('全件を見ている（絞られていない）ときだけ「0 件」と言える', () => {
  const r = run({ jobs: okJobs({ jobsTotal: 1, jobsShown: 1, jobsTruncated: false }) });
  assert.equal(r.ok, true, `落ちた項目: ${JSON.stringify(failed(r))}`);
  const row = r.checks.find((c) => c.label.includes('既存ジョブ'));
  assert.match(row.detail, /全件を確認/);
});

test('取得範囲を必ず表に出す', () => {
  const r = run({ jobs: okJobs({ jobsTotal: 152, jobsShown: 30, jobsTruncated: true }) });
  const row = r.checks.find((c) => c.label === 'ジョブ一覧の取得範囲');
...
/** 2026-08-15 の本番実測そのもの（10 名は既に queued） */
const queuedState = () => {
  const seq = okSequence();
  ...
  return { sequence: seq, trialGrant: tg, jobs };
};
...
test('【重要】いまの 10 名（queue 済み）で走らせたら止まる', () => {
  ...
  assert.ok(labels.some((l) => l.includes('既送信')), 'Step1 既送信を見ていない');
  assert.ok(labels.some((l) => l.includes('既存ジョブ')), '既存ジョブを見ていない');
  assert.ok(labels.some((l) => l.includes('Step1')), '次が Step1 でないことを見ていない');
});

test('【重要】queue 済みかどうかを人数の一致だけで判断しない', () => {
  // 「10 名」だけ合わせても、既送信とジョブがあるので通ってはいけない
  const st = queuedState();
  st.sequence.next = { step: 1, recipients: 10, truncated: false, cap: 500, recordIds: Array.from({ length: 10 }, (_, i) => `rec${i}`) };
  const r = run({ ...st, expectRecipients: 10 });
  assert.equal(r.ok, false);
});

/** 次のコホート（100 名・まだ 1 通も出していない） */
const freshCohort = (n = 100) => {
  ...
  seq.summary = {
    total: n, due: n, waiting: 0, completed: 0, stopped: 0,
    byStopReason: {}, dueByStep: { 1: n, 2: 0, 3: 0, 4: 0 },
    sentByStep: { 1: 0, 2: 0, 3: 0, 4: 0 }, byCurrentStep: { 0: n }, balanced: true,
  };
  ...
  // 過去ジョブはあるが、このキャンペーンの分はまだ無い（全件見えている）
  const jobs = okJobs({ jobsTotal: 1, jobsShown: 1, jobsTruncated: false });
  return { sequence: seq, trialGrant: tg, jobs };
};

test('【重要】次の未 queue コホートでは通る（1 回きりの道具にしない）', () => {
...
test('次のコホートでも既送信・ゲート・関所の条件は同じように効く', () => {
  const st = freshCohort(100);
  // 既に一部へ送っていたら止まる
  const a = { ...st, sequence: { ...st.sequence, summary: { ...st.sequence.summary, sentByStep: { 1: 5, 2: 0, 3: 0, 4: 0 } } } };
  assert.equal(run({ ...a, expectRecipients: 100 }).ok, false);
  // 実送信ゲートが開いていたら止まる
  assert.equal(run({ ...st, jobs: okJobs({ dispatchEnabled: true }), expectRecipients: 100 }).ok, false);
  // 関所と人数がズレたら止まる
