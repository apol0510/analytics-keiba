 ...
 *
 * ## いまの値の根拠は「本番実測」だけ（2026-09-17 訂正）
 *
 * ⚠️ **本番で安全が確認できている人数は `50` だけ。** それ以外の人数は
 *    **live の full-index 実測が無い**ので、安全と扱ってはいけない。
 *
 * ### なぜ 100 を取り下げたか
 *
 * 一度 100 に上げたが、根拠にした所要時間は **`sequenceTickPreview`（下見）の実測**だった。
 * 下見は **窓で切られている**:
 *
 ...
 *
 * したがって**下見の時間から live を見積もると必ず短く出る**。
 * 実際、下見ベースで「1 tick に 4〜5 本入る」と見積もったが、
 * 本番は **3 本**しか入らなかった（2 サイクルで再現）。
 * 同じ誤りで出した「**100 名で約 24 秒**」「**50 名で約 19 秒**」も**未検証**である。
 *
 * ### 上げるときに必要なもの
 *
 * **live の full-index 実測**（その人数で 1 campaign が
 * `MAX_CAMPAIGN_MS` に収まることを本番で確かめること）。
 * 下見・窓・シミュレーションだけを根拠に上げてはいけない。
 *
 ...
 * ## 500 にしたいなら
 *
 * `MAX_RECIPIENTS_PER_TICK`（= 500）は**正本の設計値**だが、同期 tick には入らない。
 * 500 を使うなら一括登録を Background function（15 分）へ移すこと（既存パターン:
 * `drm-entry-background` / `marketing-campaign-dispatch-background`）。
 * **env だけ上げてはいけない。**
 */
export const SYNC_TICK_MAX_RECIPIENTS = 50;
