...
test('guard: 昇格・決済・特典フィールドを書かない', () => {
  ...
  const code = codeOnly(FN) + codeOnly(MODEL) + codeOnly(ELIG) + codeOnly(CATALOG);
  ...
  // そもそも書き込み HTTP メソッドを持たない（この Function 群は読み取り専用）
  assert.equal(/method:\s*['"](POST|PATCH|PUT|DELETE)['"]/.test(codeOnly(FN)), false);
});
...
test('guard: 使う env は AK 既存のものだけ', () => {
  ...
  assert.deepEqual([...new Set(envs)].sort(), [
    ...
    'MARKETING_ADMIN_SECRET', 'MARKETING_CAMPAIGN_DISPATCH_ENABLED', 'MARKETING_CAMPAIGN_ENABLED',
...
test('guard: Phase A では有効化・実行・取消を配線しない', () => {
  assert.match(FN, /action === 'enable' \|\| action === 'run' \|\| action === 'cancel'/);
  assert.match(FN, /not_wired_phase_a/);
  assert.match(FN, /json\(501,/);
});

test('guard: preview は副作用なしと明示する', () => {
  const i = FN.indexOf('async function handlePreview');
  const body = FN.slice(i, FN.indexOf('// ── status', i));
...
const AUTO_SECTION = PAGE.slice(
  PAGE.indexOf('自動配信（下見のみ）'),
  PAGE.indexOf('</section>', PAGE.indexOf('自動配信（下見のみ）')),
);

test('guard(ui): 自動配信の画面がある', () => {
  assert.ok(PAGE.includes('自動配信（下見のみ）'), '自動配信セクションが無い');
  assert.ok(AUTO_SECTION.length > 200);
});

test('guard(ui): 管理者が画面だけで設定と影響を確認できる', () => {
  for (const label of ['automationId', '状態', '次回実行日時', '対象予定人数', '母数', '除外',
    '送信済み', '失敗', '最大送信件数', 'quiet hours', '使用テンプレート', 'contentHash', '前回実行結果']) {
    assert.ok(AUTO_SECTION.includes(label), `画面に「${label}」が無い`);
  }
});

test('guard(ui): Phase A の未配線ボタンは既定で無効', () => {
  for (const id of ['autoEnable', 'autoRun', 'autoPause', 'autoCancel']) {
