...
export const FREE_SIGNUP_ONBOARDING_STEPS = Object.freeze([
  ...
  step({
    ...
    body: [
      ...
      '',
      '予想ページでは、出走する全頭の馬名・騎手・調教師・斤量・父・性齢と、',
      '近走の詳細データを公開しています。',
      '',
    ...
    // ⚠️ 2026-09-15 に本番の実ページで突き合わせた結果、**2 種類のズレ**が見つかった。
    //    メールは「受信者が実際に確認できるもの」だけを書くため、**現在の実画面**に合わせている。
    //
    //    (a) 語彙の違い（同じものを別の語で呼んでいる。公開範囲は一致）
    //        厩舎 → **調教師**（DTO は `trainer`）/ 過去走 → **近走**（「近走詳細データ」・DTO は `recent`）
    //
    //    (b) **正本・公開 DTO と実画面の不一致**（語彙の問題ではない）
    //        `枠` … `docs/spec.md` は無料公開側に記載し `buildFreePublicRows()` も
    //               `frame` を DTO に持たせているが、**実画面に per-horse の表示が無い**
    //        `通算成績` … 同じく正本は公開事実として扱っているが、**実画面に表示が無い**
    //
    //    ⚠️ (b) は**既存の不一致**であり、どちらが正しいかは未確定。
    //       ここでは**メールの文面だけ**を実画面に合わせ、公開範囲・UI・正本は変更しない。
    //       追跡は `docs/progress.md`「正本/公開DTO と実画面の既存不一致（枠・通算成績）」。
