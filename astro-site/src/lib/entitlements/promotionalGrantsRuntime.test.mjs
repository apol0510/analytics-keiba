...
const LIGHT_GRANT = {
  ...
  [PROMO_FIELDS.LIGHT_GRANTED_AT]: iso(NOW - DAY),
};
const trialGrant = (untilMs) => ({
  [PROMO_FIELDS.TRIAL_UNTIL]: iso(untilMs),
  [PROMO_FIELDS.TRIAL_GRANTED_AT]: iso(NOW - DAY),
});
...
test('A. Light 永久無料: Light が見える / Premium は見えない', () => {
  ...
  assert.equal(e.canViewPremium, false);
  assert.equal(e.promo.lightLifetimeActive, true);
...
test('A. Light 永久無料: ログインできる（plan=light）', () => {
  ...
  assert.equal(m.reason, MEMBER_REASON.PROMO_LIGHT_LIFETIME);
});
...
test('B. Premium 無料期間中は Premium が見える', () => {
  const e = E({ ...EXPIRED_PREMIUM, ...trialGrant(NOW + 29 * DAY) });
  ...
  assert.equal(M({ ...EXPIRED_PREMIUM, ...trialGrant(NOW + 29 * DAY) }).normalizedPlan, 'premium');
});

test('B. 無料期間が終わると Premium は消える（元の状態へ戻る）', () => {
  const fields = { ...EXPIRED_PREMIUM, ...trialGrant(NOW - 1) };
  const e = E(fields);
  assert.equal(e.canViewPremium, false);
  assert.equal(e.canViewLight, false);
...
test('B. 有料 Premium が有効な会員の権利を縮めない', () => {
  ...
  const withTrial = { ...paid, ...trialGrant(NOW + 30 * DAY) };
...
test('C. Premium 30日無料 ＋ Light 永久無料 → 期間中 Premium / 終了後 Light', () => {
  const fields = { ...EXPIRED_PREMIUM, ...LIGHT_GRANT, ...trialGrant(NOW + 30 * DAY) };
...
test('三連複（LifetimeSanrenpuku）は特典の影響を受けない', () => {
  const noSrp = E({ ...EXPIRED_PREMIUM, ...LIGHT_GRANT, ...trialGrant(NOW + 10 * DAY) });
...
test('三連複の購入資格は無料特典では開かない（有料 Premium だけ）', () => {
  const trial = E({ ...EXPIRED_PREMIUM, ...trialGrant(NOW + 10 * DAY) });
...
test('Premium Plus 販売資格は無料特典で動かない（premiumActive は有料のみ）', () => {
  ...
  const withTrial = resolvePlusMemberFromFields({ ...base, ...trialGrant(NOW + 20 * DAY) }, { nowMs: NOW });
...
test('停止・退会・強制ログアウトは特典があっても権限を得ない', () => {
  const grants = { ...LIGHT_GRANT, ...trialGrant(NOW + 20 * DAY) };
  for (const bad of [
    { Status: 'suspended' },
    { Status: 'banned' },
...
test('テストアカウントは閲覧権限を得ず、そもそも付与対象にならない', () => {
  const fields = { ...EXPIRED_PREMIUM, Status: 'test', ...LIGHT_GRANT, ...trialGrant(NOW + 20 * DAY) };
  const e = E(fields);
...
test('壊れた特典データ（取り消し済みなのに値が残る）では権限を与えない', () => {
  const fields = {
    ...EXPIRED_PREMIUM,
    [PROMO_FIELDS.LIGHT_GRANTED]: true,
    [PROMO_FIELDS.LIGHT_GRANTED_AT]: iso(NOW - 10 * DAY),
    [PROMO_FIELDS.LIGHT_REVOKED_AT]: iso(NOW - DAY),
  };
