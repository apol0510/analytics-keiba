/**
 * importRecentHorseHistoriesNankan.test.mjs — recentHorseHistories 取得の認証/失敗伝播契約テスト
 * （node:test / 新規依存なし / 全 mock fetch・実通信なし）
 *   node --test scripts/importRecentHorseHistoriesNankan.test.mjs
 */
import { test } from 'node:test';
import assert from 'node:assert/strict';
import { fetchSharedRaw, buildSharedPath } from './importRecentHorseHistoriesNankan.js';
...
const PATH = buildSharedPath('2026-05-22', 'OOI'); // nankan/recentHorseHistories/2026/05/2026-05-22-OOI.json

// 1. 200 → 本文を返す
test('1. 200 は本文を返す', async () => {
  const payload = { category: 'nankan', schemaVersion: 'nankan-recent-horse-histories-1', date: '2026-05-22', venue: 'OOI', races: [] };
  const client = clientWith(() => mkRes(200, payload));
  const body = await fetchSharedRaw(PATH, client);
...
test('3. 401 は AUTH_FAILED で throw（raw fallback なし）', async () => {
  ...
  await assert.rejects(fetchSharedRaw(PATH, client), (e) => e.code === SHARED_FETCH_CODES.AUTH_FAILED);
});
...
test('4. timeout は TIMEOUT で throw', async () => {
  ...
  await assert.rejects(fetchSharedRaw(PATH, client), (e) => e.code === SHARED_FETCH_CODES.TIMEOUT);
});
...
test('5. token 未設定は TOKEN_MISSING（fetch 未実行）', async () => {
  ...
  await assert.rejects(fetchSharedRaw(PATH, client), (e) => e.code === SHARED_FETCH_CODES.TOKEN_MISSING);
...
test('6. token・Bearer が error へ漏れない', async () => {
  const client = clientWith(() => mkRes(401, 'bad'));
  await assert.rejects(fetchSharedRaw(PATH, client), (e) => {
    const hay = `${e.message}\n${e.stack}`;
