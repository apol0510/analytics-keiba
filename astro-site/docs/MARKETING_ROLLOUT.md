...
### 関所は「付与 1 回ごと」（付与側と同じ条件で待つ）

**付与側にも同じ関所がある**（`buildPlanFromSelection` の `evaluateStep1Barrier`）。
あちらは「前回付与ぶんの Step1 が**送り終わる**まで付与しない」で、
**queue しただけでは開かない**。運転手だけ緩めても付与側が `waiting_for_step1` で断るので、
運転手は付与側と同じ条件（未処理 0）で待つ。

    付与 200 → Step1 queue → 送信 → 未処理 0 → 次の付与 200 → …

- 論理バッチ 500 名は **200 + 200 + 100** の 3 回で満たす（**間に queue / 送信が入る**）
- 付与 1 回 = **3 tick**（付与 / queue / 送信起動）。cron 2 分で **1 回 6 分**
- 13,900 名 ≈ 70 回 × 6 分 ≈ **7 時間**

⚠️ 2026-08-18 の事故: ここだけ「論理バッチ単位」に緩めた結果、200 名を付与 → queue した
直後の 2 回目の付与が付与側に断られ、「予定があったのに 0 件」で自動停止した
（安全側だが前へ進めない）。**運転手と付与側の関所は必ず同じ条件にする。**

### 付与の可否は**ローカル状態**で決める（読み取り遅延に依らない）

⚠️ Airtable は**付与直後の行がすぐ読めないことがある**。関所（`outstandingStep1`）だけを
   見ると「開いている」と誤認して次を配ろうとし、付与側（自分で読み直す）が
   `waiting_for_step1` で断る → 「予定があったのに 0 件」で自動停止する（2026-08-18 に 2 度）。

- **まだ queue していない引き継ぎ（`pendingHandoffOps`）がある限り付与しない**
- 引き継ぎがあるときは、送信待ちジョブがあっても**先に queue**する
- 付与側の `waiting_for_step1` は**待ち**として扱い、自動停止しない
  （`too_many_records` などの本物の異常は従来どおり停止）

### 一時停止でも queue / 送信は流す

`paused` は**新規付与だけ**を止める。tick 自体は動かし、積み残しの queue 登録・送信は進める。

⚠️ 2026-08-18 の事故: 停止中に tick を早期リターンさせていたため、
   自動停止した時点で **queue 済み 197 通が送信されないまま滞留**した。
