...
| 最終報告は §完了報告の簡潔化 の 7 項目を primary とする（§Completion report）| **上の 6 項目が既定**。従来の「未実施の高リスク操作 / 次に必要な承認 / progress の現在地」は **「未完」「承認が必要なこと」「現状」に含めて書く**（別書式で二重に書かない）|
...
（実施 / 現状 / 未完 / ユーザーの手動作業 / 承認が必要なこと / branch・commit・PR・CI）。
