...
## 7. 本番の状態（2026-08-12 時点）

| 項目 | 状態 |
|---|---|
| deploy | `81813ab5`（#313）/ production ready |
| 管理画面の engagement パネル | 配信済み（read-only で実測） |
| 除外の適用 | **まだ適用されない見込み**（記録開始直後のため `no_open_recorded`） |
| env | **変更なし**。`MARKETING_ENGAGEMENT_GUARD` 未設定（= auto） |

**「除外 0 人」は正常。** 反応の記録が貯まり、記録開始以降に 10 通届いた無反応の相手が
現れて初めて除外が始まる。現状は 1 人あたりの生涯送信回数が最大 2〜4 回のため、
到達には数か月単位で送信を重ねる必要がある。
未完了の観測条件は `docs/progress.md`（2026-08-12 の項）に記載。
