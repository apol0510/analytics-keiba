...
> **この機能の目的**: 「買おうとした → いまは売っていない → **代わりにクーポンをどうぞ**」。
> 配る相手は**いま購入できない会員**。**取得は再開前に行われるのが前提**で、
> 再開後（買える状態）に新規取得はさせない。
...
3. `resolveCouponAccess()` / `resolveClaimDecision()`（単一源
   ...
   - **いま販売を停止している**（＝買えない。だから埋め合わせに配る）
4. 対象外は **404**（存在秘匿。401/403 は使わない）／
   **いま購入できるなら 409 `plus_on_sale`**（配る必要がない）／ gate off は **503**
5. 既に取得済みなら **PATCH せずに 200**（冪等・取得日時を上書きしない＝二重取得なし）
6. PATCH 失敗は **503**（「取得した」と言わない）

⚠️ **取得条件に「再募集が開始済み」を入れないこと。** 再募集の開始＝販売再開なので、
   入れると**「買える人だけが取得できる」＝目的と正反対**になる（2026-08-22 に発生）。
⚠️ **使えるのは「取得済み ＋ その会員の再募集が開始済みで期限内」**。
   未開始のうちは予約 write も申込への適用も fail closed。
...
| 面 | いつ出るか |
|---|---|
| `/premium-plus/`（受付休止ページ）| **買おうとして買えなかったとき**に出る案内ページ。ここが本来の入口 |
| `/premium-plus-coupon/`（クーポンページ）| 取得済み、または**いま取得できる**とき。取得の実行もここから |
| `/dashboard/`（マイページ）| 同上（2026-08-22 追加。取得ページを知らなくても辿り着けるように）|
...
| 新規取得 | **できない**（買えるようになったので埋め合わせが要らない）。取得は**停止中に**行われるのが前提 |
