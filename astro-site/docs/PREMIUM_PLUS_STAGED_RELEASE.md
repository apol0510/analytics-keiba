...
**UI は 2026-07-30 に刷新（表示のみ。write 契約・preview 契約・判定ロジックは不変）。**

- **管理接続**: 折りたたみパネル。接続後は「管理API 接続済み」の小さな表示だけ残る（secret は非表示）
- **サマリー**: カード型。優先度順に 即時販売 / 販売可 / 保留 / 候補 / 販売対象外、ROUTE A・B は補助表示
- **顧客カード**: 1 会員 1 カード
  - 上部: Email（長い場合は省略 + `title` で全文）＋ **状態バッジ**
  - 中段: ラベル付きグリッド（プラン / 三連複 / Route / PHASE / Premium経過 / 販売許可日 / 最終更新）
  - 内部メモがあれば専用枠で表示
  - 「プラン: … / 三連複: … / route: …」の 1 行連結表示は**禁止**（guard テストで固定）
- **状態バッジ**: 保留 / 段階公開中 PHASE n / 販売中 PHASE 4 / 即時販売 / 販売対象外
- **操作の分離**:
  - 通常操作 … `[表示プレビュー]` `[段階公開で販売可]` `[保留]`
  - 強い操作 … `[今すぐ販売可]`（確認ダイアログ必須・強調）`[販売対象外]`（危険色）
  - **現在の状態と同じ操作は disabled**（「適用中」を明示）
- **フィルター**: すべて / 保留 / 販売可 / 即時販売 / 販売対象外 / ROUTE A / ROUTE B（クライアント側のみ）
- **Email 検索**: ブラウザ内フィルター。API を追加で呼ばない
- **並び順**: 保留 → 販売可・段階公開中 → 即時販売 → 販売対象外。同一グループ内は最終更新の新しい順
- **レスポンシブ**: 860px / 640px でレイアウトを 1 列化。Email は横スクロールさせず省略表示
