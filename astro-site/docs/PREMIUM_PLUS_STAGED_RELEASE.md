...
⚠️ **2026-08-22 変更**: これは**独立した安全スイッチ**であって、再募集の通常フローの操作ではない。
**「販売を再開する」を通常の再募集フローで単独に押させない**（主操作は「この会員の再募集を開始する」）。
画面はサーバーの判断（`reopenLaunch.action.showPauseSwitch` / `showResumeSwitch`）に従って
**状態ごとに 1 つだけ**スイッチを出す:

| 会員の状態 | 出すスイッチ |
|---|---|
| 再募集 未開始 ＋ 販売停止中 | **出さない**（主操作の「再募集を開始する」が同時に再開する）|
| 再募集 未開始 ＋ 販売中 | 「⏸ 販売を一時停止する」だけ |
| 開始済み ＋ 販売中 | 「⏸ 販売を一時停止する」だけ（緊急停止）|
| 開始済み ＋ 販売停止中（**開始後に停止**）| 「▶ 販売を再開する」だけ |
| **販売再開が未完了**（開始前の停止が残っている）| 出さない（再募集セクションの「販売再開をやり直す」を使う）|
| 確認できない | 出さない |
...
#### 有効期限（2026-08-19 MK 確定 / 開始日時の決め方は 2026-08-21 確定）
...
| 操作の粒度 | **1 操作**（販売再開 ＋ 開始日時の確定 ＋ 14 日間の開始）。2 段階運用は廃止 |
...
##### 1 操作の中身（**2 つの保存先にまたがる。順序が意味を持つ**）

`reopenStartsAt` は Redis、`salePaused` は Airtable にある。**分散トランザクションは無い**ので、
順序と失敗時の意味を設計で固定している（単一源 `premiumPlusReopenLaunch.js`）。

```
① 前提をすべて確認（gate / Airtable read / Redis read）── 1 つでも欠ければ **何も書かない**
② 排他を取る（既存 primitive couponOperationLock を再利用。取れなければ書かない）
③ lock 後に**読み直して**判断し直す（TOCTOU を閉じる）
④ Redis:    HSETNX でその会員の開始日時（**冪等**。既にあれば書かない）
⑤ lock 検証（奪われていたら書かない）
⑥ Airtable: 販売停止の解除（**必要なときだけ** PATCH。既に false なら PATCH しない）
```

**④ を先にする理由**: ⑥ が落ちて残るのは「開始済み・販売は停止したまま」＝
**お金の経路は閉じたまま**（fail closed）。逆順だと「販売だけ開いて再募集期間が
始まっていない」＝購入経路だけ開く、という悪い側に倒れる。
④ は `HSETNX` なので**再送しても開始日時が変わらない**＝同じボタンで復旧できる。

⚠️ **「停止中なのに販売停止フィールドが未有効」なら、開始日時も書かない**
（開始だけ確定して売れない片側状態を作らない）。

##### 途中成功（partial success）を曖昧にしない

応答は `startWritten` / `saleResumed` を**別々に**返す。⑥ だけ落ちた会員は
**`incomplete`（販売再開が未完了）**として admin に出し、件数も一覧に出す
（放置すると「期限だけ進んで買えない」会員になるため）。
復旧は**同じボタンの再送**（「▶ 販売再開をやり直す」）で、開始日時は変わらない。

##### 「緊急停止」と「途中成功」を取り違えない（**この設計の要**）

どちらも見た目は「開始済み ＋ 販売停止中」で同じ。区別は**停止の時刻**でつける:

| 判定 | 意味 | この操作で自動再開してよいか |
|---|---|---|
| `pausedAt < startsAt` | 開始操作の⑥が未完了（**途中成功**）| **よい**（再送で復旧）|
| `pausedAt >= startsAt` | 開始後に運営者が**意図的に緊急停止**した | **だめ**（409 で断る）|
| `pausedAt` が読めない | 判別できない | **だめ**（安全側。明示的な「販売を再開する」を使わせる）|

##### 冪等性

- 開始日時: `HSETNX`（**再送で変わらない**）
- 販売再開: 既に `false` なら **PATCH しない**（監査日時を無駄に動かさない）
- 排他: `couponOperationLock` を**会員ごとの再募集エンティティ**（`computeReopenLockId`）で取得
- 監査: `PremiumPlusSalePausedBy` に `reopen-start op=<computeReopenOperationId>` を残す
  （**現在時刻を材料にしない**ので再送でも同じ値）
...
| 面 | 使う |
|---|---|
| 受付休止ページ `/premium-plus/` `/premium-plus-v2/` | `describeCouponForMember({ def })` |
| クーポンページ `/premium-plus-coupon/` | 同上 |
...
> **Superseded（2026-08-22）**: 「取得できるのは**販売一時停止中**の対象会員だけ」という
> 旧条件は**無効**。再募集の開始が販売停止の解除を含む 1 操作になったため、
> 停止を条件にすると**開始した瞬間に取得できなくなる**。
> **「販売再開前に先に取得させる」は通常運用にしない。**

`POST /api/premium-plus-coupon.json`

1. `ak_session` を検証（未ログイン → 404）
2. 対象レコードは**セッション由来の recordId のみ**。body の id / email は読まない
3. `resolveCouponAccess()` / `resolveClaimDecision()`（単一源
   `premiumPlusCouponAccess.js`）が次を要求する:
   - **Plus の対象会員**（`plusAudience.isPlusAudience`＝**停止を外したら**商品ページを見られる人）
   - **その会員の再募集が開始済み**（`reopenStartsAt` が確定）
   - **その会員の期限内**（開始 + 14 日）
4. 対象外は **404**（存在秘匿。401/403 は使わない）／
   **未開始・期限切れは 409**（理由を返す）／ 読み取り不能・gate off は **503**
5. 既に取得済みなら **PATCH せずに 200**（冪等・取得日時を上書きしない＝二重取得なし）
6. PATCH 失敗は **503**（「取得した」と言わない）

⚠️ **`salePaused` は取得資格の条件ではない。** 販売停止中でも開始済み・期限内なら取得できる
（購入はできないまま＝購入 gate は従来どおり）。
⚠️ **未開始の会員は取得も予約も申込（クーポン適用）もできない**（fail closed）。

URL 直打ち・fetch の直接呼び出し・古いタブからの再送でも同じ判定を通る。
**画面が CTA を出していたかどうかは根拠にしない。**

### 取得できる場所（**3 面・判定は同じ単一源**）

| 面 | いつ出るか |
|---|---|
| `/dashboard/`（マイページ）| 取得済み、または**いま取得できる**とき（2026-08-22 追加）|
| `/premium-plus-coupon/`（クーポンページ）| 同上。取得の実行もここから |
| `/premium-plus/`（受付休止ページ）| 販売停止中の対象会員に出る案内ページ。開始済みなら取得 CTA |

⚠️ 旧実装はマイページに取得導線が無く、**クーポンページを知らないと辿り着けなかった**
（本番で実際に迷いが発生）。3 面とも判定は `resolveCouponAccess()` の同じ結果を使う。

### クーポンページ `/premium-plus-coupon/`

会員が自分の状態だけを見る。クーポン名 / 取得済み / 取得日時 / 有効期限 /
現在の受付状況 / 取得できるなら取得 CTA。**一覧・検索・他人指定の経路を持たない**ので、
他会員のクーポンは構造的に見えない。`noindex` + robots.txt `Disallow`。
取得済みでも取得もできない相手には **404**（存在秘匿）。
...
| 新規取得 | **できる**（2026-08-22 修正）。条件は「Plus の対象会員 ＋ その会員の再募集が開始済みで期限内」で、**販売停止中かどうかは条件ではない** |
| 販売資格 / PHASE | 停止前の状態がそのまま戻る（クーポンは判定に一切影響しない） |

### 再募集するときの手順（**1 操作**・2026-08-22 更新）

1. 管理画面の「クーポン取得済み」などで**再募集する対象会員を決める**
2. **その会員の詳細パネル**「再募集（この会員）」で
   **「▶ この会員の再募集を開始する」**を押す。**この 1 操作で同時に**:
   - その会員の**販売一時停止を解除**する
   - **押下時のサーバー時刻**をその会員の `reopenStartsAt` として初回確定する
   - その時刻から**14 日間**のクーポン期限が開始する

   対象が複数なら**会員ごとに繰り返す**。
3. 必要なら `promotionOfferCatalog.js` に Premium Plus 用の `purchase_offer` を追加し、
   既存の offer 発行経路（`promotionalOffer.js` / `PromotionalOffers`）で offer を発行する

> **Superseded（2026-08-22）**: 「①再募集を開始 →②販売停止を外す」の**2 段階運用は廃止**。
> 「販売を再開する」を**単独で押させる通常フローは無い**。
> 旧記述（「2 の開始操作は販売停止を解除しない」「2 と 3 は別操作」）は**無効**。

⚠️ **選んでいない会員には販売も予約も開かない**（未開始のまま fail closed）。
⚠️ 段階的に開けると、**会員ごとに期限が違う**（各自の開始 + 14 日）。これは仕様どおり。
⚠️ 開始後の緊急停止は「**販売を一時停止する**」（独立した安全スイッチ）で行う。
   停止しても**開始日時と期限は変わらない**。
...
**現行実装**では、クーポンは管理者が付与・取消できない（顧客の取得でしか増えない）。
詳細パネル「再募集クーポン」は取得状況・取得日時・取得元を**読むだけ**。

**これは MK が承認した確定仕様ではない。** PR #370（`b84e6afb` / 2026-08-18）で
Claude が「顧客の意思表示だから管理者は触らない」と判断して省いたもの。
当初の指示は「admin で取得済み・未取得・取得日時を確認できるようにする」＋任意の
フィルタ / 件数追加であり、**付与・取消は要求も禁止もされていなかった**。

そのため現状では、**取消・誤取得の訂正・代理付与は Airtable の直接編集に依存する**
（該当 3 列を空にする / 値を入れる）。これを運用として許容するか、admin に操作を
足すかは **MK の判断待ち**。→ `docs/progress.md` 冒頭の未完了リスト 3・4 を参照。
...
| **取得・使用できるかの単一源**（停止とは別軸）| `src/lib/premiumPlus/premiumPlusCouponAccess.js` |
