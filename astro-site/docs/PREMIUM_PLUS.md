...
既存の合計はそのまま。**同じ JSON に `bySource` を足すだけ**にする。

```json
{ "firstAt": …, "lastAt": …, "count": 5,
  "bySource": { "dashboard": { "firstAt": …, "lastAt": …, "count": 3 } } }
```

- 古い値には `bySource` が無い → **その分は全量「クリック元不明」**
- **不明 = 合計 − 内訳の和**（保存しない・導出する）
- Redis のキーは増やさない（読み取りの往復も増えない）

##### ⚠️ 推測で振り分けない

**導線別の計測を始める前に記録された分は「クリック元不明」のまま置く。**
...
30 分の重複除外は**合計の `lastAt` だけ**を見る。
導線ごとに別々に数えると内訳の和が合計を超え、「不明」が負になる。
