# 管理画面と販売停止（2026-09-09 確定 / 正本は docs/spec.md）

会員詳細は **状態 → 次の操作 → （折りたたみ）詳細** の順。
**細かい文章を読まなくても運営できる**ことが完成条件（眼精疲労対策）。

- 状態は大きな 1 行 + 色バッジ + 段階 1 行。**PHASE / override / eligibility を主表示にしない**
- 販売状態は**トグル 1 つ**（「販売中」を押すと停止 / 「販売停止中」を押すと再開）
- **販売停止 = その会員に結果・商品内容を見せない**（購入 CTA だけを止めるのではない）
  - 実績画像 `premium-plus-media` GET も停止中は **404**（URL 直打ちで迂回できない）
  - クーポンページは**見せる**（停止中こそ配る相手）
- すでに購入できるなら「今すぐ販売可」は**出さない**
- 「再募集を開始する」→「**クーポンの利用期間（14日）を開始する**」へ改称

確定仕様の全文: `docs/spec.md`「Premium Plus 管理画面と販売停止（2026-09-09 確定）」
単一源: `astro-site/src/lib/premiumPlus/premiumPlusAdminStatusSummary.js`
