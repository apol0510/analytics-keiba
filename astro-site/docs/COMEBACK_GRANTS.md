...
## 9-2. E-5（実顧客への初回配信）— **実施済み**（2026-07-31）

> PR #189 は**実行前の判断資料**で、母集団の数値は 2026-07-31 時点のスナップショット
> （Customers 1,446 名）だった。E-5 はその後**実行済み**で、当時の判断資料としての
> 役目は終わっている。ここには **2026-08-08 に本番から read-only で再取得した実績**だけを残す。

### 配信実績（`CampaignDeliveries` の実測 / GET のみ）

| 項目 | 実測値 |
|---|---|
| `comeback-offer` 系の配信行 | **70** |
| うち `sent` | **69** |
| うち `skipped-duplicate` | **1** |
| 送信日 | **2026-07-31 のみ**（08:00:56Z 〜 15:18:06Z = JST 17:00〜翌 00:18）|
| `PromotionalOffers` の残存 | **0 件**（発行したオファーは revoke 済み）|

`skipped-duplicate` が 1 件あるのは、**重複メールアドレスを offer 発行前に fail-closed で
除外した**ため。台帳上も「送っていない」ことが残っている。

### 恒久的に有効な運用ルール

1. **オファー発行と案内メールは別工程。** `PromotionalOffers` へ発行 → 受信者ごとの専用 URL で
   案内 → 使われなければ revoke、という順序を崩さない。
2. **重複メールは offer 発行より前に落とす。** 発行後に気づくと「捨てオファー」が残る。
3. **送信のたびに env を開け、完了後は必ず閉じて記録する。**
   閉鎖の実測（dispatcher `dryRun:false` が 503）まで確認して初めて完了とする
   → `docs/progress.md`「メール送信 gate の再閉鎖」参照。
4. **母集団の数値はスナップショット。** 本書に固定値を書き写さない
   （2026-07-31 の 1,446 名は 2026-08-08 時点で 1,682 名）。判断のたびに read-only で取り直す。
