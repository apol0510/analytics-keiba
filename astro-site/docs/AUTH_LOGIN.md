...
**2026-09-10 変更（MK 指示）**: 事前の長い案内はやめ、**結果だけを 1 か所で伝える**。
...
| 場所 | 現在 |
|---|---|
| ログインメール（`send-magic-link.js`）| **削除**。別ブラウザで開きたい人向けのコピー用 URL の案内だけ残す |
| `/login/` の `no_session` 文面 | **削除**。「もう一度ログインしてください」だけにする |
| `/auth/verify` 成功画面 | **削除**。「ログインしました。まもなくマイページへ移動します。」だけにする |

つまり**この事象は画面で説明しない**。仕組みの説明はこの docs とソースのコメントに残す。
ユーザー向けの文言に内部事情を書かない方針は `docs/spec.md`
「仕組みの説明も出さない」が正本で、`userFacingNoInternals.guard.test.mjs` と
`scripts/e2e-magic-link-verify.mjs`（実ブラウザ）が強制する。

削除した文言を**戻さない**ことは `loginReasonNotice.guard.test.mjs` が強制する。
...
> ⚠️ **「失効しなくなる」わけではない。**
> `sessionStart` 起点の**絶対 TTL 90日**（`ABSOLUTE_SESSION_TTL_MS`）は延長されない。
> どれだけ頻繁にアクセスしても**初回ログインから最長 90 日で `reject`** になり、
> 再度マジックリンク認証が必要になる。ここで回避できるのは idle TTL だけ。
> `sessionRefresh.test.mjs` の「半 TTL 以内の訪問なら idle 失効しない。ただし絶対 TTL 90日で
> 必ず reject」が、延長で無期限化しないことを固定している。
