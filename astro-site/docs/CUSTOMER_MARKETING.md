...
## 2-4. 絞り込みの説明と表示名（2026-08-03）

### 単一源

表示名・説明は **`src/lib/marketing/filterDefinitions.js`** に集約する。
画面に文言を直接書かない。ここから **ボタン名 / チェック項目 / チップ /
条件要約 / 説明ポップオーバー** をすべて生成する。

```js
FILTER_DEFINITIONS.mkOfferState = {
  label: 'オファー',
  description: '顧客に発行済みの割引・購入オファーの状態で絞り込みます。',
  options: [{ value: 'live', label: '申込可能なオファーあり', description: '…' }, …],
}
```

- **`value` は API の許可値そのもの**。表示名を変えても value は変えない
- `diffAgainstAllowed()` が API 許可値と定義のズレ（説明の無い選択肢）を検出する
- 定義の無い項目・値は**言い換えない**（勝手な文言を作らない）

### 内部用語を画面に出さない

| 変更前 | 変更後 |
|---|---|
| 不明（legacy） | 状態を判定できない旧データ |
| 有効オファーあり | 申込可能なオファーあり |
| 期限 7 日以内 | オファー期限が7日以内 |
| 特典中 | 無料利用権が有効 |
| 販売可 / 保留 / 販売対象外 | 販売できる / 保留（確認待ち）/ 販売対象外 |
| 送信可能 / 送信除外 | 送信できる / 送信できない |
| ROUTE A（三連複）/ ROUTE B（Premium 30日） | 三連複の購入から / Premium 加入30日から |

### 説明の出し方（hover に頼らない）

各フィルター名の右と、各選択肢の右に **ⓘ** を置く。

- **タップ／クリックで開く**（スマホ・タブレットでも読める）
- 同時に開くのは **1 つだけ**、Esc と外側クリックで閉じる
- 画面右端でははみ出さない位置へ寄せる（`z-index: 200`）
- 幅 320px（`max-width: calc(100vw - 16px)`）、**900px 以下では画面下部のシート**
- ⓘ を押しても**チェック状態は変わらない**（`stopPropagation`）
- `aria-label` / `aria-expanded` 付き

### Email 個別検索は補助機能

通常のセグメント検索と混同しないよう、**既定は折りたたみ**（`<details>`）。

- 見出し「🔎 Email で個別検索」＋「特定の顧客をメールアドレスで探す場合だけ使用します。」
- 入力があるときだけ **「Email 条件あり」** バッジを出し、**開いたまま保つ**
- desktop は最大 420px（全幅を占有しない）／スマホは展開時のみ全幅
- 「クリア」で 1 タップ解除
...
## 4-2. 送信ごとの文面編集（2026-08-03）

件名・本文の**正本はコード**（`campaignCatalog.js`）。ただし実運用では「今日の対象者向けに
一文足したい」が毎回発生するため、**テンプレートを土台に、今回送る分だけ**を管理画面で
編集できるようにした。
...
| 編集できる | 編集できない |
|---|---|
| 件名 / 本文 | campaignId / version / 対象条件（audienceRule）/ CTA URL |

- 編集は**その送信だけ**に効く。`campaignCatalog.js` は書き換わらない
- 「既定文面に戻す」でテンプレートの内容を読み直せる
- **テンプレートそのものの更新は別機能**（本 PR には含めない）。通常の送信操作で
  既定テンプレートを上書きしない

### 検証（fail closed）

`src/lib/marketing/campaignContentDraft.js`（純粋）が単一源。

| 条件 | 扱い |
|---|---|
| 件名が空 / 改行を含む / 200 文字超 | **エラー**（送れない） |
| 本文が空 / 5000 文字超 | **エラー** |
| `{{salutation}}` 以外の差し込み | **エラー**（空文字へ黙って置換しない） |
| `{{` の閉じ忘れ | **エラー** |
| HTML タグ | **エラー**（本文は文字として送られる） |
| 本文中の生 URL | **エラー**（リンクは CTA 経由） |
| 件名 40 文字超 / 宛名の差し込みなし | 警告（送信は止めない） |

差し込みは **`{{salutation}}` だけ**。カタログの `CAMPAIGN_PLACEHOLDERS` と一致させる
（存在しない変数を管理画面に増やさない）。

### 確認した文面だけが送れる

1. **dry-run** … 送信と同じ `renderCampaign` で描画し、`contentHash`（sha256 先頭 16）を返す
2. 画面は「確認した文面」として hash ごと保持する
3. **件名・本文を 1 文字でも変えると失効**し、キュー登録・送信ボタンが押せなくなる
   （`件名または本文が変更されたため、送信対象と内容をもう一度確認してください`）
4. `planFingerprint` の種にも `contentHash` を含めるため、文面を変えたトークンは Function 側で 409
5. キュー登録時、Function は受け取った `contentHash` を**再計算して照合**する（不一致は 409）

### スナップショット（保存先は既存フィールドのみ）

**新しい Airtable テーブル・フィールドは作っていない。**

| 保存先 | 内容 |
|---|---|
| `ScheduledEmails.Subject` | 確定した件名 |
| `ScheduledEmails.Content` | 確定した本文（描画済み HTML） |
| `ScheduledEmails.Notes` | `marketing campaign <id> v<n> content:<hash12>[ edited]` |

dispatcher（`marketing-campaign-dispatch.js`）は**この保存済みスナップショットで送る**。
`campaignCatalog.js` から作り直さないため、キュー登録後に管理画面で文面を編集しても
**登録済みジョブの内容は変わらない**（retry も同じ内容）。

### 最終確認

「今すぐ送信」の前に、件名全文 / 本文プレビュー / キャンペーン名・version / 対象人数 /
除外人数 / 通常配信・テスト専用 / 内容 hash / operationId を出し、
**「表示されている件名・本文を、この対象者へ送信します」にチェックするまで送信できない**。

検証: `npm run test:marketing`
（`campaignContentDraft.test.mjs` / `campaignEditorState.test.mjs` /
`campaignContentSnapshot.guard.test.mjs` / `adminMarketingHandler.smoke.test.mjs`）

## 4-3. 結果パネルの単一源（2026-08-03）

キャンペーンの dry-run 結果を出す場所は **Step 4 だけ**。

- 施策パネルからキャンペーンを外した（同じ確認が 2 か所に出て、どちらが最新か分からなくなっていた）
- 施策パネルは「特典・オファーの下見」に限定し、折りたたみへ移した
- 連打・多重リクエストで結果パネルが積み上がっていたため、実行世代（seq）で
  古い応答を捨て、常に**最新 1 件だけ**を描くようにした
- 特典側の除外は `skippedPreview`（人物単位）を読む。集計しか見ていなかったため、
  除外が 1 件でもあると常に「誰が対象か確定できません」になっていた
...
### 🎛 顧客マーケティング運用画面（2026-08-01 追加）

一覧・カルテを「見るだけ」から「**状態を把握 → 抽出 → 判断 → 実行 → 追跡**」へ広げた。
**表示系はすべて read-only**（Customers / Offers / Deliveries / ScheduledEmails へ 1 バイトも書かない）。

| 機能 | 単一源 | 何をするか |
|---|---|---|
| 時系列履歴 | `marketing/customerTimeline.js` | 既存台帳の値だけを新しい順に並べ、**行ごとに出所（台帳.列）を併記** |
| 推奨アクション | `marketing/recommendedActions.js` | 「次に何をするか」を理由・使用データ・送信可否・最短実行日時つきで提案（**自動実行しない**）|
| 配信結果の取得 | `marketing/deliveryActivity.js` | 配信基盤へ **GET のみ**。開封・クリックは詳細を取れた通だけ |
| 一覧の運用列 | `admin-marketing` の `marketingColumns()` | 閲覧権限 / 無料特典 / 有効オファー / 次回送信可能時刻 |

#### 取得できないもの（推測で埋めない）

| 情報 | 状態 | 画面での扱い |
|---|---|---|
| 問い合わせ履歴 | **台帳が存在しない**（Base のテーブル一覧に無い）| 「取得できません」と明示 |
| 過去の契約の増減 | 契約履歴テーブルが無い。Customers は**現在値のスナップショット** | 最新 1 件ぶんのみ表示し、注記を出す |
| オファー取消の日時 | `PromotionalOffers` に取消日時の列が無い | **日時不明**として末尾に分離（近い日付で代用しない）|
| 開封・クリック | AK に保存していない。配信基盤の保持期間が短い（実測 3 日） | 取得できた通数を明記。**取得不能を 0 件と表示しない** |

> 恒久的に反応を追う設計は **`EMAIL_EVENT_LEDGER.md`（Phase 1）** に分離した。
> Event Webhook は署名検証つきで既に稼働しており、届いたイベントを append-only の
> `EmailEvents` へ残す実装が **既定 OFF** で入っている（テーブル作成と env 投入で有効化）。
> ⚠️ マーケ配信は現状 `custom_args` を刻んでいないため、届くイベントは `email` しか
> 手掛かりが無く **`unresolved`（顧客へ結び付けない）** になる。紐付けには送信側の刻印が要る。

#### 推奨アクションのルール

判定は既存の単一源へ委譲し、画面側で契約・送信可否を再実装しない。

| 状況 | 推奨 |
|---|---|
| 送信除外（bounce / complaint / 配信停止 / 停止 / test） | **送信禁止**（最優先で表示）|
| 24 時間以内に送信済み | 次に送れる時刻を表示（この制限は下げない）|
| 有効な有料契約 | カムバック施策の**対象外**と明示 |
| 有効オファーあり・未申込 | 期限前リマインド（残り 7 日以内は警告色）|
| 期限切れ × 30 日以内にログイン | カムバック案内（`expired-comeback`）|
| 期限切れ × 31 日以上 / 記録なし | Light 無料体験（`light-30d-free`）|
| 無料特典の期間中 | 終了日と、終了 7 日前からの案内時期 |
| 開封あり・クリックなし | 文面の見直し（**取得できているときだけ**）|
| クリックあり・未申込 | 個別フォロー |
| Premium Plus eligible / review | 対応候補（review は送信不可として表示）|

#### 施策パネル

選択中の顧客に対して実行できる施策を並べるが、**このパネルから実行されるのは dry-run だけ**。
対象件数・変更前後・除外理由・operationId・取り消し方法を表示し、
実行は各タブの既存フロー（確認 → 実行）に委ねる（新しい実行経路を増やさない）。

#### 絞り込み

契約 / プラン / 送信可否 / Plus 資格 / 送信履歴 / 最終ログイン / オファー / 無料特典 / 送信タイミング
を **AND** で組み合わせる。適用中の条件と該当件数を画面に常時表示する。

#### 画面幅

列が増えたため「主要 / 詳細」の切替を用意し、720px 以下では詳細列を隠す
（詳細はカルテで確認する）。
...
| 機能 | 単一源 | 内容 |
|---|---|---|
| 選択集合の更新 | `marketing/campaignPlanView.js` の `updateSelection()` | 「全顧客から選択」「表示中のみ選択」「全解除」。**recordId が正本**でメールは識別子にしない |
...
| ボタン | 対象 | 絞り込みへの依存 |
|---|---|---|
| **全顧客から選択** | いま読み込んでいる全件（`mkData.rows`）のうち送信可能な相手 | **依存しない** |
| **表示中のみ選択** | 検索語・AND 絞り込みを通った表示行のうち送信可能な相手 | 依存する |
| **全解除** | 選択を空にする | — |

どちらも `updateSelection()` へ委譲し、画面側に選択集合の更新ロジックを持たない。
「全顧客から選択」は**追加件数と合計を必ず画面に出す**（大量選択を黙って行わない）。

#### 人物単位の対応が確定できないときは対象者一覧を出さない

権威 API が recordId 付きの除外明細を返さない場合、「除外に載っていない = 対象」と
みなすのは**推測**になる（集計だけでは誰が落ちたのか分からない）。
そのまま「この人に実行されます」と出すと、実際には送られない相手を対象として見せてしまう。

次を満たすときだけ人物一覧を作る（`detailComplete=true`）。満たさなければ
**`executable=false` + included を空**にし、理由を画面へ出す。

- 除外が 1 件以上あるなら、**その全件に recordId が付いている**
- 空 recordId・重複 recordId・**選択していない recordId** が混ざっていない
- 選択に重複が無く、`選択 − 除外明細 = 対象件数` が一致する

API 側（`admin-marketing` の dry-run）も `detailComplete` を返し、
明細が 1 件でも欠けていれば false を宣言する（画面だけで補完しない）。
確定できない場合でも、**分かっている除外者は表示**する（手掛かりを消さない）。
...
この操作の副作用（dry-run は `none`）。

> ⚠️ **キューの取り消し API は存在しない**（2026-08-01 実測）。
> キュー登録後は「実配信しなければ送られない」だけで、`ScheduledEmails` に PENDING が残る。
> rollback 表示でも「取り消せる」と書かず、`未配信のまま停止・キュー取消機能は未実装` と出す。
> 配信後は**取り消し不能**。
