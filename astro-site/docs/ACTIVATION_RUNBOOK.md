...
### キュー登録の途中終了で「送られてしまう」ことは無い（2026-08-20〜）

キュー登録は次の順で進む。**印が付いている間、dispatcher はそのジョブを送らない。**

1. `ScheduledEmails` にジョブを作る（`Notes` に `queue:unverified` の印を付ける）
2. `CampaignDeliveries` を `DeliveryKey` 冪等で upsert する
3. 配信行の実在を**読み戻して確認**する（足りなければ補完 → それでも駄目なら巻き戻す）
4. 確認できたときだけ**印を外す**（＝送ってよい状態にする）。外せたことも読み戻して確かめる

途中で実行が終わっても、残るのは**印が付いたままのジョブ**＝ dispatcher が
`blocked: 'queue_unverified'` で弾くので、**配信行が無いのに送られることはない**。
積み直すときは `JobId`（plan fingerprint 由来）が同じなので**既存行を作り直して使う**
（同じ JobId の行を 2 つ作らない）。**送信済みのジョブは積み直さない**（409）。

⚠️ キュー登録と cron tick は **Redis の排他**を取る。取れない / 確かめられないときは
**1 バイトも書かずに終わる**（`409 busy` / `503 unavailable`）。
