...
async function handleProspectSequenceCheck({ now, req }) {
  ...
  // ⚠️ 全件を 1 回で見ると Function の実行時間を超える（実測 504）。
  //    索引の窓（offset / limit）で分割して呼ぶ。既定 2,000 件。
  ...
  // 復元は 1 回だけ（同じ入力を 2 回作らない。時間もかかる）
  ...
  return json(200, {
    ...
    sideEffects: 'none',
    campaignId,
    window: { offset: from, limit, indexSize: inputs.indexSize, nextOffset: from + inputs.prospects.length < inputs.indexSize ? from + inputs.prospects.length : null },
    pool: counts,
